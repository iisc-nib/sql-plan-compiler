...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
HT_18.insert(cuco::pair{KEY_18[ITEM], ITEM*TB + tid});
BUF_18[(ITEM*TB + tid) * 1 + 0] = ITEM*TB + tid;
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_20 = get_aggregation_slot(KEY_20[ITEM], HT_20, SLOT_COUNT_20);
...
int* d_SLOT_COUNT_20;
cudaMalloc(&d_SLOT_COUNT_20, sizeof(int));
cudaMemset(d_SLOT_COUNT_20, 0, sizeof(int));
//Aggregate in hashtable
...
main_22<<<std::ceil((float)COUNT20/(float)TILE_SIZE), TB>>>(COUNT20, d_MAT21aggr0__tmp_attr0, d_MAT_IDX21, d_aggr0__tmp_attr0, d_cycles_per_warp_main_22_materialize_21);
uint64_t MATCOUNT_21 = 0;
cudaMemcpy(&MATCOUNT_21, d_MAT_IDX21, sizeof(uint64_t), cudaMemcpyDeviceToHost);
