...
__global__ void count_1(uint64_t* COUNT0, DBI32Type* date__d_yearmonthnum, size_t date_size) {
...
reg_date__d_yearmonthnum[ITEM] = date__d_yearmonthnum[tid + ITEM];
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (tid + ITEM < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
count_1<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT0, d_date__d_yearmonthnum, date_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_date__d_datekey, d_date__d_yearmonthnum, date_size);
//Create aggregation hash table
