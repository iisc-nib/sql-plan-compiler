...
__global__ void count_10(uint64_t* COUNT15, DBI32Type* date__d_yearmonthnum, size_t date_size) {
...
count_10<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT15, d_date__d_yearmonthnum, date_size);
uint64_t COUNT15;
