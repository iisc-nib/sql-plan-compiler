...
__global__ void count_10(uint64_t* COUNT28, DBStringType* supplier__s_region, size_t supplier_size) {
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < supplier_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
atomicAdd((int*)COUNT28, 1);
}
}
template<typename HASHTABLE_INSERT>
__global__ void main_10(uint64_t* BUF_28, uint64_t* BUF_IDX_28, HASHTABLE_INSERT HT_28, int64_t* cycles_per_warp_main_10_join_build_28, int64_t* cycles_per_warp_main_10_selection_11, int64_t* cycles_per_warp_main_10_selection_12, int64_t* cycles_per_warp_main_10_selection_13, int64_t* cycles_per_warp_main_10_selection_14, int64_t* cycles_per_warp_main_10_selection_9, DBStringType* supplier__s_region, DBI32Type* supplier__s_suppkey, size_t supplier_size) {
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < supplier_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_28 = atomicAdd((int*)BUF_IDX_28, 1);
HT_28.insert(cuco::pair{KEY_28[ITEM], buf_idx_28});
BUF_28[(buf_idx_28) * 1 + 0] = ITEM*TB + tid;
}
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_10_join_build_28[blockIdx.x] = cycles_per_warp;}
}
__global__ void count_4(uint64_t* COUNT29, DBStringType* customer__c_region, size_t customer_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
template<typename HASHTABLE_INSERT>
__global__ void main_4(uint64_t* BUF_29, uint64_t* BUF_IDX_29, HASHTABLE_INSERT HT_29, DBI32Type* customer__c_custkey, DBStringType* customer__c_region, size_t customer_size, int64_t* cycles_per_warp_main_4_join_build_29, int64_t* cycles_per_warp_main_4_selection_3, int64_t* cycles_per_warp_main_4_selection_5, int64_t* cycles_per_warp_main_4_selection_6, int64_t* cycles_per_warp_main_4_selection_7, int64_t* cycles_per_warp_main_4_selection_8) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_4_selection_3[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < customer_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_4_selection_5[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < customer_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_4_selection_6[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < customer_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_4_selection_7[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < customer_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_4_selection_8[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
...
if (!selection_flags[ITEM]) continue;
KEY_29[ITEM] = 0;
KEY_29[ITEM] |= reg_customer__c_custkey[ITEM];
}
// Insert hash table kernel;
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < customer_size); ++ITEM) {
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_4_join_build_29[blockIdx.x] = cycles_per_warp;}
}
__global__ void count_16(uint64_t* COUNT30, DBStringType* part__p_mfgr, size_t part_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
template<typename HASHTABLE_INSERT>
__global__ void main_16(uint64_t* BUF_30, uint64_t* BUF_IDX_30, HASHTABLE_INSERT HT_30, int64_t* cycles_per_warp_main_16_join_build_30, int64_t* cycles_per_warp_main_16_selection_15, int64_t* cycles_per_warp_main_16_selection_17, int64_t* cycles_per_warp_main_16_selection_18, DBStringType* part__p_mfgr, DBI32Type* part__p_partkey, size_t part_size) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_16_selection_15[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < part_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_16_selection_17[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < part_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_16_selection_18[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
...
if (!selection_flags[ITEM]) continue;
KEY_30[ITEM] = 0;
KEY_30[ITEM] |= reg_part__p_partkey[ITEM];
}
// Insert hash table kernel;
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < part_size); ++ITEM) {
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_16_join_build_30[blockIdx.x] = cycles_per_warp;}
}
__global__ void count_1(uint64_t* COUNT31, size_t date_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
template<typename HASHTABLE_INSERT>
__global__ void main_1(uint64_t* BUF_31, uint64_t* BUF_IDX_31, HASHTABLE_INSERT HT_31, int64_t* cycles_per_warp_main_1_join_build_31, int64_t* cycles_per_warp_main_1_selection_0, int64_t* cycles_per_warp_main_1_selection_2, DBI32Type* date__d_datekey, size_t date_size) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
__global__ void count_20(uint64_t* BUF_28, uint64_t* BUF_29, uint64_t* BUF_30, uint64_t* BUF_31, HASHTABLE_PROBE HT_28, HASHTABLE_PROBE HT_29, HASHTABLE_PROBE HT_30, HASHTABLE_PROBE HT_31, HASHTABLE_INSERT HT_33, DBI16Type* customer__c_nation_encoded, DBI32Type* date__d_year, DBI32Type* lineorder__lo_custkey, DBI32Type* lineorder__lo_orderdate, DBI32Type* lineorder__lo_partkey, DBI32Type* lineorder__lo_suppkey, size_t lineorder_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
if (!selection_flags[ITEM]) continue;
KEY_29[ITEM] = 0;
KEY_29[ITEM] |= reg_lineorder__lo_custkey[ITEM];
}
...
if (!selection_flags[ITEM]) continue;
KEY_30[ITEM] = 0;
KEY_30[ITEM] |= reg_lineorder__lo_partkey[ITEM];
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_customer__c_nation_encoded[ITEM] = customer__c_nation_encoded[BUF_29[slot_second29[ITEM] * 1 + 0]];
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_33 = HT_33.find(KEY_33[ITEM])->second;
aggregate_sum(&aggr0__tmp_attr0[buf_idx_33], reg_map0__tmp_attr1[ITEM]);
KEY_33date__d_year[buf_idx_33] = reg_date__d_year[ITEM];
KEY_33customer__c_nation_encoded[buf_idx_33] = reg_customer__c_nation_encoded[ITEM];
}
...
__global__ void count_35(size_t COUNT33, uint64_t* COUNT34) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
if (!selection_flags[ITEM]) continue;
atomicAdd((int*)COUNT34, 1);
}
}
__global__ void main_35(size_t COUNT33, DBDecimalType* MAT34aggr0__tmp_attr0, DBI16Type* MAT34customer__c_nation_encoded, DBI32Type* MAT34date__d_year, uint64_t* MAT_IDX34, DBDecimalType* aggr0__tmp_attr0, DBI16Type* customer__c_nation_encoded, int64_t* cycles_per_warp_main_35_materialize_34, DBI32Type* date__d_year) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
MAT34customer__c_nation_encoded[mat_idx34] = reg_customer__c_nation_encoded[ITEM];
...
auto main_10_selection_9_cpw_size = std::ceil((float)supplier_size/(float)TILE_SIZE);
...
auto main_10_selection_11_cpw_size = std::ceil((float)supplier_size/(float)TILE_SIZE);
...
auto main_10_selection_12_cpw_size = std::ceil((float)supplier_size/(float)TILE_SIZE);
...
auto main_10_selection_13_cpw_size = std::ceil((float)supplier_size/(float)TILE_SIZE);
...
auto main_10_selection_14_cpw_size = std::ceil((float)supplier_size/(float)TILE_SIZE);
...
//Materialize count
uint64_t* d_COUNT28;
cudaMalloc(&d_COUNT28, sizeof(uint64_t));
cudaMemset(d_COUNT28, 0, sizeof(uint64_t));
count_10<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT28, d_supplier__s_region, supplier_size);
uint64_t COUNT28;
cudaMemcpy(&COUNT28, d_COUNT28, sizeof(uint64_t), cudaMemcpyDeviceToHost);
int64_t* d_cycles_per_warp_main_10_join_build_28;
auto main_10_join_build_28_cpw_size = std::ceil((float)supplier_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_10_join_build_28, sizeof(int64_t) * main_10_join_build_28_cpw_size);
cudaMemset(d_cycles_per_warp_main_10_join_build_28, -1, sizeof(int64_t) * main_10_join_build_28_cpw_size);
...
main_10<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_28, d_BUF_IDX_28, d_HT_28.ref(cuco::insert), d_cycles_per_warp_main_10_join_build_28, d_cycles_per_warp_main_10_selection_11, d_cycles_per_warp_main_10_selection_12, d_cycles_per_warp_main_10_selection_13, d_cycles_per_warp_main_10_selection_14, d_cycles_per_warp_main_10_selection_9, d_supplier__s_region, d_supplier__s_suppkey, supplier_size);
...
std::cout << std::endl;
int64_t* cycles_per_warp_main_10_join_build_28 = (int64_t*)malloc(sizeof(int64_t) * main_10_join_build_28_cpw_size);
cudaMemcpy(cycles_per_warp_main_10_join_build_28, d_cycles_per_warp_main_10_join_build_28, sizeof(int64_t) * main_10_join_build_28_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_10_join_build_28 ";
for (auto i=0ull; i < main_10_join_build_28_cpw_size; i++) std::cout << cycles_per_warp_main_10_join_build_28[i] << " ";
std::cout << std::endl;
int64_t* d_cycles_per_warp_main_4_selection_3;
auto main_4_selection_3_cpw_size = std::ceil((float)customer_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_4_selection_3, sizeof(int64_t) * main_4_selection_3_cpw_size);
cudaMemset(d_cycles_per_warp_main_4_selection_3, -1, sizeof(int64_t) * main_4_selection_3_cpw_size);
int64_t* d_cycles_per_warp_main_4_selection_5;
auto main_4_selection_5_cpw_size = std::ceil((float)customer_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_4_selection_5, sizeof(int64_t) * main_4_selection_5_cpw_size);
cudaMemset(d_cycles_per_warp_main_4_selection_5, -1, sizeof(int64_t) * main_4_selection_5_cpw_size);
int64_t* d_cycles_per_warp_main_4_selection_6;
auto main_4_selection_6_cpw_size = std::ceil((float)customer_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_4_selection_6, sizeof(int64_t) * main_4_selection_6_cpw_size);
cudaMemset(d_cycles_per_warp_main_4_selection_6, -1, sizeof(int64_t) * main_4_selection_6_cpw_size);
int64_t* d_cycles_per_warp_main_4_selection_7;
auto main_4_selection_7_cpw_size = std::ceil((float)customer_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_4_selection_7, sizeof(int64_t) * main_4_selection_7_cpw_size);
cudaMemset(d_cycles_per_warp_main_4_selection_7, -1, sizeof(int64_t) * main_4_selection_7_cpw_size);
int64_t* d_cycles_per_warp_main_4_selection_8;
auto main_4_selection_8_cpw_size = std::ceil((float)customer_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_4_selection_8, sizeof(int64_t) * main_4_selection_8_cpw_size);
cudaMemset(d_cycles_per_warp_main_4_selection_8, -1, sizeof(int64_t) * main_4_selection_8_cpw_size);
//Materialize count
uint64_t* d_COUNT29;
cudaMalloc(&d_COUNT29, sizeof(uint64_t));
cudaMemset(d_COUNT29, 0, sizeof(uint64_t));
count_4<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT29, d_customer__c_region, customer_size);
uint64_t COUNT29;
cudaMemcpy(&COUNT29, d_COUNT29, sizeof(uint64_t), cudaMemcpyDeviceToHost);
int64_t* d_cycles_per_warp_main_4_join_build_29;
auto main_4_join_build_29_cpw_size = std::ceil((float)customer_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_4_join_build_29, sizeof(int64_t) * main_4_join_build_29_cpw_size);
cudaMemset(d_cycles_per_warp_main_4_join_build_29, -1, sizeof(int64_t) * main_4_join_build_29_cpw_size);
// Insert hash table control;
...
main_4<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_29, d_BUF_IDX_29, d_HT_29.ref(cuco::insert), d_customer__c_custkey, d_customer__c_region, customer_size, d_cycles_per_warp_main_4_join_build_29, d_cycles_per_warp_main_4_selection_3, d_cycles_per_warp_main_4_selection_5, d_cycles_per_warp_main_4_selection_6, d_cycles_per_warp_main_4_selection_7, d_cycles_per_warp_main_4_selection_8);
int64_t* cycles_per_warp_main_4_selection_3 = (int64_t*)malloc(sizeof(int64_t) * main_4_selection_3_cpw_size);
cudaMemcpy(cycles_per_warp_main_4_selection_3, d_cycles_per_warp_main_4_selection_3, sizeof(int64_t) * main_4_selection_3_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_4_selection_3 ";
for (auto i=0ull; i < main_4_selection_3_cpw_size; i++) std::cout << cycles_per_warp_main_4_selection_3[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_4_selection_5 = (int64_t*)malloc(sizeof(int64_t) * main_4_selection_5_cpw_size);
cudaMemcpy(cycles_per_warp_main_4_selection_5, d_cycles_per_warp_main_4_selection_5, sizeof(int64_t) * main_4_selection_5_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_4_selection_5 ";
for (auto i=0ull; i < main_4_selection_5_cpw_size; i++) std::cout << cycles_per_warp_main_4_selection_5[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_4_selection_6 = (int64_t*)malloc(sizeof(int64_t) * main_4_selection_6_cpw_size);
cudaMemcpy(cycles_per_warp_main_4_selection_6, d_cycles_per_warp_main_4_selection_6, sizeof(int64_t) * main_4_selection_6_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_4_selection_6 ";
for (auto i=0ull; i < main_4_selection_6_cpw_size; i++) std::cout << cycles_per_warp_main_4_selection_6[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_4_selection_7 = (int64_t*)malloc(sizeof(int64_t) * main_4_selection_7_cpw_size);
cudaMemcpy(cycles_per_warp_main_4_selection_7, d_cycles_per_warp_main_4_selection_7, sizeof(int64_t) * main_4_selection_7_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_4_selection_7 ";
for (auto i=0ull; i < main_4_selection_7_cpw_size; i++) std::cout << cycles_per_warp_main_4_selection_7[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_4_selection_8 = (int64_t*)malloc(sizeof(int64_t) * main_4_selection_8_cpw_size);
cudaMemcpy(cycles_per_warp_main_4_selection_8, d_cycles_per_warp_main_4_selection_8, sizeof(int64_t) * main_4_selection_8_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_4_selection_8 ";
for (auto i=0ull; i < main_4_selection_8_cpw_size; i++) std::cout << cycles_per_warp_main_4_selection_8[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_4_join_build_29 = (int64_t*)malloc(sizeof(int64_t) * main_4_join_build_29_cpw_size);
cudaMemcpy(cycles_per_warp_main_4_join_build_29, d_cycles_per_warp_main_4_join_build_29, sizeof(int64_t) * main_4_join_build_29_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_4_join_build_29 ";
for (auto i=0ull; i < main_4_join_build_29_cpw_size; i++) std::cout << cycles_per_warp_main_4_join_build_29[i] << " ";
std::cout << std::endl;
int64_t* d_cycles_per_warp_main_16_selection_15;
auto main_16_selection_15_cpw_size = std::ceil((float)part_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_16_selection_15, sizeof(int64_t) * main_16_selection_15_cpw_size);
cudaMemset(d_cycles_per_warp_main_16_selection_15, -1, sizeof(int64_t) * main_16_selection_15_cpw_size);
int64_t* d_cycles_per_warp_main_16_selection_17;
auto main_16_selection_17_cpw_size = std::ceil((float)part_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_16_selection_17, sizeof(int64_t) * main_16_selection_17_cpw_size);
cudaMemset(d_cycles_per_warp_main_16_selection_17, -1, sizeof(int64_t) * main_16_selection_17_cpw_size);
int64_t* d_cycles_per_warp_main_16_selection_18;
auto main_16_selection_18_cpw_size = std::ceil((float)part_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_16_selection_18, sizeof(int64_t) * main_16_selection_18_cpw_size);
cudaMemset(d_cycles_per_warp_main_16_selection_18, -1, sizeof(int64_t) * main_16_selection_18_cpw_size);
//Materialize count
uint64_t* d_COUNT30;
cudaMalloc(&d_COUNT30, sizeof(uint64_t));
cudaMemset(d_COUNT30, 0, sizeof(uint64_t));
count_16<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT30, d_part__p_mfgr, part_size);
uint64_t COUNT30;
cudaMemcpy(&COUNT30, d_COUNT30, sizeof(uint64_t), cudaMemcpyDeviceToHost);
int64_t* d_cycles_per_warp_main_16_join_build_30;
auto main_16_join_build_30_cpw_size = std::ceil((float)part_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_16_join_build_30, sizeof(int64_t) * main_16_join_build_30_cpw_size);
cudaMemset(d_cycles_per_warp_main_16_join_build_30, -1, sizeof(int64_t) * main_16_join_build_30_cpw_size);
// Insert hash table control;
...
main_16<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_30, d_BUF_IDX_30, d_HT_30.ref(cuco::insert), d_cycles_per_warp_main_16_join_build_30, d_cycles_per_warp_main_16_selection_15, d_cycles_per_warp_main_16_selection_17, d_cycles_per_warp_main_16_selection_18, d_part__p_mfgr, d_part__p_partkey, part_size);
int64_t* cycles_per_warp_main_16_selection_15 = (int64_t*)malloc(sizeof(int64_t) * main_16_selection_15_cpw_size);
cudaMemcpy(cycles_per_warp_main_16_selection_15, d_cycles_per_warp_main_16_selection_15, sizeof(int64_t) * main_16_selection_15_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_16_selection_15 ";
for (auto i=0ull; i < main_16_selection_15_cpw_size; i++) std::cout << cycles_per_warp_main_16_selection_15[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_16_selection_17 = (int64_t*)malloc(sizeof(int64_t) * main_16_selection_17_cpw_size);
cudaMemcpy(cycles_per_warp_main_16_selection_17, d_cycles_per_warp_main_16_selection_17, sizeof(int64_t) * main_16_selection_17_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_16_selection_17 ";
for (auto i=0ull; i < main_16_selection_17_cpw_size; i++) std::cout << cycles_per_warp_main_16_selection_17[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_16_selection_18 = (int64_t*)malloc(sizeof(int64_t) * main_16_selection_18_cpw_size);
cudaMemcpy(cycles_per_warp_main_16_selection_18, d_cycles_per_warp_main_16_selection_18, sizeof(int64_t) * main_16_selection_18_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_16_selection_18 ";
for (auto i=0ull; i < main_16_selection_18_cpw_size; i++) std::cout << cycles_per_warp_main_16_selection_18[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_16_join_build_30 = (int64_t*)malloc(sizeof(int64_t) * main_16_join_build_30_cpw_size);
cudaMemcpy(cycles_per_warp_main_16_join_build_30, d_cycles_per_warp_main_16_join_build_30, sizeof(int64_t) * main_16_join_build_30_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_16_join_build_30 ";
for (auto i=0ull; i < main_16_join_build_30_cpw_size; i++) std::cout << cycles_per_warp_main_16_join_build_30[i] << " ";
std::cout << std::endl;
...
count_1<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT31, date_size);
uint64_t COUNT31;
...
main_1<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_31, d_BUF_IDX_31, d_HT_31.ref(cuco::insert), d_cycles_per_warp_main_1_join_build_31, d_cycles_per_warp_main_1_selection_0, d_cycles_per_warp_main_1_selection_2, d_date__d_datekey, date_size);
...
//Create aggregation hash table
auto d_HT_33 = cuco::static_map{ (int)87950*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_20<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_28, d_BUF_29, d_BUF_30, d_BUF_31, d_HT_28.ref(cuco::find), d_HT_29.ref(cuco::find), d_HT_30.ref(cuco::find), d_HT_31.ref(cuco::find), d_HT_33.ref(cuco::insert), d_customer__c_nation_encoded, d_date__d_year, d_lineorder__lo_custkey, d_lineorder__lo_orderdate, d_lineorder__lo_partkey, d_lineorder__lo_suppkey, lineorder_size);
size_t COUNT33 = d_HT_33.size();
thrust::device_vector<int64_t> keys_33(COUNT33), vals_33(COUNT33);
d_HT_33.retrieve_all(keys_33.begin(), vals_33.begin());
d_HT_33.clear();
int64_t* raw_keys33 = thrust::raw_pointer_cast(keys_33.data());
insertKeys<<<std::ceil((float)COUNT33/128.), 128>>>(raw_keys33, d_HT_33.ref(cuco::insert), COUNT33);
...
//Materialize count
uint64_t* d_COUNT34;
cudaMalloc(&d_COUNT34, sizeof(uint64_t));
cudaMemset(d_COUNT34, 0, sizeof(uint64_t));
count_35<<<std::ceil((float)COUNT33/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT33, d_COUNT34);
uint64_t COUNT34;
cudaMemcpy(&COUNT34, d_COUNT34, sizeof(uint64_t), cudaMemcpyDeviceToHost);
...
auto MAT34customer__c_nation_encoded = (DBI16Type*)malloc(sizeof(DBI16Type) * COUNT34);
DBI16Type* d_MAT34customer__c_nation_encoded;
cudaMalloc(&d_MAT34customer__c_nation_encoded, sizeof(DBI16Type) * COUNT34);
...
main_35<<<std::ceil((float)COUNT33/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT33, d_MAT34aggr0__tmp_attr0, d_MAT34customer__c_nation_encoded, d_MAT34date__d_year, d_MAT_IDX34, d_aggr0__tmp_attr0, d_KEY_33customer__c_nation_encoded, d_cycles_per_warp_main_35_materialize_34, d_KEY_33date__d_year);
cudaMemcpy(MAT34date__d_year, d_MAT34date__d_year, sizeof(DBI32Type) * COUNT34, cudaMemcpyDeviceToHost);
cudaMemcpy(MAT34customer__c_nation_encoded, d_MAT34customer__c_nation_encoded, sizeof(DBI16Type) * COUNT34, cudaMemcpyDeviceToHost);
