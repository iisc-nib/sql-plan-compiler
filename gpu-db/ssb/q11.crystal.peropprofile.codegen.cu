...
__global__ void count_10(uint64_t* COUNT15, DBI32Type* date__d_year, size_t date_size) {
...
selection_flags[ITEM] &= evaluatePredicate(reg_date__d_year[ITEM], 1993, Predicate::eq);
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
__global__ void main_10(uint64_t* BUF_15, uint64_t* BUF_IDX_15, HASHTABLE_INSERT HT_15, int64_t* cycles_per_warp_main_10_join_build_15, int64_t* cycles_per_warp_main_10_selection_11, int64_t* cycles_per_warp_main_10_selection_12, int64_t* cycles_per_warp_main_10_selection_13, int64_t* cycles_per_warp_main_10_selection_14, int64_t* cycles_per_warp_main_10_selection_9, DBI32Type* date__d_datekey, DBI32Type* date__d_year, size_t date_size) {
...
__global__ void count_1(uint64_t* BUF_15, HASHTABLE_PROBE HT_15, HASHTABLE_INSERT HT_17, DBI32Type* lineorder__lo_discount, DBI32Type* lineorder__lo_orderdate, DBI32Type* lineorder__lo_quantity, size_t lineorder_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
selection_flags[ITEM] &= evaluatePredicate(reg_lineorder__lo_discount[ITEM], 1, Predicate::gte) && evaluatePredicate(reg_lineorder__lo_discount[ITEM], 3, Predicate::lte);
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
selection_flags[ITEM] &= evaluatePredicate(reg_lineorder__lo_quantity[ITEM], 25, Predicate::lt);
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_17 = HT_17.find(KEY_17[ITEM])->second;
...
__global__ void count_19(size_t COUNT17, uint64_t* COUNT18) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
count_10<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT15, d_date__d_year, date_size);
uint64_t COUNT15;
...
main_10<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_15, d_BUF_IDX_15, d_HT_15.ref(cuco::insert), d_cycles_per_warp_main_10_join_build_15, d_cycles_per_warp_main_10_selection_11, d_cycles_per_warp_main_10_selection_12, d_cycles_per_warp_main_10_selection_13, d_cycles_per_warp_main_10_selection_14, d_cycles_per_warp_main_10_selection_9, d_date__d_datekey, d_date__d_year, date_size);
...
count_1<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_15, d_HT_15.ref(cuco::find), d_HT_17.ref(cuco::insert), d_lineorder__lo_discount, d_lineorder__lo_orderdate, d_lineorder__lo_quantity, lineorder_size);
size_t COUNT17 = d_HT_17.size();
thrust::device_vector<int64_t> keys_17(COUNT17), vals_17(COUNT17);
d_HT_17.retrieve_all(keys_17.begin(), vals_17.begin());
d_HT_17.clear();
int64_t* raw_keys17 = thrust::raw_pointer_cast(keys_17.data());
insertKeys<<<std::ceil((float)COUNT17/128.), 128>>>(raw_keys17, d_HT_17.ref(cuco::insert), COUNT17);
...
count_19<<<std::ceil((float)COUNT17/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT17, d_COUNT18);
uint64_t COUNT18;
...
main_19<<<std::ceil((float)COUNT17/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT17, d_MAT18aggr0__tmp_attr0, d_MAT_IDX18, d_aggr0__tmp_attr0, d_cycles_per_warp_main_19_materialize_18);
