...
__global__ void count_10(uint64_t* COUNT18, DBI32Type* date__d_weeknuminyear, DBI32Type* date__d_year, size_t date_size) {
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
atomicAdd((int*)COUNT18, 1);
}
}
template<typename HASHTABLE_INSERT>
__global__ void main_10(uint64_t* BUF_18, uint64_t* BUF_IDX_18, HASHTABLE_INSERT HT_18, int64_t* cycles_per_warp_main_10_join_build_18, int64_t* cycles_per_warp_main_10_selection_11, int64_t* cycles_per_warp_main_10_selection_12, int64_t* cycles_per_warp_main_10_selection_13, int64_t* cycles_per_warp_main_10_selection_14, int64_t* cycles_per_warp_main_10_selection_15, int64_t* cycles_per_warp_main_10_selection_16, int64_t* cycles_per_warp_main_10_selection_17, int64_t* cycles_per_warp_main_10_selection_9, DBI32Type* date__d_datekey, DBI32Type* date__d_weeknuminyear, DBI32Type* date__d_year, size_t date_size) {
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_10_selection_15[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_10_selection_16[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_10_selection_17[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
uint64_t KEY_18[ITEMS_PER_THREAD];
...
if (!selection_flags[ITEM]) continue;
KEY_18[ITEM] = 0;
KEY_18[ITEM] |= reg_date__d_datekey[ITEM];
}
// Insert hash table kernel;
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_18 = atomicAdd((int*)BUF_IDX_18, 1);
HT_18.insert(cuco::pair{KEY_18[ITEM], buf_idx_18});
BUF_18[(buf_idx_18) * 1 + 0] = ITEM*TB + tid;
}
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_10_join_build_18[blockIdx.x] = cycles_per_warp;}
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
__global__ void count_1(uint64_t* BUF_18, HASHTABLE_PROBE HT_18, HASHTABLE_INSERT HT_20, DBI32Type* lineorder__lo_discount, DBI32Type* lineorder__lo_orderdate, DBI32Type* lineorder__lo_quantity, size_t lineorder_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
if (!selection_flags[ITEM]) continue;
KEY_18[ITEM] = 0;
KEY_18[ITEM] |= reg_lineorder__lo_orderdate[ITEM];
}
int64_t slot_second18[ITEMS_PER_THREAD];
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto SLOT_18 = HT_18.find(KEY_18[ITEM]);
if (SLOT_18 == HT_18.end()) {selection_flags[ITEM] = 0; continue;}
slot_second18[ITEM] = SLOT_18->second;
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_1_join_probe_18[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
...
aggregate_sum(&aggr0__tmp_attr0[buf_idx_20], reg_map0__tmp_attr1[ITEM]);
}
...
__global__ void main_22(size_t COUNT20, DBDecimalType* MAT21aggr0__tmp_attr0, uint64_t* MAT_IDX21, DBDecimalType* aggr0__tmp_attr0, int64_t* cycles_per_warp_main_22_materialize_21) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
MAT21aggr0__tmp_attr0[mat_idx21] = reg_aggr0__tmp_attr0[ITEM];
}
...
int64_t* d_cycles_per_warp_main_10_selection_15;
auto main_10_selection_15_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_10_selection_15, sizeof(int64_t) * main_10_selection_15_cpw_size);
cudaMemset(d_cycles_per_warp_main_10_selection_15, -1, sizeof(int64_t) * main_10_selection_15_cpw_size);
int64_t* d_cycles_per_warp_main_10_selection_16;
auto main_10_selection_16_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_10_selection_16, sizeof(int64_t) * main_10_selection_16_cpw_size);
cudaMemset(d_cycles_per_warp_main_10_selection_16, -1, sizeof(int64_t) * main_10_selection_16_cpw_size);
int64_t* d_cycles_per_warp_main_10_selection_17;
auto main_10_selection_17_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_10_selection_17, sizeof(int64_t) * main_10_selection_17_cpw_size);
cudaMemset(d_cycles_per_warp_main_10_selection_17, -1, sizeof(int64_t) * main_10_selection_17_cpw_size);
//Materialize count
uint64_t* d_COUNT18;
cudaMalloc(&d_COUNT18, sizeof(uint64_t));
cudaMemset(d_COUNT18, 0, sizeof(uint64_t));
count_10<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT18, d_date__d_weeknuminyear, d_date__d_year, date_size);
uint64_t COUNT18;
cudaMemcpy(&COUNT18, d_COUNT18, sizeof(uint64_t), cudaMemcpyDeviceToHost);
int64_t* d_cycles_per_warp_main_10_join_build_18;
auto main_10_join_build_18_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_10_join_build_18, sizeof(int64_t) * main_10_join_build_18_cpw_size);
cudaMemset(d_cycles_per_warp_main_10_join_build_18, -1, sizeof(int64_t) * main_10_join_build_18_cpw_size);
// Insert hash table control;
uint64_t* d_BUF_IDX_18;
cudaMalloc(&d_BUF_IDX_18, sizeof(uint64_t));
cudaMemset(d_BUF_IDX_18, 0, sizeof(uint64_t));
uint64_t* d_BUF_18;
cudaMalloc(&d_BUF_18, sizeof(uint64_t) * COUNT18 * 1);
auto d_HT_18 = cuco::static_map{ (int)COUNT18*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
main_10<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_18, d_BUF_IDX_18, d_HT_18.ref(cuco::insert), d_cycles_per_warp_main_10_join_build_18, d_cycles_per_warp_main_10_selection_11, d_cycles_per_warp_main_10_selection_12, d_cycles_per_warp_main_10_selection_13, d_cycles_per_warp_main_10_selection_14, d_cycles_per_warp_main_10_selection_15, d_cycles_per_warp_main_10_selection_16, d_cycles_per_warp_main_10_selection_17, d_cycles_per_warp_main_10_selection_9, d_date__d_datekey, d_date__d_weeknuminyear, d_date__d_year, date_size);
...
std::cout << std::endl;
int64_t* cycles_per_warp_main_10_selection_15 = (int64_t*)malloc(sizeof(int64_t) * main_10_selection_15_cpw_size);
cudaMemcpy(cycles_per_warp_main_10_selection_15, d_cycles_per_warp_main_10_selection_15, sizeof(int64_t) * main_10_selection_15_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_10_selection_15 ";
for (auto i=0ull; i < main_10_selection_15_cpw_size; i++) std::cout << cycles_per_warp_main_10_selection_15[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_10_selection_16 = (int64_t*)malloc(sizeof(int64_t) * main_10_selection_16_cpw_size);
cudaMemcpy(cycles_per_warp_main_10_selection_16, d_cycles_per_warp_main_10_selection_16, sizeof(int64_t) * main_10_selection_16_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_10_selection_16 ";
for (auto i=0ull; i < main_10_selection_16_cpw_size; i++) std::cout << cycles_per_warp_main_10_selection_16[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_10_selection_17 = (int64_t*)malloc(sizeof(int64_t) * main_10_selection_17_cpw_size);
cudaMemcpy(cycles_per_warp_main_10_selection_17, d_cycles_per_warp_main_10_selection_17, sizeof(int64_t) * main_10_selection_17_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_10_selection_17 ";
for (auto i=0ull; i < main_10_selection_17_cpw_size; i++) std::cout << cycles_per_warp_main_10_selection_17[i] << " ";
std::cout << std::endl;
int64_t* cycles_per_warp_main_10_join_build_18 = (int64_t*)malloc(sizeof(int64_t) * main_10_join_build_18_cpw_size);
cudaMemcpy(cycles_per_warp_main_10_join_build_18, d_cycles_per_warp_main_10_join_build_18, sizeof(int64_t) * main_10_join_build_18_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_10_join_build_18 ";
for (auto i=0ull; i < main_10_join_build_18_cpw_size; i++) std::cout << cycles_per_warp_main_10_join_build_18[i] << " ";
std::cout << std::endl;
...
int64_t* d_cycles_per_warp_main_1_join_probe_18;
auto main_1_join_probe_18_cpw_size = std::ceil((float)lineorder_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_1_join_probe_18, sizeof(int64_t) * main_1_join_probe_18_cpw_size);
cudaMemset(d_cycles_per_warp_main_1_join_probe_18, -1, sizeof(int64_t) * main_1_join_probe_18_cpw_size);
int64_t* d_cycles_per_warp_main_1_map_19;
auto main_1_map_19_cpw_size = std::ceil((float)lineorder_size/(float)TILE_SIZE);
...
//Create aggregation hash table
auto d_HT_20 = cuco::static_map{ (int)1*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_1<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_18, d_HT_18.ref(cuco::find), d_HT_20.ref(cuco::insert), d_lineorder__lo_discount, d_lineorder__lo_orderdate, d_lineorder__lo_quantity, lineorder_size);
...
auto main_1_aggregation_20_cpw_size = std::ceil((float)lineorder_size/(float)TILE_SIZE);
...
cudaMalloc(&d_aggr0__tmp_attr0, sizeof(DBDecimalType) * COUNT20);
cudaMemset(d_aggr0__tmp_attr0, 0, sizeof(DBDecimalType) * COUNT20);
main_1<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_18, d_HT_18.ref(cuco::find), d_HT_20.ref(cuco::find), d_aggr0__tmp_attr0, d_cycles_per_warp_main_1_aggregation_20, d_cycles_per_warp_main_1_join_probe_18, d_cycles_per_warp_main_1_map_19, d_cycles_per_warp_main_1_selection_0, d_cycles_per_warp_main_1_selection_2, d_cycles_per_warp_main_1_selection_3, d_cycles_per_warp_main_1_selection_4, d_cycles_per_warp_main_1_selection_5, d_cycles_per_warp_main_1_selection_6, d_cycles_per_warp_main_1_selection_7, d_cycles_per_warp_main_1_selection_8, d_lineorder__lo_discount, d_lineorder__lo_extendedprice, d_lineorder__lo_orderdate, d_lineorder__lo_quantity, lineorder_size);
...
std::cout << std::endl;
int64_t* cycles_per_warp_main_1_join_probe_18 = (int64_t*)malloc(sizeof(int64_t) * main_1_join_probe_18_cpw_size);
cudaMemcpy(cycles_per_warp_main_1_join_probe_18, d_cycles_per_warp_main_1_join_probe_18, sizeof(int64_t) * main_1_join_probe_18_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_1_join_probe_18 ";
for (auto i=0ull; i < main_1_join_probe_18_cpw_size; i++) std::cout << cycles_per_warp_main_1_join_probe_18[i] << " ";
std::cout << std::endl;
...
auto MAT21aggr0__tmp_attr0 = (DBDecimalType*)malloc(sizeof(DBDecimalType) * COUNT21);
DBDecimalType* d_MAT21aggr0__tmp_attr0;
cudaMalloc(&d_MAT21aggr0__tmp_attr0, sizeof(DBDecimalType) * COUNT21);
main_22<<<std::ceil((float)COUNT20/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT20, d_MAT21aggr0__tmp_attr0, d_MAT_IDX21, d_aggr0__tmp_attr0, d_cycles_per_warp_main_22_materialize_21);
cudaMemcpy(MAT21aggr0__tmp_attr0, d_MAT21aggr0__tmp_attr0, sizeof(DBDecimalType) * COUNT21, cudaMemcpyDeviceToHost);
...
cudaFree(d_BUF_18);
cudaFree(d_BUF_IDX_18);
cudaFree(d_COUNT18);
cudaFree(d_aggr0__tmp_attr0);
cudaFree(d_COUNT21);
cudaFree(d_MAT21aggr0__tmp_attr0);
cudaFree(d_MAT_IDX21);
free(MAT21aggr0__tmp_attr0);
}