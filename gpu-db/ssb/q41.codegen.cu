...
if (!(true)) return;
uint64_t KEY_8 = 0;
auto reg_date__d_year = date__d_year[BUF_6[slot_second6 * 1 + 0]];

KEY_8 |= reg_date__d_year;
auto reg_customer__c_nation_encoded = customer__c_nation_encoded[BUF_2[slot_second2 * 1 + 0]];
KEY_8 <<= 16;
...
//Create aggregation hash table
auto d_HT_8 = cuco::static_map{ (int)87906*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_9<<<std::ceil((float)lineorder_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_4, d_BUF_6, d_HT_0.ref(cuco::for_each), d_HT_2.ref(cuco::for_each), d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::insert), d_customer__c_nation_encoded, d_date__d_year, d_lineorder__lo_custkey, d_lineorder__lo_orderdate, d_lineorder__lo_partkey, d_lineorder__lo_suppkey, lineorder_size);
