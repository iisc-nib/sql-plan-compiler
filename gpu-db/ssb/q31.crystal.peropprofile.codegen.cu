...
__global__ void count_1(uint64_t* COUNT19, DBStringType* customer__c_region, size_t customer_size) {
...
selection_flags[ITEM] &= evaluatePredicate(reg_customer__c_region[ITEM], "ASIA", Predicate::eq);
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < customer_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
if (!selection_flags[ITEM]) continue;
KEY_19[ITEM] = 0;
KEY_19[ITEM] |= reg_customer__c_custkey[ITEM];
}
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_1_join_build_19[blockIdx.x] = cycles_per_warp;}
}
__global__ void count_14(uint64_t* COUNT26, DBStringType* supplier__s_region, size_t supplier_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < supplier_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
atomicAdd((int*)COUNT26, 1);
}
}
template<typename HASHTABLE_INSERT>
__global__ void main_14(uint64_t* BUF_26, uint64_t* BUF_IDX_26, HASHTABLE_INSERT HT_26, int64_t* cycles_per_warp_main_14_join_build_26, int64_t* cycles_per_warp_main_14_selection_13, int64_t* cycles_per_warp_main_14_selection_15, int64_t* cycles_per_warp_main_14_selection_16, int64_t* cycles_per_warp_main_14_selection_17, int64_t* cycles_per_warp_main_14_selection_18, DBStringType* supplier__s_region, DBI32Type* supplier__s_suppkey, size_t supplier_size) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
if (threadIdx.x == 0) start = clock64();
uint64_t KEY_26[ITEMS_PER_THREAD];
...
if (!selection_flags[ITEM]) continue;
KEY_26[ITEM] = 0;
KEY_26[ITEM] |= reg_supplier__s_suppkey[ITEM];
}
// Insert hash table kernel;
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < supplier_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_26 = atomicAdd((int*)BUF_IDX_26, 1);
HT_26.insert(cuco::pair{KEY_26[ITEM], buf_idx_26});
BUF_26[(buf_idx_26) * 1 + 0] = ITEM*TB + tid;
}
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_14_join_build_26[blockIdx.x] = cycles_per_warp;}
}
__global__ void count_21(uint64_t* COUNT27, DBI32Type* date__d_year, size_t date_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
template<typename HASHTABLE_INSERT>
__global__ void main_21(uint64_t* BUF_27, uint64_t* BUF_IDX_27, HASHTABLE_INSERT HT_27, int64_t* cycles_per_warp_main_21_join_build_27, int64_t* cycles_per_warp_main_21_selection_20, int64_t* cycles_per_warp_main_21_selection_22, int64_t* cycles_per_warp_main_21_selection_23, int64_t* cycles_per_warp_main_21_selection_24, int64_t* cycles_per_warp_main_21_selection_25, DBI32Type* date__d_datekey, DBI32Type* date__d_year, size_t date_size) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_21_join_build_27[blockIdx.x] = cycles_per_warp;}
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
__global__ void count_7(uint64_t* BUF_19, uint64_t* BUF_26, uint64_t* BUF_27, HASHTABLE_PROBE HT_19, HASHTABLE_PROBE HT_26, HASHTABLE_PROBE HT_27, HASHTABLE_INSERT HT_28, DBI16Type* customer__c_nation_encoded, DBI32Type* date__d_year, DBI32Type* lineorder__lo_custkey, DBI32Type* lineorder__lo_orderdate, DBI32Type* lineorder__lo_suppkey, size_t lineorder_size, DBI16Type* supplier__s_nation_encoded) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
if (!selection_flags[ITEM]) continue;
KEY_19[ITEM] = 0;
KEY_19[ITEM] |= reg_lineorder__lo_custkey[ITEM];
}
...
if (!selection_flags[ITEM]) continue;
KEY_26[ITEM] = 0;
KEY_26[ITEM] |= reg_lineorder__lo_suppkey[ITEM];
}
int64_t slot_second26[ITEMS_PER_THREAD];
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto SLOT_26 = HT_26.find(KEY_26[ITEM]);
if (SLOT_26 == HT_26.end()) {selection_flags[ITEM] = 0; continue;}
slot_second26[ITEM] = SLOT_26->second;
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_customer__c_nation_encoded[ITEM] = customer__c_nation_encoded[BUF_19[slot_second19[ITEM] * 1 + 0]];
}
DBI16Type reg_supplier__s_nation_encoded[ITEMS_PER_THREAD];
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_supplier__s_nation_encoded[ITEM] = supplier__s_nation_encoded[BUF_26[slot_second26[ITEM] * 1 + 0]];
}
DBI32Type reg_date__d_year[ITEMS_PER_THREAD];
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
...
if (!selection_flags[ITEM]) continue;
KEY_28[ITEM] = 0;
KEY_28[ITEM] |= reg_customer__c_nation_encoded[ITEM];
KEY_28[ITEM] <<= 16;
KEY_28[ITEM] |= reg_supplier__s_nation_encoded[ITEM];
KEY_28[ITEM] <<= 32;
...
__global__ void main_7(uint64_t* BUF_19, uint64_t* BUF_26, uint64_t* BUF_27, HASHTABLE_PROBE HT_19, HASHTABLE_PROBE HT_26, HASHTABLE_PROBE HT_27, HASHTABLE_FIND HT_28, DBI16Type* KEY_28customer__c_nation_encoded, DBI32Type* KEY_28date__d_year, DBI16Type* KEY_28supplier__s_nation_encoded, DBDecimalType* aggr0__tmp_attr0, DBI16Type* customer__c_nation_encoded, int64_t* cycles_per_warp_main_7_aggregation_28, int64_t* cycles_per_warp_main_7_join_probe_19, int64_t* cycles_per_warp_main_7_join_probe_26, int64_t* cycles_per_warp_main_7_join_probe_27, int64_t* cycles_per_warp_main_7_selection_10, int64_t* cycles_per_warp_main_7_selection_11, int64_t* cycles_per_warp_main_7_selection_12, int64_t* cycles_per_warp_main_7_selection_6, int64_t* cycles_per_warp_main_7_selection_8, int64_t* cycles_per_warp_main_7_selection_9, DBI32Type* date__d_year, DBI32Type* lineorder__lo_custkey, DBI32Type* lineorder__lo_orderdate, DBDecimalType* lineorder__lo_revenue, DBI32Type* lineorder__lo_suppkey, size_t lineorder_size, DBI16Type* supplier__s_nation_encoded) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_7_join_probe_26[blockIdx.x] = cycles_per_warp;}
if (threadIdx.x == 0) start = clock64();
uint64_t KEY_27[ITEMS_PER_THREAD];
DBI32Type reg_lineorder__lo_orderdate[ITEMS_PER_THREAD];
#pragma unroll
...
KEY_28customer__c_nation_encoded[buf_idx_28] = reg_customer__c_nation_encoded[ITEM];
KEY_28supplier__s_nation_encoded[buf_idx_28] = reg_supplier__s_nation_encoded[ITEM];
...
__global__ void main_30(size_t COUNT28, DBDecimalType* MAT29aggr0__tmp_attr0, DBI16Type* MAT29customer__c_nation_encoded, DBI32Type* MAT29date__d_year, DBI16Type* MAT29supplier__s_nation_encoded, uint64_t* MAT_IDX29, DBDecimalType* aggr0__tmp_attr0, DBI16Type* customer__c_nation_encoded, int64_t* cycles_per_warp_main_30_materialize_29, DBI32Type* date__d_year, DBI16Type* supplier__s_nation_encoded) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
MAT29customer__c_nation_encoded[mat_idx29] = reg_customer__c_nation_encoded[ITEM];
MAT29supplier__s_nation_encoded[mat_idx29] = reg_supplier__s_nation_encoded[ITEM];
...
count_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT19, d_customer__c_region, customer_size);
uint64_t COUNT19;
cudaMemcpy(&COUNT19, d_COUNT19, sizeof(uint64_t), cudaMemcpyDeviceToHost);
int64_t* d_cycles_per_warp_main_1_join_build_19;
auto main_1_join_build_19_cpw_size = std::ceil((float)customer_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_1_join_build_19, sizeof(int64_t) * main_1_join_build_19_cpw_size);
cudaMemset(d_cycles_per_warp_main_1_join_build_19, -1, sizeof(int64_t) * main_1_join_build_19_cpw_size);
...
std::cout << std::endl;
int64_t* cycles_per_warp_main_1_join_build_19 = (int64_t*)malloc(sizeof(int64_t) * main_1_join_build_19_cpw_size);
cudaMemcpy(cycles_per_warp_main_1_join_build_19, d_cycles_per_warp_main_1_join_build_19, sizeof(int64_t) * main_1_join_build_19_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_1_join_build_19 ";
for (auto i=0ull; i < main_1_join_build_19_cpw_size; i++) std::cout << cycles_per_warp_main_1_join_build_19[i] << " ";
std::cout << std::endl;
...
//Materialize count
uint64_t* d_COUNT26;
cudaMalloc(&d_COUNT26, sizeof(uint64_t));
cudaMemset(d_COUNT26, 0, sizeof(uint64_t));
count_14<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT26, d_supplier__s_region, supplier_size);
uint64_t COUNT26;
cudaMemcpy(&COUNT26, d_COUNT26, sizeof(uint64_t), cudaMemcpyDeviceToHost);
int64_t* d_cycles_per_warp_main_14_join_build_26;
auto main_14_join_build_26_cpw_size = std::ceil((float)supplier_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_14_join_build_26, sizeof(int64_t) * main_14_join_build_26_cpw_size);
cudaMemset(d_cycles_per_warp_main_14_join_build_26, -1, sizeof(int64_t) * main_14_join_build_26_cpw_size);
// Insert hash table control;
uint64_t* d_BUF_IDX_26;
cudaMalloc(&d_BUF_IDX_26, sizeof(uint64_t));
cudaMemset(d_BUF_IDX_26, 0, sizeof(uint64_t));
uint64_t* d_BUF_26;
cudaMalloc(&d_BUF_26, sizeof(uint64_t) * COUNT26 * 1);
auto d_HT_26 = cuco::static_map{ (int)COUNT26*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
main_14<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_26, d_BUF_IDX_26, d_HT_26.ref(cuco::insert), d_cycles_per_warp_main_14_join_build_26, d_cycles_per_warp_main_14_selection_13, d_cycles_per_warp_main_14_selection_15, d_cycles_per_warp_main_14_selection_16, d_cycles_per_warp_main_14_selection_17, d_cycles_per_warp_main_14_selection_18, d_supplier__s_region, d_supplier__s_suppkey, supplier_size);
...
std::cout << std::endl;
int64_t* cycles_per_warp_main_14_join_build_26 = (int64_t*)malloc(sizeof(int64_t) * main_14_join_build_26_cpw_size);
cudaMemcpy(cycles_per_warp_main_14_join_build_26, d_cycles_per_warp_main_14_join_build_26, sizeof(int64_t) * main_14_join_build_26_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_14_join_build_26 ";
for (auto i=0ull; i < main_14_join_build_26_cpw_size; i++) std::cout << cycles_per_warp_main_14_join_build_26[i] << " ";
std::cout << std::endl;
int64_t* d_cycles_per_warp_main_21_selection_20;
auto main_21_selection_20_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
...
auto main_21_selection_22_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
...
auto main_21_selection_23_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
...
auto main_21_selection_24_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
...
auto main_21_selection_25_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
...
count_21<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT27, d_date__d_year, date_size);
uint64_t COUNT27;
cudaMemcpy(&COUNT27, d_COUNT27, sizeof(uint64_t), cudaMemcpyDeviceToHost);
int64_t* d_cycles_per_warp_main_21_join_build_27;
auto main_21_join_build_27_cpw_size = std::ceil((float)date_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_21_join_build_27, sizeof(int64_t) * main_21_join_build_27_cpw_size);
cudaMemset(d_cycles_per_warp_main_21_join_build_27, -1, sizeof(int64_t) * main_21_join_build_27_cpw_size);
// Insert hash table control;
...
main_21<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_27, d_BUF_IDX_27, d_HT_27.ref(cuco::insert), d_cycles_per_warp_main_21_join_build_27, d_cycles_per_warp_main_21_selection_20, d_cycles_per_warp_main_21_selection_22, d_cycles_per_warp_main_21_selection_23, d_cycles_per_warp_main_21_selection_24, d_cycles_per_warp_main_21_selection_25, d_date__d_datekey, d_date__d_year, date_size);
...
std::cout << std::endl;
int64_t* cycles_per_warp_main_21_join_build_27 = (int64_t*)malloc(sizeof(int64_t) * main_21_join_build_27_cpw_size);
cudaMemcpy(cycles_per_warp_main_21_join_build_27, d_cycles_per_warp_main_21_join_build_27, sizeof(int64_t) * main_21_join_build_27_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_21_join_build_27 ";
for (auto i=0ull; i < main_21_join_build_27_cpw_size; i++) std::cout << cycles_per_warp_main_21_join_build_27[i] << " ";
std::cout << std::endl;
...
int64_t* d_cycles_per_warp_main_7_join_probe_26;
auto main_7_join_probe_26_cpw_size = std::ceil((float)lineorder_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_7_join_probe_26, sizeof(int64_t) * main_7_join_probe_26_cpw_size);
cudaMemset(d_cycles_per_warp_main_7_join_probe_26, -1, sizeof(int64_t) * main_7_join_probe_26_cpw_size);
...
//Create aggregation hash table
auto d_HT_28 = cuco::static_map{ (int)144285*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_7<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_19, d_BUF_26, d_BUF_27, d_HT_19.ref(cuco::find), d_HT_26.ref(cuco::find), d_HT_27.ref(cuco::find), d_HT_28.ref(cuco::insert), d_customer__c_nation_encoded, d_date__d_year, d_lineorder__lo_custkey, d_lineorder__lo_orderdate, d_lineorder__lo_suppkey, lineorder_size, d_supplier__s_nation_encoded);
...
DBI16Type* d_KEY_28customer__c_nation_encoded;
cudaMalloc(&d_KEY_28customer__c_nation_encoded, sizeof(DBI16Type) * COUNT28);
cudaMemset(d_KEY_28customer__c_nation_encoded, 0, sizeof(DBI16Type) * COUNT28);
DBI16Type* d_KEY_28supplier__s_nation_encoded;
cudaMalloc(&d_KEY_28supplier__s_nation_encoded, sizeof(DBI16Type) * COUNT28);
cudaMemset(d_KEY_28supplier__s_nation_encoded, 0, sizeof(DBI16Type) * COUNT28);
...
main_7<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_19, d_BUF_26, d_BUF_27, d_HT_19.ref(cuco::find), d_HT_26.ref(cuco::find), d_HT_27.ref(cuco::find), d_HT_28.ref(cuco::find), d_KEY_28customer__c_nation_encoded, d_KEY_28date__d_year, d_KEY_28supplier__s_nation_encoded, d_aggr0__tmp_attr0, d_customer__c_nation_encoded, d_cycles_per_warp_main_7_aggregation_28, d_cycles_per_warp_main_7_join_probe_19, d_cycles_per_warp_main_7_join_probe_26, d_cycles_per_warp_main_7_join_probe_27, d_cycles_per_warp_main_7_selection_10, d_cycles_per_warp_main_7_selection_11, d_cycles_per_warp_main_7_selection_12, d_cycles_per_warp_main_7_selection_6, d_cycles_per_warp_main_7_selection_8, d_cycles_per_warp_main_7_selection_9, d_date__d_year, d_lineorder__lo_custkey, d_lineorder__lo_orderdate, d_lineorder__lo_revenue, d_lineorder__lo_suppkey, lineorder_size, d_supplier__s_nation_encoded);
...
std::cout << std::endl;
int64_t* cycles_per_warp_main_7_join_probe_26 = (int64_t*)malloc(sizeof(int64_t) * main_7_join_probe_26_cpw_size);
cudaMemcpy(cycles_per_warp_main_7_join_probe_26, d_cycles_per_warp_main_7_join_probe_26, sizeof(int64_t) * main_7_join_probe_26_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_7_join_probe_26 ";
for (auto i=0ull; i < main_7_join_probe_26_cpw_size; i++) std::cout << cycles_per_warp_main_7_join_probe_26[i] << " ";
std::cout << std::endl;
...
auto MAT29customer__c_nation_encoded = (DBI16Type*)malloc(sizeof(DBI16Type) * COUNT29);
DBI16Type* d_MAT29customer__c_nation_encoded;
cudaMalloc(&d_MAT29customer__c_nation_encoded, sizeof(DBI16Type) * COUNT29);
auto MAT29supplier__s_nation_encoded = (DBI16Type*)malloc(sizeof(DBI16Type) * COUNT29);
DBI16Type* d_MAT29supplier__s_nation_encoded;
cudaMalloc(&d_MAT29supplier__s_nation_encoded, sizeof(DBI16Type) * COUNT29);
...
main_30<<<std::ceil((float)COUNT28/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT28, d_MAT29aggr0__tmp_attr0, d_MAT29customer__c_nation_encoded, d_MAT29date__d_year, d_MAT29supplier__s_nation_encoded, d_MAT_IDX29, d_aggr0__tmp_attr0, d_KEY_28customer__c_nation_encoded, d_cycles_per_warp_main_30_materialize_29, d_KEY_28date__d_year, d_KEY_28supplier__s_nation_encoded);
cudaMemcpy(MAT29customer__c_nation_encoded, d_MAT29customer__c_nation_encoded, sizeof(DBI16Type) * COUNT29, cudaMemcpyDeviceToHost);
cudaMemcpy(MAT29supplier__s_nation_encoded, d_MAT29supplier__s_nation_encoded, sizeof(DBI16Type) * COUNT29, cudaMemcpyDeviceToHost);
...
cudaFree(d_COUNT19);
cudaFree(d_BUF_26);
cudaFree(d_BUF_IDX_26);
cudaFree(d_COUNT26);
cudaFree(d_BUF_27);
cudaFree(d_BUF_IDX_27);
cudaFree(d_COUNT27);
cudaFree(d_KEY_28customer__c_nation_encoded);
cudaFree(d_KEY_28date__d_year);
cudaFree(d_KEY_28supplier__s_nation_encoded);
cudaFree(d_aggr0__tmp_attr0);
cudaFree(d_COUNT29);
cudaFree(d_MAT29aggr0__tmp_attr0);
cudaFree(d_MAT29customer__c_nation_encoded);
cudaFree(d_MAT29date__d_year);
cudaFree(d_MAT29supplier__s_nation_encoded);
cudaFree(d_MAT_IDX29);
free(MAT29aggr0__tmp_attr0);
free(MAT29customer__c_nation_encoded);
free(MAT29date__d_year);
free(MAT29supplier__s_nation_encoded);
}