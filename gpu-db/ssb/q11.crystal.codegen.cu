...
if (!selection_flags[ITEM]) continue;
reg_lineorder__lo_discount[ITEM] = lineorder__lo_discount[tid + ITEM];
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (tid + ITEM < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (tid + ITEM < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_lineorder__lo_quantity[ITEM] = lineorder__lo_quantity[tid + ITEM];
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (tid + ITEM < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (tid + ITEM < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_lineorder__lo_extendedprice[ITEM] = lineorder__lo_extendedprice[tid + ITEM];
}
...
//Materialize count
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (tid + ITEM < COUNT2); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
count_1<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT0, d_date__d_year, date_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_date__d_datekey, d_date__d_year, date_size);
//Create aggregation hash table
...
count_3<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_lineorder__lo_discount, d_lineorder__lo_orderdate, d_lineorder__lo_quantity, lineorder_size);
...
main_3<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_aggr0__tmp_attr0, d_lineorder__lo_discount, d_lineorder__lo_extendedprice, d_lineorder__lo_orderdate, d_lineorder__lo_quantity, lineorder_size);
//Materialize count
uint64_t* d_COUNT4;
