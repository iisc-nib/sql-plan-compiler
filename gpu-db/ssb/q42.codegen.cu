...
__global__ void count_7(uint64_t* BUF_0, uint64_t* BUF_2, uint64_t* BUF_4, uint64_t* COUNT6, HASHTABLE_PROBE_PK HT_0, HASHTABLE_PROBE_PK HT_2, HASHTABLE_PROBE_PK HT_4, DBI32Type* lineorder__lo_custkey, DBI32Type* lineorder__lo_orderdate, DBI32Type* lineorder__lo_suppkey, size_t lineorder_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
auto reg_supplier__s_nation_encoded = supplier__s_nation_encoded[BUF_6[slot_second6 * 4 + 2]];
KEY_8 <<= 16;
...
count_7<<<std::ceil((float)lineorder_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_4, d_COUNT6, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::find), d_lineorder__lo_custkey, d_lineorder__lo_orderdate, d_lineorder__lo_suppkey, lineorder_size);
uint64_t COUNT6;
...
count_9<<<std::ceil((float)part_size/128.), 128>>>(d_BUF_6, d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::insert), d_date__d_year, d_part__p_category_encoded, d_part__p_mfgr, d_part__p_partkey, part_size, d_supplier__s_nation_encoded);
