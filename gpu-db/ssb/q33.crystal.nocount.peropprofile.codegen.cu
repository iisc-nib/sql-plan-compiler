...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_24 = get_aggregation_slot(KEY_24[ITEM], HT_24, SLOT_COUNT_24);
...
main_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_15, d_BUF_IDX_15, d_HT_15.ref(cuco::insert), d_customer__c_city, d_customer__c_custkey, customer_size, d_cycles_per_warp_main_1_join_build_15, d_cycles_per_warp_main_1_selection_0, d_cycles_per_warp_main_1_selection_2, d_cycles_per_warp_main_1_selection_3);
...
main_12<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TB>>>(d_BUF_22, d_BUF_IDX_22, d_HT_22.ref(cuco::insert), d_cycles_per_warp_main_12_join_build_22, d_cycles_per_warp_main_12_selection_11, d_cycles_per_warp_main_12_selection_13, d_cycles_per_warp_main_12_selection_14, d_supplier__s_city, d_supplier__s_suppkey, supplier_size);
...
main_17<<<std::ceil((float)date_size/(float)TILE_SIZE), TB>>>(d_BUF_23, d_BUF_IDX_23, d_HT_23.ref(cuco::insert), d_cycles_per_warp_main_17_join_build_23, d_cycles_per_warp_main_17_selection_16, d_cycles_per_warp_main_17_selection_18, d_cycles_per_warp_main_17_selection_19, d_cycles_per_warp_main_17_selection_20, d_cycles_per_warp_main_17_selection_21, d_date__d_datekey, d_date__d_year, date_size);
...
size_t COUNT24 = 132;
auto d_HT_24 = cuco::static_map{ (int)132*2, cuco::empty_key{(int64_t)-1},         cuco::empty_value{(int64_t)-1},         thrust::equal_to<int64_t>{},         cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
int* d_SLOT_COUNT_24;
cudaMalloc(&d_SLOT_COUNT_24, sizeof(int));
cudaMemset(d_SLOT_COUNT_24, 0, sizeof(int));
//Aggregate in hashtable
...
main_26<<<std::ceil((float)COUNT24/(float)TILE_SIZE), TB>>>(COUNT24, d_MAT25aggr0__tmp_attr0, d_MAT25customer__c_city_encoded, d_MAT25date__d_year, d_MAT25supplier__s_city_encoded, d_MAT_IDX25, d_aggr0__tmp_attr0, d_KEY_24customer__c_city_encoded, d_cycles_per_warp_main_26_materialize_25, d_KEY_24date__d_year, d_KEY_24supplier__s_city_encoded);
uint64_t MATCOUNT_25 = 0;
cudaMemcpy(&MATCOUNT_25, d_MAT_IDX25, sizeof(uint64_t), cudaMemcpyDeviceToHost);
