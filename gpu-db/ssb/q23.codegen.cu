...
__global__ void count_5(uint64_t* BUF_0, uint64_t* BUF_2, uint64_t* COUNT4, HASHTABLE_PROBE_PK HT_0, HASHTABLE_PROBE_PK HT_2, DBI32Type* lineorder__lo_partkey, DBI32Type* lineorder__lo_suppkey, size_t lineorder_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void main_5(uint64_t* BUF_0, uint64_t* BUF_2, uint64_t* BUF_4, uint64_t* BUF_IDX_4, HASHTABLE_PROBE_PK HT_0, HASHTABLE_PROBE_PK HT_2, HASHTABLE_INSERT HT_4, DBI32Type* lineorder__lo_orderdate, DBI32Type* lineorder__lo_partkey, DBI32Type* lineorder__lo_suppkey, size_t lineorder_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineorder_size) return;
if (!(!(false))) return;
if (!(!(false))) return;
...
BUF_4[buf_idx_4 * 3 + 1] = BUF_0[SLOT_0->second * 1 + 0];
...
auto reg_part__p_brand1_encoded = part__p_brand1_encoded[BUF_4[slot_second4 * 3 + 1]];
KEY_6 <<= 16;
...
auto reg_lineorder__lo_revenue = lineorder__lo_revenue[BUF_4[slot_second4 * 3 + 0]];
...
count_1<<<std::ceil((float)part_size/128.), 128>>>(d_COUNT0, d_part__p_brand1, part_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)part_size/128.), 128>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_part__p_brand1, d_part__p_partkey, part_size);
//Materialize count
uint64_t* d_COUNT2;
...
count_5<<<std::ceil((float)lineorder_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_COUNT4, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_lineorder__lo_partkey, d_lineorder__lo_suppkey, lineorder_size);
uint64_t COUNT4;
...
main_5<<<std::ceil((float)lineorder_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_4, d_BUF_IDX_4, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::insert), d_lineorder__lo_orderdate, d_lineorder__lo_partkey, d_lineorder__lo_suppkey, lineorder_size);
//Create aggregation hash table
auto d_HT_6 = cuco::static_map{ (int)1208*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_7<<<std::ceil((float)date_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::insert), d_date__d_datekey, d_date__d_year, date_size, d_part__p_brand1_encoded);
...
main_7<<<std::ceil((float)date_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::find), d_KEY_6date__d_year, d_KEY_6part__p_brand1_encoded, d_aggr0__tmp_attr0, d_date__d_datekey, d_date__d_year, date_size, d_lineorder__lo_revenue, d_part__p_brand1_encoded);
//Materialize count
uint64_t* d_COUNT8;
