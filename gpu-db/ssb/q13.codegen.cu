...
__global__ void main_1(uint64_t* BUF_0, uint64_t* BUF_IDX_0, HASHTABLE_INSERT_PK HT_0, DBI32Type* date__d_datekey, DBI32Type* date__d_weeknuminyear, DBI32Type* date__d_year, size_t date_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= date_size) return;
auto reg_date__d_weeknuminyear = date__d_weeknuminyear[tid];
if (!(evaluatePredicate(reg_date__d_weeknuminyear, 6, Predicate::eq))) return;
...
count_1<<<std::ceil((float)date_size/128.), 128>>>(d_COUNT0, d_date__d_weeknuminyear, d_date__d_year, date_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)date_size/128.), 128>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_date__d_datekey, d_date__d_weeknuminyear, d_date__d_year, date_size);
//Create aggregation hash table
