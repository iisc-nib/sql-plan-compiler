...
__global__ void count_11(uint64_t* COUNT22, DBStringType* part__p_category, size_t part_size) {
...
selection_flags[ITEM] &= evaluatePredicate(reg_part__p_category[ITEM], "MFGR#12", Predicate::eq);
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < part_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
__global__ void main_11(uint64_t* BUF_22, uint64_t* BUF_IDX_22, HASHTABLE_INSERT HT_22, int64_t* cycles_per_warp_main_11_join_build_22, int64_t* cycles_per_warp_main_11_selection_10, int64_t* cycles_per_warp_main_11_selection_12, int64_t* cycles_per_warp_main_11_selection_13, int64_t* cycles_per_warp_main_11_selection_14, int64_t* cycles_per_warp_main_11_selection_15, DBStringType* part__p_category, DBI32Type* part__p_partkey, size_t part_size) {
...
__global__ void count_17(uint64_t* COUNT23, DBStringType* supplier__s_region, size_t supplier_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
__global__ void count_8(uint64_t* COUNT24, size_t date_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_24 = atomicAdd((int*)BUF_IDX_24, 1);
HT_24.insert(cuco::pair{KEY_24[ITEM], buf_idx_24});
BUF_24[(buf_idx_24) * 1 + 0] = ITEM*TB + tid;
}
...
__global__ void count_1(uint64_t* BUF_22, uint64_t* BUF_23, uint64_t* BUF_24, HASHTABLE_PROBE HT_22, HASHTABLE_PROBE HT_23, HASHTABLE_PROBE HT_24, HASHTABLE_INSERT HT_25, DBI32Type* date__d_year, DBI32Type* lineorder__lo_orderdate, DBI32Type* lineorder__lo_partkey, DBI32Type* lineorder__lo_suppkey, size_t lineorder_size, DBI16Type* part__p_brand1_encoded) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_25 = HT_25.find(KEY_25[ITEM])->second;
...
__global__ void count_27(size_t COUNT25, uint64_t* COUNT26) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
count_11<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT22, d_part__p_category, part_size);
uint64_t COUNT22;
...
main_11<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_22, d_BUF_IDX_22, d_HT_22.ref(cuco::insert), d_cycles_per_warp_main_11_join_build_22, d_cycles_per_warp_main_11_selection_10, d_cycles_per_warp_main_11_selection_12, d_cycles_per_warp_main_11_selection_13, d_cycles_per_warp_main_11_selection_14, d_cycles_per_warp_main_11_selection_15, d_part__p_category, d_part__p_partkey, part_size);
...
count_17<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT23, d_supplier__s_region, supplier_size);
uint64_t COUNT23;
...
count_8<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT24, date_size);
uint64_t COUNT24;
...
main_8<<<std::ceil((float)date_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_24, d_BUF_IDX_24, d_HT_24.ref(cuco::insert), d_cycles_per_warp_main_8_join_build_24, d_cycles_per_warp_main_8_selection_7, d_cycles_per_warp_main_8_selection_9, d_date__d_datekey, date_size);
...
//Create aggregation hash table
auto d_HT_25 = cuco::static_map{ (int)52974*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_1<<<std::ceil((float)lineorder_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_22, d_BUF_23, d_BUF_24, d_HT_22.ref(cuco::find), d_HT_23.ref(cuco::find), d_HT_24.ref(cuco::find), d_HT_25.ref(cuco::insert), d_date__d_year, d_lineorder__lo_orderdate, d_lineorder__lo_partkey, d_lineorder__lo_suppkey, lineorder_size, d_part__p_brand1_encoded);
size_t COUNT25 = d_HT_25.size();
thrust::device_vector<int64_t> keys_25(COUNT25), vals_25(COUNT25);
d_HT_25.retrieve_all(keys_25.begin(), vals_25.begin());
d_HT_25.clear();
int64_t* raw_keys25 = thrust::raw_pointer_cast(keys_25.data());
insertKeys<<<std::ceil((float)COUNT25/128.), 128>>>(raw_keys25, d_HT_25.ref(cuco::insert), COUNT25);
...
count_27<<<std::ceil((float)COUNT25/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT25, d_COUNT26);
uint64_t COUNT26;
...
main_27<<<std::ceil((float)COUNT25/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT25, d_MAT26aggr0__tmp_attr0, d_MAT26date__d_year, d_MAT26part__p_brand1_encoded, d_MAT_IDX26, d_aggr0__tmp_attr0, d_cycles_per_warp_main_27_materialize_26, d_KEY_25date__d_year, d_KEY_25part__p_brand1_encoded);
