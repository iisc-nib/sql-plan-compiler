...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < customer_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
HT_19.insert(cuco::pair{KEY_19[ITEM], ITEM*TB + tid});
BUF_19[(ITEM*TB + tid) * 1 + 0] = ITEM*TB + tid;
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < supplier_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
HT_26.insert(cuco::pair{KEY_26[ITEM], ITEM*TB + tid});
BUF_26[(ITEM*TB + tid) * 1 + 0] = ITEM*TB + tid;
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < date_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
HT_27.insert(cuco::pair{KEY_27[ITEM], ITEM*TB + tid});
BUF_27[(ITEM*TB + tid) * 1 + 0] = ITEM*TB + tid;
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineorder_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_28 = get_aggregation_slot(KEY_28[ITEM], HT_28, SLOT_COUNT_28);
...
int* d_SLOT_COUNT_28;
cudaMalloc(&d_SLOT_COUNT_28, sizeof(int));
cudaMemset(d_SLOT_COUNT_28, 0, sizeof(int));
//Aggregate in hashtable
