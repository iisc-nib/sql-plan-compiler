...
ag = 0.;
__syncthreads();
if ((evaluatePredicate(reg_lineorder__lo_discount, 4, Predicate::gte) && evaluatePredicate(reg_lineorder__lo_discount, 6, Predicate::lte))) {
    auto reg_lineorder__lo_quantity = lineorder__lo_quantity[tid];
...
main_626db78a9f10<<<std::ceil((float)lineorder_size/256.), 256>>>(d_BUF_626db78bf120, d_HT_626db787ae30.ref(cuco::find), d_HT_626db78bf120.ref(cuco::for_each), d_aggr0__tmp_attr0, d_lineorder__lo_discount, d_lineorder__lo_extendedprice, d_lineorder__lo_orderdate, d_lineorder__lo_quantity, lineorder_size);
//Materialize count
