...
__global__ void count_3(uint64_t* COUNT6, DBStringType* part__p_name, size_t part_size) {
...
__global__ void count_7(uint64_t* COUNT8, HASHTABLE_PROBE_SJ HT_6, DBI32Type* partsupp__ps_partkey, size_t partsupp_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
__global__ void count_5(uint64_t* BUF_8, HASHTABLE_PROBE HT_8, HASHTABLE_INSERT HT_9, DBI32Type* lineitem__l_partkey, DBDateType* lineitem__l_shipdate, DBI32Type* lineitem__l_suppkey, size_t lineitem_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
aggregate_any(&moved_aggr_u_1__ps_partkey[buf_idx_9], reg_partsupp__ps_partkey[ITEM]);
aggregate_any(&moved_aggr_u_2__ps_availqty[buf_idx_9], reg_partsupp__ps_availqty[ITEM]);
...
__global__ void count_1(uint64_t* COUNT10, DBStringType* nation__n_name, size_t nation_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
__global__ void count_13(uint64_t* COUNT16, size_t COUNT9, DBDecimalType* aggr0__tmp_attr0, DBI32Type* moved_aggr__ps_suppkey, DBI32Type* moved_aggr_u_1__ps_partkey, DBI32Type* moved_aggr_u_2__ps_availqty) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < COUNT9); ++ITEM) {
reg_partsupp__ps_availqty[ITEM] = moved_aggr_u_2__ps_availqty[ITEM*TB + tid];
}
...
__global__ void count_11(uint64_t* BUF_10, uint64_t* COUNT17, HASHTABLE_PROBE HT_10, HASHTABLE_PROBE_SJ HT_16, DBI32Type* supplier__s_nationkey, DBI32Type* supplier__s_suppkey, size_t supplier_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
count_3<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT6, d_part__p_name, part_size);
uint64_t COUNT6;
...
main_3<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_HT_6.ref(cuco::insert), d_cycles_per_warp_main_3_selection_2, d_cycles_per_warp_main_3_semi_join_build_6, d_part__p_name, d_part__p_partkey, part_size);
...
count_7<<<std::ceil((float)partsupp_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT8, d_HT_6.ref(cuco::find), d_partsupp__ps_partkey, partsupp_size);
uint64_t COUNT8;
...
main_7<<<std::ceil((float)partsupp_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_8, d_BUF_IDX_8, d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::insert), d_cycles_per_warp_main_7_join_build_8, d_cycles_per_warp_main_7_semi_join_probe_6, d_partsupp__ps_partkey, d_partsupp__ps_suppkey, partsupp_size);
...
count_5<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_8, d_HT_8.ref(cuco::find), d_HT_9.ref(cuco::insert), d_lineitem__l_partkey, d_lineitem__l_shipdate, d_lineitem__l_suppkey, lineitem_size);
...
cudaMalloc(&d_moved_aggr_u_1__ps_partkey, sizeof(DBI32Type) * COUNT9);
cudaMemset(d_moved_aggr_u_1__ps_partkey, 0, sizeof(DBI32Type) * COUNT9);
DBI32Type* d_moved_aggr_u_2__ps_availqty;
cudaMalloc(&d_moved_aggr_u_2__ps_availqty, sizeof(DBI32Type) * COUNT9);
cudaMemset(d_moved_aggr_u_2__ps_availqty, 0, sizeof(DBI32Type) * COUNT9);
...
count_1<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT10, d_nation__n_name, nation_size);
uint64_t COUNT10;
...
main_1<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_10, d_BUF_IDX_10, d_HT_10.ref(cuco::insert), d_cycles_per_warp_main_1_join_build_10, d_cycles_per_warp_main_1_selection_0, d_nation__n_name, d_nation__n_nationkey, nation_size);
...
count_13<<<std::ceil((float)COUNT9/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT16, COUNT9, d_aggr0__tmp_attr0, d_moved_aggr__ps_suppkey, d_moved_aggr_u_1__ps_partkey, d_moved_aggr_u_2__ps_availqty);
uint64_t COUNT16;
...
main_13<<<std::ceil((float)COUNT9/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT9, d_HT_16.ref(cuco::insert), d_aggr0__tmp_attr0, d_cycles_per_warp_main_13_map_12, d_cycles_per_warp_main_13_selection_14, d_cycles_per_warp_main_13_selection_15, d_cycles_per_warp_main_13_semi_join_build_16, d_moved_aggr__ps_suppkey, d_moved_aggr_u_1__ps_partkey, d_moved_aggr_u_2__ps_availqty);
...
count_11<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_10, d_COUNT17, d_HT_10.ref(cuco::find), d_HT_16.ref(cuco::find), d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
uint64_t COUNT17;
...
main_11<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_10, d_HT_10.ref(cuco::find), d_HT_16.ref(cuco::find), d_MAT17supplier__s_address_encoded, d_MAT17supplier__s_name_encoded, d_MAT_IDX17, d_cycles_per_warp_main_11_join_probe_10, d_cycles_per_warp_main_11_materialize_17, d_cycles_per_warp_main_11_semi_join_probe_16, d_supplier__s_address_encoded, d_supplier__s_name_encoded, d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
