...
__global__ void count_7(uint64_t* COUNT14, DBStringType* part__p_brand, DBStringType* part__p_container, DBI32Type* part__p_size, size_t part_size) {
...
selection_flags[ITEM] &= ((evaluatePredicate(reg_part__p_brand[ITEM], "Brand#12", Predicate::eq)) && ((evaluatePredicate(reg_part__p_container[ITEM], "SM CASE", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "SM BOX", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "SM PACK", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "SM PKG", Predicate::eq))) && (evaluatePredicate(reg_part__p_size[ITEM], 1, Predicate::gte) && evaluatePredicate(reg_part__p_size[ITEM], 5, Predicate::lte))) || ((evaluatePredicate(reg_part__p_brand[ITEM], "Brand#23", Predicate::eq)) && (evaluatePredicate(reg_part__p_size[ITEM], 1, Predicate::gte) && evaluatePredicate(reg_part__p_size[ITEM], 10, Predicate::lte)) && ((evaluatePredicate(reg_part__p_container[ITEM], "MED BAG", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "MED BOX", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "MED PKG", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "MED PACK", Predicate::eq)))) || (((evaluatePredicate(reg_part__p_container[ITEM], "LG CASE", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "LG BOX", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "LG PACK", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "LG PKG", Predicate::eq))) && (evaluatePredicate(reg_part__p_brand[ITEM], "Brand#34", Predicate::eq)) && (evaluatePredicate(reg_part__p_size[ITEM], 1, Predicate::gte) && evaluatePredicate(reg_part__p_size[ITEM], 15, Predicate::lte)));
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < part_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
...
__global__ void count_1(uint64_t* BUF_14, HASHTABLE_PROBE HT_14, HASHTABLE_INSERT HT_16, DBI32Type* lineitem__l_partkey, DBDecimalType* lineitem__l_quantity, DBStringType* lineitem__l_shipinstruct, DBStringType* lineitem__l_shipmode, size_t lineitem_size, DBStringType* part__p_brand, DBStringType* part__p_container, DBI32Type* part__p_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
selection_flags[ITEM] &= (((evaluatePredicate(reg_part__p_brand[ITEM], "Brand#12", Predicate::eq)) && (evaluatePredicate(reg_lineitem__l_quantity[ITEM], 1.0, Predicate::gte) && evaluatePredicate(reg_lineitem__l_quantity[ITEM], 11.00, Predicate::lte)) && ((evaluatePredicate(reg_part__p_container[ITEM], "SM CASE", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "SM BOX", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "SM PACK", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "SM PKG", Predicate::eq))) && (evaluatePredicate(reg_part__p_size[ITEM], 1, Predicate::gte) && evaluatePredicate(reg_part__p_size[ITEM], 5, Predicate::lte))) || ((evaluatePredicate(reg_lineitem__l_quantity[ITEM], 10.0, Predicate::gte) && evaluatePredicate(reg_lineitem__l_quantity[ITEM], 20.0, Predicate::lte)) && (evaluatePredicate(reg_part__p_brand[ITEM], "Brand#23", Predicate::eq)) && (evaluatePredicate(reg_part__p_size[ITEM], 1, Predicate::gte) && evaluatePredicate(reg_part__p_size[ITEM], 10, Predicate::lte)) && ((evaluatePredicate(reg_part__p_container[ITEM], "MED BAG", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "MED BOX", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "MED PKG", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "MED PACK", Predicate::eq)))) || (((evaluatePredicate(reg_part__p_container[ITEM], "LG CASE", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "LG BOX", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "LG PACK", Predicate::eq)) || (evaluatePredicate(reg_part__p_container[ITEM], "LG PKG", Predicate::eq))) && (evaluatePredicate(reg_part__p_brand[ITEM], "Brand#34", Predicate::eq)) && (evaluatePredicate(reg_lineitem__l_quantity[ITEM], 20.0, Predicate::gte) && evaluatePredicate(reg_lineitem__l_quantity[ITEM], 30.00, Predicate::lte)) && (evaluatePredicate(reg_part__p_size[ITEM], 1, Predicate::gte) && evaluatePredicate(reg_part__p_size[ITEM], 15, Predicate::lte)))) && (true);
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_16 = HT_16.find(KEY_16[ITEM])->second;
...
__global__ void count_18(size_t COUNT16, uint64_t* COUNT17) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
count_7<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT14, d_part__p_brand, d_part__p_container, d_part__p_size, part_size);
uint64_t COUNT14;
...
count_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_14, d_HT_14.ref(cuco::find), d_HT_16.ref(cuco::insert), d_lineitem__l_partkey, d_lineitem__l_quantity, d_lineitem__l_shipinstruct, d_lineitem__l_shipmode, lineitem_size, d_part__p_brand, d_part__p_container, d_part__p_size);
size_t COUNT16 = d_HT_16.size();
thrust::device_vector<int64_t> keys_16(COUNT16), vals_16(COUNT16);
d_HT_16.retrieve_all(keys_16.begin(), vals_16.begin());
d_HT_16.clear();
int64_t* raw_keys16 = thrust::raw_pointer_cast(keys_16.data());
insertKeys<<<std::ceil((float)COUNT16/128.), 128>>>(raw_keys16, d_HT_16.ref(cuco::insert), COUNT16);
...
count_18<<<std::ceil((float)COUNT16/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT16, d_COUNT17);
uint64_t COUNT17;
...
main_18<<<std::ceil((float)COUNT16/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT16, d_MAT17aggr0__tmp_attr0, d_MAT_IDX17, d_aggr0__tmp_attr0, d_cycles_per_warp_main_18_materialize_17);
