...
__global__ void main_2(uint64_t* BUF_0, HASHTABLE_PROBE HT_0, HASHTABLE_FIND HT_3, DBI32Type* KEY_3customer__c_custkey, int* SLOT_COUNT_3, DBI64Type* aggr0__tmp_attr0, DBI32Type* customer__c_custkey, int64_t* cycles_per_warp_main_2_aggregation_3, int64_t* cycles_per_warp_main_2_join_probe_0, DBI32Type* orders__o_custkey, DBI32Type* orders__o_orderkey, size_t orders_size) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < orders_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_3 = get_aggregation_slot(KEY_3[ITEM], HT_3, SLOT_COUNT_3);
...
__global__ void main_5(size_t COUNT3, HASHTABLE_FIND HT_4, DBI64Type* KEY_4aggr0__tmp_attr0, int* SLOT_COUNT_4, DBI64Type* aggr0__tmp_attr0, DBI64Type* aggr1__tmp_attr1, int64_t* cycles_per_warp_main_5_aggregation_4) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
main_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_customer__c_custkey, customer_size, d_cycles_per_warp_main_1_join_build_0);
...
int* d_SLOT_COUNT_3;
cudaMalloc(&d_SLOT_COUNT_3, sizeof(int));
cudaMemset(d_SLOT_COUNT_3, 0, sizeof(int));
//Aggregate in hashtable
...
main_2<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_3.ref(cuco::insert_and_find), d_KEY_3customer__c_custkey, d_SLOT_COUNT_3, d_aggr0__tmp_attr0, d_customer__c_custkey, d_cycles_per_warp_main_2_aggregation_3, d_cycles_per_warp_main_2_join_probe_0, d_orders__o_custkey, d_orders__o_orderkey, orders_size);
...
main_5<<<std::ceil((float)COUNT3/(float)TILE_SIZE), TB>>>(COUNT3, d_HT_4.ref(cuco::insert_and_find), d_KEY_4aggr0__tmp_attr0, d_SLOT_COUNT_4, d_aggr0__tmp_attr0, d_aggr1__tmp_attr1, d_cycles_per_warp_main_5_aggregation_4);
...
main_7<<<std::ceil((float)COUNT4/(float)TILE_SIZE), TB>>>(COUNT4, d_MAT6aggr0__tmp_attr0, d_MAT6aggr1__tmp_attr1, d_MAT_IDX6, d_KEY_4aggr0__tmp_attr0, d_aggr1__tmp_attr1, d_cycles_per_warp_main_7_materialize_6);
