...
__global__ void main_1(uint64_t* BUF_0, uint64_t* BUF_IDX_0, HASHTABLE_INSERT HT_0, DBI32Type* orders__o_custkey, DBDateType* orders__o_orderdate, size_t orders_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= orders_size) return;
auto reg_orders__o_orderdate = orders__o_orderdate[tid];
if (!(evaluatePredicate(reg_orders__o_orderdate, 8674, Predicate::gte) && evaluatePredicate(reg_orders__o_orderdate, 8766, Predicate::lt))) return;
...
__global__ void count_3(uint64_t* COUNT2, size_t nation_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void main_3(uint64_t* BUF_2, uint64_t* BUF_IDX_2, HASHTABLE_INSERT_PK HT_2, DBI32Type* nation__n_nationkey, size_t nation_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= nation_size) return;
uint64_t KEY_2 = 0;
...
__global__ void count_5(uint64_t* BUF_0, uint64_t* BUF_2, uint64_t* COUNT4, HASHTABLE_PROBE HT_0, HASHTABLE_PROBE_PK HT_2, DBI32Type* customer__c_custkey, DBI32Type* customer__c_nationkey, size_t customer_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename HASHTABLE_PROBE, typename HASHTABLE_PROBE_PK, typename HASHTABLE_INSERT_PK>
__global__ void main_5(uint64_t* BUF_0, uint64_t* BUF_2, uint64_t* BUF_4, uint64_t* BUF_IDX_4, HASHTABLE_PROBE HT_0, HASHTABLE_PROBE_PK HT_2, HASHTABLE_INSERT_PK HT_4, DBI32Type* customer__c_custkey, DBI32Type* customer__c_nationkey, size_t customer_size, DBI32Type* orders__o_orderkey) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= customer_size) return;
uint64_t KEY_0 = 0;
auto reg_customer__c_custkey = customer__c_custkey[tid];
...
if (!(true)) return;
uint64_t KEY_4 = 0;
auto reg_orders__o_orderkey = orders__o_orderkey[BUF_0[slot_second0 * 1 + 0]];
...
BUF_4[buf_idx_4 * 3 + 1] = BUF_0[slot_second0 * 1 + 0];
BUF_4[buf_idx_4 * 3 + 2] = BUF_2[SLOT_2->second * 1 + 0];
});
}
template<typename HASHTABLE_PROBE_PK, typename HASHTABLE_INSERT>
__global__ void count_7(uint64_t* BUF_4, HASHTABLE_PROBE_PK HT_4, HASHTABLE_INSERT HT_6, DBI32Type* customer__c_custkey, DBI32Type* lineitem__l_orderkey, DBCharType* lineitem__l_returnflag, size_t lineitem_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
if (!(true)) return;
uint64_t KEY_6 = 0;
auto reg_customer__c_custkey = customer__c_custkey[BUF_4[SLOT_4->second * 3 + 0]];
...
__global__ void main_7(uint64_t* BUF_4, HASHTABLE_PROBE_PK HT_4, HASHTABLE_FIND HT_6, DBI32Type* KEY_6customer__c_custkey, DBDecimalType* aggr0__tmp_attr0, DBDecimalType* aggr__c_acctbal, DBI16Type* aggr__n_name_encoded, DBDecimalType* customer__c_acctbal, DBI32Type* customer__c_custkey, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBI32Type* lineitem__l_orderkey, DBCharType* lineitem__l_returnflag, size_t lineitem_size, DBI16Type* nation__n_name_encoded) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineitem_size) return;
auto reg_lineitem__l_returnflag = lineitem__l_returnflag[tid];
if (!(evaluatePredicate(reg_lineitem__l_returnflag, 'R', Predicate::eq))) return;
...
auto reg_nation__n_name_encoded = nation__n_name_encoded[BUF_4[SLOT_4->second * 3 + 2]];
aggregate_any(&aggr__n_name_encoded[buf_idx_6], reg_nation__n_name_encoded);
auto reg_customer__c_acctbal = customer__c_acctbal[BUF_4[SLOT_4->second * 3 + 0]];
...
main_1<<<std::ceil((float)orders_size/128.), 128>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_orders__o_custkey, d_orders__o_orderdate, orders_size);
//Materialize count
uint64_t* d_COUNT2;
cudaMalloc(&d_COUNT2, sizeof(uint64_t));
cudaMemset(d_COUNT2, 0, sizeof(uint64_t));
count_3<<<std::ceil((float)nation_size/128.), 128>>>(d_COUNT2, nation_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)nation_size/128.), 128>>>(d_BUF_2, d_BUF_IDX_2, d_HT_2.ref(cuco::insert), d_nation__n_nationkey, nation_size);
//Materialize count
uint64_t* d_COUNT4;
cudaMalloc(&d_COUNT4, sizeof(uint64_t));
cudaMemset(d_COUNT4, 0, sizeof(uint64_t));
count_5<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_COUNT4, d_HT_0.ref(cuco::for_each), d_HT_2.ref(cuco::find), d_customer__c_custkey, d_customer__c_nationkey, customer_size);
uint64_t COUNT4;
...
main_5<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_4, d_BUF_IDX_4, d_HT_0.ref(cuco::for_each), d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::insert), d_customer__c_custkey, d_customer__c_nationkey, customer_size, d_orders__o_orderkey);
//Create aggregation hash table
auto d_HT_6 = cuco::static_map{ (int)62017*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_7<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_customer__c_custkey, d_lineitem__l_orderkey, d_lineitem__l_returnflag, lineitem_size);
...
main_7<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_KEY_6customer__c_custkey, d_aggr0__tmp_attr0, d_aggr__c_acctbal, d_aggr__n_name_encoded, d_customer__c_acctbal, d_customer__c_custkey, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_returnflag, lineitem_size, d_nation__n_name_encoded);
//Materialize count
uint64_t* d_COUNT8;
