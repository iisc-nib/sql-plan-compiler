...
BUF_8[(ITEM*TB + tid) * 3 + 0] = ITEM*TB + tid;
BUF_8[(ITEM*TB + tid) * 3 + 1] = BUF_6[slot_second6[ITEM] * 2 + 0];
BUF_8[(ITEM*TB + tid) * 3 + 2] = BUF_6[slot_second6[ITEM] * 2 + 1];
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < orders_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
HT_9.insert(cuco::pair{KEY_9[ITEM], ITEM*TB + tid});
BUF_9[(ITEM*TB + tid) * 4 + 0] = BUF_8[slot_second8[ITEM] * 3 + 0];
BUF_9[(ITEM*TB + tid) * 4 + 1] = ITEM*TB + tid;
BUF_9[(ITEM*TB + tid) * 4 + 2] = BUF_8[slot_second8[ITEM] * 3 + 1];
BUF_9[(ITEM*TB + tid) * 4 + 3] = BUF_8[slot_second8[ITEM] * 3 + 2];
}
...
__global__ void main_10(uint64_t* BUF_11, uint64_t* BUF_9, HASHTABLE_PROBE HT_11, HASHTABLE_FIND HT_14, HASHTABLE_PROBE HT_9, DBI16Type* KEY_14nation__n_name_encoded, int* SLOT_COUNT_14, DBDecimalType* aggr0__tmp_attr0, DBI32Type* customer__c_nationkey, int64_t* cycles_per_warp_main_10_aggregation_14, int64_t* cycles_per_warp_main_10_join_probe_11, int64_t* cycles_per_warp_main_10_join_probe_9, int64_t* cycles_per_warp_main_10_map_13, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBI32Type* lineitem__l_orderkey, DBI32Type* lineitem__l_suppkey, size_t lineitem_size, DBI16Type* nation__n_name_encoded) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
main_3<<<std::ceil((float)region_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_cycles_per_warp_main_3_join_build_4, d_cycles_per_warp_main_3_selection_2, d_region__r_name, d_region__r_regionkey, region_size);
...
main_5<<<std::ceil((float)nation_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_6, d_BUF_IDX_6, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_cycles_per_warp_main_5_join_build_6, d_cycles_per_warp_main_5_join_probe_4, d_nation__n_nationkey, d_nation__n_regionkey, nation_size);
...
main_7<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_6, d_BUF_8, d_BUF_IDX_8, d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::insert), d_customer__c_custkey, d_customer__c_nationkey, customer_size, d_cycles_per_warp_main_7_join_build_8, d_cycles_per_warp_main_7_join_probe_6);
...
main_1<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_8, d_BUF_9, d_BUF_IDX_9, d_HT_8.ref(cuco::find), d_HT_9.ref(cuco::insert), d_cycles_per_warp_main_1_join_build_9, d_cycles_per_warp_main_1_join_probe_8, d_cycles_per_warp_main_1_selection_0, d_orders__o_custkey, d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
...
main_12<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TB>>>(d_BUF_11, d_BUF_IDX_11, d_HT_11.ref(cuco::insert), d_cycles_per_warp_main_12_join_build_11, d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
...
main_10<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_11, d_BUF_9, d_HT_11.ref(cuco::find), d_HT_14.ref(cuco::insert_and_find), d_HT_9.ref(cuco::find), d_KEY_14nation__n_name_encoded, d_SLOT_COUNT_14, d_aggr0__tmp_attr0, d_customer__c_nationkey, d_cycles_per_warp_main_10_aggregation_14, d_cycles_per_warp_main_10_join_probe_11, d_cycles_per_warp_main_10_join_probe_9, d_cycles_per_warp_main_10_map_13, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded);
...
main_16<<<std::ceil((float)COUNT14/(float)TILE_SIZE), TB>>>(COUNT14, d_MAT15aggr0__tmp_attr0, d_MAT15nation__n_name_encoded, d_MAT_IDX15, d_aggr0__tmp_attr0, d_cycles_per_warp_main_16_materialize_15, d_KEY_14nation__n_name_encoded);
uint64_t MATCOUNT_15 = 0;
cudaMemcpy(&MATCOUNT_15, d_MAT_IDX15, sizeof(uint64_t), cudaMemcpyDeviceToHost);
