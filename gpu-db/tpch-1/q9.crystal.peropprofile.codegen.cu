...
__global__ void count_2(uint64_t* BUF_0, uint64_t* COUNT3, HASHTABLE_PROBE HT_0, DBI32Type* supplier__s_nationkey, size_t supplier_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
BUF_3[(buf_idx_3) * 2 + 0] = ITEM*TB + tid;
BUF_3[(buf_idx_3) * 2 + 1] = BUF_0[slot_second0[ITEM] * 1 + 0];
}
...
__global__ void count_9(uint64_t* COUNT8, size_t part_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
template<typename HASHTABLE_PROBE>
__global__ void count_4(uint64_t* BUF_3, uint64_t* COUNT10, HASHTABLE_PROBE HT_3, DBI32Type* partsupp__ps_suppkey, size_t partsupp_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
BUF_10[(buf_idx_10) * 3 + 0] = BUF_3[slot_second3[ITEM] * 2 + 0];
BUF_10[(buf_idx_10) * 3 + 1] = ITEM*TB + tid;
BUF_10[(buf_idx_10) * 3 + 2] = BUF_3[slot_second3[ITEM] * 2 + 1];
}
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_4_join_build_10[blockIdx.x] = cycles_per_warp;}
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
__global__ void count_7(uint64_t* BUF_10, uint64_t* BUF_5, uint64_t* BUF_8, HASHTABLE_PROBE HT_10, HASHTABLE_INSERT HT_13, HASHTABLE_PROBE HT_5, HASHTABLE_PROBE HT_8, DBI32Type* lineitem__l_orderkey, DBI32Type* lineitem__l_partkey, DBI32Type* lineitem__l_suppkey, size_t lineitem_size, DBI16Type* nation__n_name_encoded, DBDateType* orders__o_orderdate) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_13 = HT_13.find(KEY_13[ITEM])->second;
...
__global__ void count_15(size_t COUNT13, uint64_t* COUNT14) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
main_1<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_cycles_per_warp_main_1_join_build_0, d_nation__n_nationkey, nation_size);
...
count_2<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_COUNT3, d_HT_0.ref(cuco::find), d_supplier__s_nationkey, supplier_size);
uint64_t COUNT3;
...
main_2<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_3, d_BUF_IDX_3, d_HT_0.ref(cuco::find), d_HT_3.ref(cuco::insert), d_cycles_per_warp_main_2_join_build_3, d_cycles_per_warp_main_2_join_probe_0, d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
...
count_9<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT8, part_size);
uint64_t COUNT8;
...
main_9<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_8, d_BUF_IDX_8, d_HT_8.ref(cuco::insert), d_cycles_per_warp_main_9_join_build_8, d_part__p_partkey, part_size);
...
count_4<<<std::ceil((float)partsupp_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_3, d_COUNT10, d_HT_3.ref(cuco::find), d_partsupp__ps_suppkey, partsupp_size);
uint64_t COUNT10;
...
main_4<<<std::ceil((float)partsupp_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_10, d_BUF_3, d_BUF_IDX_10, d_HT_10.ref(cuco::insert), d_HT_3.ref(cuco::find), d_cycles_per_warp_main_4_join_build_10, d_cycles_per_warp_main_4_join_probe_3, d_partsupp__ps_partkey, d_partsupp__ps_suppkey, partsupp_size, d_supplier__s_suppkey);
...
count_7<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_10, d_BUF_5, d_BUF_8, d_HT_10.ref(cuco::find), d_HT_13.ref(cuco::insert), d_HT_5.ref(cuco::find), d_HT_8.ref(cuco::find), d_lineitem__l_orderkey, d_lineitem__l_partkey, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded, d_orders__o_orderdate);
size_t COUNT13 = d_HT_13.size();
thrust::device_vector<int64_t> keys_13(COUNT13), vals_13(COUNT13);
d_HT_13.retrieve_all(keys_13.begin(), vals_13.begin());
d_HT_13.clear();
int64_t* raw_keys13 = thrust::raw_pointer_cast(keys_13.data());
insertKeys<<<std::ceil((float)COUNT13/128.), 128>>>(raw_keys13, d_HT_13.ref(cuco::insert), COUNT13);
...
count_15<<<std::ceil((float)COUNT13/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT13, d_COUNT14);
uint64_t COUNT14;
...
main_15<<<std::ceil((float)COUNT13/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT13, d_MAT14aggr0__tmp_attr2, d_MAT14map0__tmp_attr0, d_MAT14nation__n_name_encoded, d_MAT_IDX14, d_aggr0__tmp_attr2, d_cycles_per_warp_main_15_materialize_14, d_KEY_13map0__tmp_attr0, d_KEY_13nation__n_name_encoded);
