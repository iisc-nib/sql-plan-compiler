...
__global__ void count_1(uint64_t* COUNT4, DBStringType* customer__c_mktsegment, size_t customer_size) {
...
__global__ void count_3(uint64_t* BUF_4, uint64_t* COUNT7, HASHTABLE_PROBE HT_4, DBI32Type* orders__o_custkey, DBDateType* orders__o_orderdate, size_t orders_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
BUF_7[(buf_idx_7) * 2 + 0] = BUF_4[slot_second4[ITEM] * 1 + 0];
BUF_7[(buf_idx_7) * 2 + 1] = ITEM*TB + tid;
}
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_3_join_build_7[blockIdx.x] = cycles_per_warp;}
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
__global__ void count_6(uint64_t* BUF_7, HASHTABLE_PROBE HT_7, HASHTABLE_INSERT HT_9, DBI32Type* lineitem__l_orderkey, DBDateType* lineitem__l_shipdate, size_t lineitem_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
__global__ void main_6(uint64_t* BUF_7, HASHTABLE_PROBE HT_7, HASHTABLE_FIND HT_9, DBI32Type* KEY_9lineitem__l_orderkey, DBDecimalType* aggr0__tmp_attr0, DBDateType* aggr__o_orderdate, DBI32Type* aggr__o_shippriority, int64_t* cycles_per_warp_main_6_aggregation_9, int64_t* cycles_per_warp_main_6_join_probe_7, int64_t* cycles_per_warp_main_6_map_8, int64_t* cycles_per_warp_main_6_selection_5, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBI32Type* lineitem__l_orderkey, DBDateType* lineitem__l_shipdate, size_t lineitem_size, DBDateType* orders__o_orderdate, DBI32Type* orders__o_shippriority) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
count_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT4, d_customer__c_mktsegment, customer_size);
uint64_t COUNT4;
...
main_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_customer__c_custkey, d_customer__c_mktsegment, customer_size, d_cycles_per_warp_main_1_join_build_4, d_cycles_per_warp_main_1_selection_0);
...
count_3<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_4, d_COUNT7, d_HT_4.ref(cuco::find), d_orders__o_custkey, d_orders__o_orderdate, orders_size);
uint64_t COUNT7;
...
count_6<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_7, d_HT_7.ref(cuco::find), d_HT_9.ref(cuco::insert), d_lineitem__l_orderkey, d_lineitem__l_shipdate, lineitem_size);
