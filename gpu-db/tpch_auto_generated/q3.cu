...
__global__ void pipeline_1 (int32_t* o_custkey, int32_t* c_custkey, int8_t* c_mktsegment, int64_t* B0_idx, size_t customer_size) {
  int64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  if (tid >= customer_size) return;
  int8_t reg_c_mktsegment = c_mktsegment[tid];
  if (!(reg_c_mktsegment == 1)) return;
  ...
  __global__ void pipeline_0 (int32_t* o_custkey, int32_t* c_custkey, int8_t* c_mktsegment, TY_HT0_I HT0_I, TY_HT0_F HT0_F, int64_t* B0_customer, int64_t* B0_idx, size_t customer_size) {
  int64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  if (tid >= customer_size) return;
  int8_t reg_c_mktsegment = c_mktsegment[tid];
  if (!(reg_c_mktsegment == 1)) return;
  ...
  template <typename TY_HT0_I, typename TY_HT0_F>
  __global__ void pipeline_3 (int32_t* o_orderdate, int32_t* o_custkey, int32_t* l_orderkey, int32_t* o_orderkey, TY_HT0_I HT0_I, TY_HT0_F HT0_F, int64_t* B0_customer, size_t orders_size, int64_t* B1_idx) {
  int64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  __global__ void pipeline_2 (int32_t* o_orderdate, int32_t* o_custkey, int32_t* l_orderkey, int32_t* o_orderkey, TY_HT0_I HT0_I, TY_HT0_F HT0_F, TY_HT1_I HT1_I, TY_HT1_F HT1_F, int64_t* B1_idx, size_t orders_size, int64_t* B1_customer, int64_t* B1_orders, int64_t* B0_customer) {
  int64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  if (tid >= orders_size) return;
  int32_t reg_o_orderdate = o_orderdate[tid];
  if (!(reg_o_orderdate < 9204)) return;
  ...
  __global__ void pipeline_4 (double* l_extendedprice, int32_t* l_orderkey, int32_t* l_shipdate, double* l_discount, TY_HT1_I HT1_I, TY_HT1_F HT1_F, TY_HT2_I HT2_I, TY_HT2_F HT2_F, int64_t* B1_orders, size_t lineitem_size, int64_t* B1_customer) {
  int64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  template <typename TY_HT1_I, typename TY_HT1_F, typename TY_HT2_I, typename TY_HT2_F>
  __global__ void pipeline_5 (int32_t* o_shippriority, int32_t* l_orderkey, int32_t* l_shipdate, double* l_extendedprice, int32_t* o_orderdate, double* l_discount, int32_t* agg_o_orderdate, int32_t* agg_l_orderkey, int32_t* agg_o_shippriority, double* revenue, double* agg_l_discount, TY_HT1_I HT1_I, TY_HT1_F HT1_F, TY_HT2_I HT2_I, TY_HT2_F HT2_F, int64_t* B1_orders, size_t lineitem_size, int64_t* B1_customer) {
  int64_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  if (tid >= lineitem_size) return;
  int32_t reg_l_shipdate = l_shipdate[tid];
  if (!(reg_l_shipdate > 9204)) return;
  ...
  pipeline_1<<<std::ceil((float)customer_size/(float)32), 32>>>(d_o_custkey, d_c_custkey, d_c_mktsegment, B0_idx, customer_size);
  ...
  pipeline_0<<<std::ceil((float)customer_size/(float)32), 32>>>(d_o_custkey, d_c_custkey, d_c_mktsegment, d_HT0_I, d_HT0_F, B0_customer, B0_idx, customer_size);
  ...
  pipeline_3<<<std::ceil((float)orders_size/(float)32), 32>>>(d_o_orderdate, d_o_custkey, d_l_orderkey, d_o_orderkey, d_HT0_I, d_HT0_F, B0_customer, orders_size, B1_idx);
  ...
  pipeline_2<<<std::ceil((float)orders_size/(float)32), 32>>>(d_o_orderdate, d_o_custkey, d_l_orderkey, d_o_orderkey, d_HT0_I, d_HT0_F, d_HT1_I, d_HT1_F, B1_idx, orders_size, B1_customer, B1_orders, B0_customer);
  ...
  pipeline_4<<<std::ceil((float)lineitem_size/(float)32), 32>>>(d_l_extendedprice, d_l_orderkey, d_l_shipdate, d_l_discount, d_HT1_I, d_HT1_F, d_HT2_I, d_HT2_F, B1_orders, lineitem_size, B1_customer);
  ...
  pipeline_5<<<std::ceil((float)lineitem_size/(float)32), 32>>>(d_o_shippriority, d_l_orderkey, d_l_shipdate, d_l_extendedprice, d_o_orderdate, d_l_discount, d_agg_o_orderdate, d_agg_l_orderkey, d_agg_o_shippriority, d_revenue, d_agg_l_discount, d_HT1_I, d_HT1_F, d_HT2_I, d_HT2_F, B1_orders, lineitem_size, B1_customer);
