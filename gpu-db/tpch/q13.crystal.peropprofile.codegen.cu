...
__global__ void main_1(uint64_t* BUF_0, uint64_t* BUF_IDX_0, HASHTABLE_INSERT HT_0, DBI32Type* customer__c_custkey, size_t customer_size, int64_t* cycles_per_warp_main_1_join_build_0) {
...
__global__ void count_2(uint64_t* BUF_0, HASHTABLE_PROBE HT_0, HASHTABLE_INSERT HT_3, DBI32Type* customer__c_custkey, DBI32Type* orders__o_custkey, size_t orders_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
if (!selection_flags[ITEM]) continue;
KEY_3[ITEM] = 0;
KEY_3[ITEM] |= reg_customer__c_custkey[ITEM];
}
//Create aggregation hash table
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < orders_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
HT_3.insert(cuco::pair{KEY_3[ITEM], 1});
}
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_FIND>
__global__ void main_2(uint64_t* BUF_0, HASHTABLE_PROBE HT_0, HASHTABLE_FIND HT_3, DBI32Type* KEY_3customer__c_custkey, DBI64Type* aggr0__tmp_attr0, DBI32Type* customer__c_custkey, int64_t* cycles_per_warp_main_2_aggregation_3, int64_t* cycles_per_warp_main_2_join_probe_0, DBI32Type* orders__o_custkey, DBI32Type* orders__o_orderkey, size_t orders_size) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < orders_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_3 = HT_3.find(KEY_3[ITEM])->second;
aggregate_sum(&aggr0__tmp_attr0[buf_idx_3], 1);
KEY_3customer__c_custkey[buf_idx_3] = reg_customer__c_custkey[ITEM];
}
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_2_aggregation_3[blockIdx.x] = cycles_per_warp;}
}
template<typename HASHTABLE_INSERT>
__global__ void count_5(size_t COUNT3, HASHTABLE_INSERT HT_4, DBI64Type* aggr0__tmp_attr0) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < COUNT3); ++ITEM) {
...
__global__ void main_5(size_t COUNT3, HASHTABLE_FIND HT_4, DBI64Type* KEY_4aggr0__tmp_attr0, DBI64Type* aggr0__tmp_attr0, DBI64Type* aggr1__tmp_attr1, int64_t* cycles_per_warp_main_5_aggregation_4) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
if (threadIdx.x == 0) {            stop = clock64();            cycles_per_warp = (stop - start);            cycles_per_warp_main_5_aggregation_4[blockIdx.x] = cycles_per_warp;}
}
...
__global__ void main_7(size_t COUNT4, DBI64Type* MAT6aggr0__tmp_attr0, DBI64Type* MAT6aggr1__tmp_attr1, uint64_t* MAT_IDX6, DBI64Type* aggr0__tmp_attr0, DBI64Type* aggr1__tmp_attr1, int64_t* cycles_per_warp_main_7_materialize_6) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
auto main_1_join_build_0_cpw_size = std::ceil((float)customer_size/(float)TILE_SIZE);
...
main_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_customer__c_custkey, customer_size, d_cycles_per_warp_main_1_join_build_0);
...
auto main_2_join_probe_0_cpw_size = std::ceil((float)orders_size/(float)TILE_SIZE);
...
//Create aggregation hash table
auto d_HT_3 = cuco::static_map{ (int)1500000*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_2<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_3.ref(cuco::insert), d_customer__c_custkey, d_orders__o_custkey, orders_size);
size_t COUNT3 = d_HT_3.size();
thrust::device_vector<int64_t> keys_3(COUNT3), vals_3(COUNT3);
d_HT_3.retrieve_all(keys_3.begin(), vals_3.begin());
d_HT_3.clear();
int64_t* raw_keys3 = thrust::raw_pointer_cast(keys_3.data());
insertKeys<<<std::ceil((float)COUNT3/128.), 128>>>(raw_keys3, d_HT_3.ref(cuco::insert), COUNT3);
int64_t* d_cycles_per_warp_main_2_aggregation_3;
auto main_2_aggregation_3_cpw_size = std::ceil((float)orders_size/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_2_aggregation_3, sizeof(int64_t) * main_2_aggregation_3_cpw_size);
cudaMemset(d_cycles_per_warp_main_2_aggregation_3, -1, sizeof(int64_t) * main_2_aggregation_3_cpw_size);
//Aggregate in hashtable
DBI64Type* d_aggr0__tmp_attr0;
cudaMalloc(&d_aggr0__tmp_attr0, sizeof(DBI64Type) * COUNT3);
cudaMemset(d_aggr0__tmp_attr0, 0, sizeof(DBI64Type) * COUNT3);
DBI32Type* d_KEY_3customer__c_custkey;
cudaMalloc(&d_KEY_3customer__c_custkey, sizeof(DBI32Type) * COUNT3);
cudaMemset(d_KEY_3customer__c_custkey, 0, sizeof(DBI32Type) * COUNT3);
main_2<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_3.ref(cuco::find), d_KEY_3customer__c_custkey, d_aggr0__tmp_attr0, d_customer__c_custkey, d_cycles_per_warp_main_2_aggregation_3, d_cycles_per_warp_main_2_join_probe_0, d_orders__o_custkey, d_orders__o_orderkey, orders_size);
...
std::cout << std::endl;
int64_t* cycles_per_warp_main_2_aggregation_3 = (int64_t*)malloc(sizeof(int64_t) * main_2_aggregation_3_cpw_size);
cudaMemcpy(cycles_per_warp_main_2_aggregation_3, d_cycles_per_warp_main_2_aggregation_3, sizeof(int64_t) * main_2_aggregation_3_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_2_aggregation_3 ";
for (auto i=0ull; i < main_2_aggregation_3_cpw_size; i++) std::cout << cycles_per_warp_main_2_aggregation_3[i] << " ";
std::cout << std::endl;
//Create aggregation hash table
...
count_5<<<std::ceil((float)COUNT3/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT3, d_HT_4.ref(cuco::insert), d_aggr0__tmp_attr0);
...
int64_t* d_cycles_per_warp_main_5_aggregation_4;
auto main_5_aggregation_4_cpw_size = std::ceil((float)COUNT3/(float)TILE_SIZE);
cudaMalloc(&d_cycles_per_warp_main_5_aggregation_4, sizeof(int64_t) * main_5_aggregation_4_cpw_size);
cudaMemset(d_cycles_per_warp_main_5_aggregation_4, -1, sizeof(int64_t) * main_5_aggregation_4_cpw_size);
//Aggregate in hashtable
...
main_5<<<std::ceil((float)COUNT3/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT3, d_HT_4.ref(cuco::find), d_KEY_4aggr0__tmp_attr0, d_aggr0__tmp_attr0, d_aggr1__tmp_attr1, d_cycles_per_warp_main_5_aggregation_4);
int64_t* cycles_per_warp_main_5_aggregation_4 = (int64_t*)malloc(sizeof(int64_t) * main_5_aggregation_4_cpw_size);
cudaMemcpy(cycles_per_warp_main_5_aggregation_4, d_cycles_per_warp_main_5_aggregation_4, sizeof(int64_t) * main_5_aggregation_4_cpw_size, cudaMemcpyDeviceToHost);
std::cout << "main_5_aggregation_4 ";
for (auto i=0ull; i < main_5_aggregation_4_cpw_size; i++) std::cout << cycles_per_warp_main_5_aggregation_4[i] << " ";
std::cout << std::endl;
//Materialize count
uint64_t* d_COUNT6;
...
auto main_7_materialize_6_cpw_size = std::ceil((float)COUNT4/(float)TILE_SIZE);
...
main_7<<<std::ceil((float)COUNT4/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT4, d_MAT6aggr0__tmp_attr0, d_MAT6aggr1__tmp_attr1, d_MAT_IDX6, d_KEY_4aggr0__tmp_attr0, d_aggr1__tmp_attr1, d_cycles_per_warp_main_7_materialize_6);
...
cudaFree(d_COUNT0);
cudaFree(d_KEY_3customer__c_custkey);
