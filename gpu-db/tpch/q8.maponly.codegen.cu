...
__global__ void count_5(uint64_t* COUNT4, DBDateType* orders__o_orderdate, size_t orders_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename HASHTABLE_INSERT_PK>
__global__ void main_5(uint64_t* BUF_4, uint64_t* BUF_IDX_4, HASHTABLE_INSERT_PK HT_4, DBDateType* orders__o_orderdate, DBI32Type* orders__o_orderkey, size_t orders_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= orders_size) return;
auto reg_orders__o_orderdate = orders__o_orderdate[tid];
if (!(evaluatePredicate(reg_orders__o_orderdate, 9131, Predicate::gte) && evaluatePredicate(reg_orders__o_orderdate, 9861, Predicate::lte))) return;
...
__global__ void count_7(uint64_t* COUNT6, size_t customer_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename HASHTABLE_INSERT_PK>
__global__ void main_7(uint64_t* BUF_6, uint64_t* BUF_IDX_6, HASHTABLE_INSERT_PK HT_6, DBI32Type* customer__c_custkey, size_t customer_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= customer_size) return;
uint64_t KEY_6 = 0;
...
__global__ void count_11(uint64_t* COUNT10, size_t supplier_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename HASHTABLE_INSERT_PK>
__global__ void main_11(uint64_t* BUF_10, uint64_t* BUF_IDX_10, HASHTABLE_INSERT_PK HT_10, DBI32Type* supplier__s_suppkey, size_t supplier_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= supplier_size) return;
uint64_t KEY_10 = 0;
...
__global__ void count_15(uint64_t* BUF_0, uint64_t* BUF_10, uint64_t* BUF_12, uint64_t* BUF_4, uint64_t* BUF_6, uint64_t* BUF_8, HASHTABLE_PROBE_PK HT_0, HASHTABLE_PROBE_PK HT_10, HASHTABLE_PROBE_PK HT_12, HASHTABLE_INSERT HT_14, HASHTABLE_PROBE_PK HT_4, HASHTABLE_PROBE_PK HT_6, HASHTABLE_PROBE_PK HT_8, DBI32Type* customer__c_nationkey, DBI32Type* lineitem__l_orderkey, DBI32Type* lineitem__l_partkey, DBI32Type* lineitem__l_suppkey, size_t lineitem_size, DBI32Type* orders__o_custkey, DBDateType* orders__o_orderdate, DBI32Type* supplier__s_nationkey) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
if (!(true)) return;
uint64_t KEY_6 = 0;
auto reg_orders__o_custkey = orders__o_custkey[BUF_4[SLOT_4->second * 1 + 0]];
...
if (!(true)) return;
uint64_t KEY_8 = 0;
auto reg_customer__c_nationkey = customer__c_nationkey[BUF_6[SLOT_6->second * 1 + 0]];
...
//Probe Hash table
auto SLOT_10 = HT_10.find(KEY_10);
if (SLOT_10 == HT_10.end()) return;
if (!(true)) return;
uint64_t KEY_12 = 0;
auto reg_supplier__s_nationkey = supplier__s_nationkey[BUF_10[SLOT_10->second * 1 + 0]];
...
if (!(true)) return;
auto reg_orders__o_orderdate = orders__o_orderdate[BUF_4[SLOT_4->second * 1 + 0]];
uint64_t KEY_14 = 0;
...
count_5<<<std::ceil((float)orders_size/128.), 128>>>(d_COUNT4, d_orders__o_orderdate, orders_size);
uint64_t COUNT4;
...
main_5<<<std::ceil((float)orders_size/128.), 128>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
//Materialize count
uint64_t* d_COUNT6;
cudaMalloc(&d_COUNT6, sizeof(uint64_t));
cudaMemset(d_COUNT6, 0, sizeof(uint64_t));
count_7<<<std::ceil((float)customer_size/128.), 128>>>(d_COUNT6, customer_size);
uint64_t COUNT6;
...
main_7<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_6, d_BUF_IDX_6, d_HT_6.ref(cuco::insert), d_customer__c_custkey, customer_size);
//Materialize count
uint64_t* d_COUNT8;
...
count_11<<<std::ceil((float)supplier_size/128.), 128>>>(d_COUNT10, supplier_size);
uint64_t COUNT10;
...
main_11<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_10, d_BUF_IDX_10, d_HT_10.ref(cuco::insert), d_supplier__s_suppkey, supplier_size);
//Materialize count
uint64_t* d_COUNT12;
...
count_15<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_0, d_BUF_10, d_BUF_12, d_BUF_4, d_BUF_6, d_BUF_8, d_HT_0.ref(cuco::find), d_HT_10.ref(cuco::find), d_HT_12.ref(cuco::find), d_HT_14.ref(cuco::insert), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_customer__c_nationkey, d_lineitem__l_orderkey, d_lineitem__l_partkey, d_lineitem__l_suppkey, lineitem_size, d_orders__o_custkey, d_orders__o_orderdate, d_supplier__s_nationkey);
...
main_15<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_0, d_BUF_10, d_BUF_12, d_BUF_4, d_BUF_6, d_BUF_8, d_HT_0.ref(cuco::find), d_HT_10.ref(cuco::find), d_HT_12.ref(cuco::find), d_HT_14.ref(cuco::find), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_14map0__tmp_attr0, d_aggr0__tmp_attr2, d_aggr0__tmp_attr4, d_customer__c_nationkey, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_partkey, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name, d_orders__o_custkey, d_orders__o_orderdate, d_supplier__s_nationkey);
//Materialize count
uint64_t* d_COUNT16;
