...
main_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_HT_0.ref(cuco::insert_and_find), d_SLOT_COUNT_0, d_aggr0__tmp_attr0, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_quantity, d_lineitem__l_shipdate, lineitem_size);
COUNT0 = d_HT_0.size();
...
for (auto i=0ull; i < MATCOUNT_2; i++) { std::cout << "" << MAT2aggr0__tmp_attr0[i];
std::cout << std::endl; }
