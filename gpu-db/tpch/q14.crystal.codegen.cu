...
__global__ void count_1(uint64_t* COUNT0, size_t part_size) {
...
__global__ void count_3(uint64_t* BUF_0, HASHTABLE_PROBE HT_0, HASHTABLE_INSERT HT_2, DBI32Type* lineitem__l_partkey, DBDateType* lineitem__l_shipdate, size_t lineitem_size, DBStringType* part__p_type) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_part__p_type[ITEM] = part__p_type[BUF_0[slot_second0[ITEM] * 1 + 0]];
}
...
MAT4map1__tmp_attr4[mat_idx4] = reg_map1__tmp_attr4[ITEM];
}
}
...
count_1<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT0, part_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_part__p_partkey, part_size);
//Create aggregation hash table
...
count_3<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_lineitem__l_partkey, d_lineitem__l_shipdate, lineitem_size, d_part__p_type);
...
main_3<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_aggr0__tmp_attr0, d_aggr0__tmp_attr2, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_partkey, d_lineitem__l_shipdate, lineitem_size, d_part__p_type);
//Materialize count
uint64_t* d_COUNT4;
...
main_5<<<std::ceil((float)COUNT2/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT2, d_MAT4map1__tmp_attr4, d_MAT_IDX4, d_aggr0__tmp_attr0, d_aggr0__tmp_attr2);
cudaMemcpy(MAT4map1__tmp_attr4, d_MAT4map1__tmp_attr4, sizeof(DBDecimalType) * COUNT4, cudaMemcpyDeviceToHost);
for (auto i=0ull; i < COUNT4; i++) { std::cout << "" << MAT4map1__tmp_attr4[i];
std::cout << std::endl; }
cudaFree(d_BUF_0);
