...
__global__ void main_1(uint64_t* BUF_0, uint64_t* BUF_IDX_0, HASHTABLE_INSERT HT_0, DBI32Type* orders__o_orderkey, size_t orders_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
if (!selection_flags[ITEM]) continue;
KEY_2[ITEM] = 0;
KEY_2[ITEM] |= reg_lineitem__l_shipmode_encoded[ITEM];
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_orders__o_orderpriority[ITEM] = orders__o_orderpriority[BUF_0[slot_second0[ITEM] * 1 + 0]];
}
...
aggregate_sum(&aggr0__tmp_attr2[buf_idx_2], reg_map0__tmp_attr3[ITEM]);
aggregate_sum(&aggr0__tmp_attr0[buf_idx_2], reg_map0__tmp_attr1[ITEM]);
KEY_2lineitem__l_shipmode_encoded[buf_idx_2] = reg_lineitem__l_shipmode_encoded[ITEM];
}
}
...
MAT4lineitem__l_shipmode_encoded[mat_idx4] = reg_lineitem__l_shipmode_encoded[ITEM];
MAT4aggr0__tmp_attr0[mat_idx4] = reg_aggr0__tmp_attr0[ITEM];
MAT4aggr0__tmp_attr2[mat_idx4] = reg_aggr0__tmp_attr2[ITEM];
}
}
...
count_1<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT0, orders_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_orders__o_orderkey, orders_size);
//Create aggregation hash table
auto d_HT_2 = cuco::static_map{ (int)17582*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_3<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_lineitem__l_commitdate, d_lineitem__l_orderkey, d_lineitem__l_receiptdate, d_lineitem__l_shipdate, d_lineitem__l_shipmode, d_lineitem__l_shipmode_encoded, lineitem_size);
...
main_5<<<std::ceil((float)COUNT2/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT2, d_MAT4aggr0__tmp_attr0, d_MAT4aggr0__tmp_attr2, d_MAT4lineitem__l_shipmode_encoded, d_MAT_IDX4, d_aggr0__tmp_attr0, d_aggr0__tmp_attr2, d_KEY_2lineitem__l_shipmode_encoded);
