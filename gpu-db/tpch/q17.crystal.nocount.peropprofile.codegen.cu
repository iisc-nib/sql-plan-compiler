...
BUF_3[(ITEM*TB + tid) * 1 + 0] = ITEM*TB + tid;
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_12 = get_aggregation_slot(KEY_12[ITEM], HT_12, SLOT_COUNT_12);
...
main_1<<<std::ceil((float)part_size/(float)TILE_SIZE), TB>>>(d_BUF_3, d_BUF_IDX_3, d_HT_3.ref(cuco::insert), d_cycles_per_warp_main_1_join_build_3, d_cycles_per_warp_main_1_selection_0, d_cycles_per_warp_main_1_selection_2, d_part__p_brand, d_part__p_container, d_part__p_partkey, part_size);
...
main_4<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_3, d_HT_3.ref(cuco::find), d_HT_5.ref(cuco::insert_and_find), d_KEY_5lineitem_u_1__l_partkey, d_SLOT_COUNT_5, d_aggr_rw__rw0, d_aggr_rw__rw1, d_cycles_per_warp_main_4_aggregation_5, d_cycles_per_warp_main_4_join_probe_3, lineitem_size, d_lineitem__l_partkey, d_lineitem__l_quantity, d_moved_aggr__p_partkey, d_part__p_partkey);
...
main_7<<<std::ceil((float)COUNT5/(float)TILE_SIZE), TB>>>(d_BUF_10, d_BUF_IDX_10, COUNT5, d_HT_10.ref(cuco::insert), d_cycles_per_warp_main_7_join_build_10, d_cycles_per_warp_main_7_map_6, d_cycles_per_warp_main_7_map_8, d_cycles_per_warp_main_7_selection_9, d_moved_aggr__p_partkey);
...
int* d_SLOT_COUNT_12;
cudaMalloc(&d_SLOT_COUNT_12, sizeof(int));
cudaMemset(d_SLOT_COUNT_12, 0, sizeof(int));
//Aggregate in hashtable
...
main_11<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_10, d_HT_10.ref(cuco::find), d_HT_12.ref(cuco::insert_and_find), d_SLOT_COUNT_12, d_aggr1__tmp_attr2, d_aggr_rw__rw0, d_aggr_rw__rw1, d_cycles_per_warp_main_11_aggregation_12, d_cycles_per_warp_main_11_join_probe_10, d_lineitem__l_extendedprice, d_lineitem__l_partkey, d_lineitem__l_quantity, lineitem_size);
...
main_14<<<std::ceil((float)COUNT12/(float)TILE_SIZE), TB>>>(COUNT12, d_MAT15map1__tmp_attr3, d_MAT_IDX15, d_aggr1__tmp_attr2, d_cycles_per_warp_main_14_map_13, d_cycles_per_warp_main_14_materialize_15);
