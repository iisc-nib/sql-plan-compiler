...
count_1<<<std::ceil((float)nation_size/32.), 32>>>(d_COUNT0, nation_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)nation_size/32.), 32>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_nation__n_nationkey, nation_size);
//Materialize count
uint64_t* d_COUNT2;
cudaMalloc(&d_COUNT2, sizeof(uint64_t));
cudaMemset(d_COUNT2, 0, sizeof(uint64_t));
count_3<<<std::ceil((float)supplier_size/32.), 32>>>(d_BUF_0, d_COUNT2, d_HT_0.ref(cuco::for_each), d_supplier__s_nationkey, supplier_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)supplier_size/32.), 32>>>(d_BUF_0, d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::for_each), d_HT_2.ref(cuco::insert), d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
//Materialize count
uint64_t* d_COUNT4;
cudaMalloc(&d_COUNT4, sizeof(uint64_t));
cudaMemset(d_COUNT4, 0, sizeof(uint64_t));
count_5<<<std::ceil((float)orders_size/32.), 32>>>(d_COUNT4, orders_size);
uint64_t COUNT4;
...
main_5<<<std::ceil((float)orders_size/32.), 32>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_orders__o_orderkey, orders_size);
//Materialize count
uint64_t* d_COUNT6;
cudaMalloc(&d_COUNT6, sizeof(uint64_t));
cudaMemset(d_COUNT6, 0, sizeof(uint64_t));
count_7<<<std::ceil((float)part_size/32.), 32>>>(d_COUNT6, part_size);
uint64_t COUNT6;
...
main_7<<<std::ceil((float)part_size/32.), 32>>>(d_BUF_6, d_BUF_IDX_6, d_HT_6.ref(cuco::insert), d_part__p_partkey, part_size);
//Materialize count
uint64_t* d_COUNT8;
cudaMalloc(&d_COUNT8, sizeof(uint64_t));
cudaMemset(d_COUNT8, 0, sizeof(uint64_t));
count_9<<<std::ceil((float)partsupp_size/32.), 32>>>(d_BUF_2, d_COUNT8, d_HT_2.ref(cuco::for_each), d_partsupp__ps_suppkey, partsupp_size);
uint64_t COUNT8;
...
main_9<<<std::ceil((float)partsupp_size/32.), 32>>>(d_BUF_2, d_BUF_8, d_BUF_IDX_8, d_HT_2.ref(cuco::for_each), d_HT_8.ref(cuco::insert), d_partsupp__ps_partkey, d_partsupp__ps_suppkey, partsupp_size, d_supplier__s_suppkey);
//Create aggregation hash table
...
count_11<<<std::ceil((float)lineitem_size/32.), 32>>>(d_BUF_4, d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::insert), d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::for_each), d_lineitem__l_orderkey, d_lineitem__l_partkey, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded, d_orders__o_orderdate);
...
main_11<<<std::ceil((float)lineitem_size/32.), 32>>>(d_BUF_4, d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::find), d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::for_each), d_KEY_10map0__tmp_attr0, d_KEY_10nation__n_name_encoded, d_aggr0__tmp_attr2, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_partkey, d_lineitem__l_quantity, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded, d_orders__o_orderdate, d_partsupp__ps_supplycost);
//Materialize count
uint64_t* d_COUNT12;
...
main_13<<<std::ceil((float)COUNT10/32.), 32>>>(COUNT10, d_MAT12aggr0__tmp_attr2, d_MAT12map0__tmp_attr0, d_MAT12nation__n_name_encoded, d_MAT_IDX12, d_aggr0__tmp_attr2, d_KEY_10map0__tmp_attr0, d_KEY_10nation__n_name_encoded);
