...
BUF_14[(ITEM*TB + tid) * 2 + 0] = BUF_12[slot_second12[ITEM] * 1 + 0];
BUF_14[(ITEM*TB + tid) * 2 + 1] = ITEM*TB + tid;
}
...
BUF_16[(ITEM*TB + tid) * 3 + 0] = ITEM*TB + tid;
BUF_16[(ITEM*TB + tid) * 3 + 1] = BUF_14[slot_second14[ITEM] * 2 + 0];
BUF_16[(ITEM*TB + tid) * 3 + 2] = BUF_14[slot_second14[ITEM] * 2 + 1];
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < supplier_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
HT_17.insert(cuco::pair{KEY_17[ITEM], ITEM*TB + tid});
BUF_17[(ITEM*TB + tid) * 2 + 0] = ITEM*TB + tid;
BUF_17[(ITEM*TB + tid) * 2 + 1] = BUF_6[slot_second6[ITEM] * 1 + 0];
}
...
main_3<<<std::ceil((float)nation_size/(float)TILE_SIZE), TB>>>(d_BUF_6, d_BUF_IDX_6, d_HT_6.ref(cuco::insert), d_cycles_per_warp_main_3_join_build_6, d_cycles_per_warp_main_3_selection_2, d_cycles_per_warp_main_3_selection_4, d_cycles_per_warp_main_3_selection_5, d_nation__n_name, d_nation__n_nationkey, nation_size);
...
main_9<<<std::ceil((float)nation_size/(float)TILE_SIZE), TB>>>(d_BUF_12, d_BUF_IDX_12, d_HT_12.ref(cuco::insert), d_cycles_per_warp_main_9_join_build_12, d_cycles_per_warp_main_9_selection_10, d_cycles_per_warp_main_9_selection_11, d_cycles_per_warp_main_9_selection_8, d_nation__n_name, d_nation__n_nationkey, nation_size);
...
main_13<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_12, d_BUF_14, d_BUF_IDX_14, d_HT_12.ref(cuco::find), d_HT_14.ref(cuco::insert), d_customer__c_custkey, d_customer__c_nationkey, customer_size, d_cycles_per_warp_main_13_join_build_14, d_cycles_per_warp_main_13_join_probe_12);
...
main_15<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_14, d_BUF_16, d_BUF_IDX_16, d_HT_14.ref(cuco::find), d_HT_16.ref(cuco::insert), d_cycles_per_warp_main_15_join_build_16, d_cycles_per_warp_main_15_join_probe_14, d_orders__o_custkey, d_orders__o_orderkey, orders_size);
...
main_7<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TB>>>(d_BUF_17, d_BUF_6, d_BUF_IDX_17, d_HT_17.ref(cuco::insert), d_HT_6.ref(cuco::find), d_cycles_per_warp_main_7_join_build_17, d_cycles_per_warp_main_7_join_probe_6, d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
...
main_22<<<std::ceil((float)COUNT20/(float)TILE_SIZE), TB>>>(COUNT20, d_MAT21aggr0__tmp_attr2, d_MAT21map0__tmp_attr0, d_MAT21n1___n_name_encoded, d_MAT21n2___n_name_encoded, d_MAT_IDX21, d_aggr0__tmp_attr2, d_cycles_per_warp_main_22_materialize_21, d_KEY_20map0__tmp_attr0, d_KEY_20n1___n_name_encoded, d_KEY_20n2___n_name_encoded);
