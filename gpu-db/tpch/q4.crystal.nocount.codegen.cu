...
main_3<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert_and_find), d_KEY_2orders__o_orderpriority_encoded, d_SLOT_COUNT_2, d_aggr0__tmp_attr0, d_orders__o_orderdate, d_orders__o_orderkey, d_orders__o_orderpriority_encoded, orders_size);
COUNT2 = d_HT_2.size();
