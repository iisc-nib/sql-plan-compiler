...
__global__ void main_7(uint64_t* BUF_2, uint64_t* BUF_4, HASHTABLE_PROBE HT_2, HASHTABLE_PROBE HT_4, HASHTABLE_FIND HT_6, DBI32Type* KEY_6customer__c_custkey, DBDecimalType* aggr0__tmp_attr0, DBDecimalType* aggr__c_acctbal, DBStringType* aggr__c_address, DBStringType* aggr__c_comment, DBStringType* aggr__c_name, DBStringType* aggr__c_phone, DBStringType* aggr__n_name, DBDecimalType* customer__c_acctbal, DBStringType* customer__c_address, DBStringType* customer__c_comment, DBI32Type* customer__c_custkey, DBStringType* customer__c_name, DBI32Type* customer__c_nationkey, DBStringType* customer__c_phone, size_t customer_size, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBStringType* nation__n_name) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= customer_size) return;
uint64_t KEY_2 = 0;
auto reg_customer__c_custkey = customer__c_custkey[tid];
...
auto reg_lineitem__l_discount = lineitem__l_discount[BUF_2[SLOT_2->second * 2 + 1]];
auto reg_lineitem__l_extendedprice = lineitem__l_extendedprice[BUF_2[SLOT_2->second * 2 + 1]];
...
aggregate_any(&aggr__c_comment[buf_idx_6], reg_customer__c_comment);
auto reg_customer__c_name = customer__c_name[tid];
aggregate_any(&aggr__c_name[buf_idx_6], reg_customer__c_name);
...
aggregate_any(&aggr__c_phone[buf_idx_6], reg_customer__c_phone);
auto reg_nation__n_name = nation__n_name[BUF_4[SLOT_4->second * 1 + 0]];
aggregate_any(&aggr__n_name[buf_idx_6], reg_nation__n_name);
auto reg_customer__c_address = customer__c_address[tid];
aggregate_any(&aggr__c_address[buf_idx_6], reg_customer__c_address);
...
count_7<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_2, d_BUF_4, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_customer__c_custkey, d_customer__c_nationkey, customer_size);
...
cudaMalloc(&d_aggr__c_comment, sizeof(DBStringType) * COUNT6);
cudaMemset(d_aggr__c_comment, 0, sizeof(DBStringType) * COUNT6);
DBStringType* d_aggr__c_name;
cudaMalloc(&d_aggr__c_name, sizeof(DBStringType) * COUNT6);
cudaMemset(d_aggr__c_name, 0, sizeof(DBStringType) * COUNT6);
...
cudaMalloc(&d_aggr__c_phone, sizeof(DBStringType) * COUNT6);
cudaMemset(d_aggr__c_phone, 0, sizeof(DBStringType) * COUNT6);
DBStringType* d_aggr__n_name;
cudaMalloc(&d_aggr__n_name, sizeof(DBStringType) * COUNT6);
cudaMemset(d_aggr__n_name, 0, sizeof(DBStringType) * COUNT6);
DBStringType* d_aggr__c_address;
cudaMalloc(&d_aggr__c_address, sizeof(DBStringType) * COUNT6);
cudaMemset(d_aggr__c_address, 0, sizeof(DBStringType) * COUNT6);
...
main_7<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_2, d_BUF_4, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_KEY_6customer__c_custkey, d_aggr0__tmp_attr0, d_aggr__c_acctbal, d_aggr__c_address, d_aggr__c_comment, d_aggr__c_name, d_aggr__c_phone, d_aggr__n_name, d_customer__c_acctbal, d_customer__c_address, d_customer__c_comment, d_customer__c_custkey, d_customer__c_name, d_customer__c_nationkey, d_customer__c_phone, customer_size, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_nation__n_name);
//Materialize count
uint64_t* d_COUNT8;
