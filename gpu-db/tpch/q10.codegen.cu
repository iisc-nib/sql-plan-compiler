...
__global__ void count_3(uint64_t* BUF_0, uint64_t* COUNT2, HASHTABLE_PROBE HT_0, DBI32Type* lineitem__l_orderkey, DBCharType* lineitem__l_returnflag, size_t lineitem_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void main_3(uint64_t* BUF_0, uint64_t* BUF_2, uint64_t* BUF_IDX_2, HASHTABLE_PROBE HT_0, HASHTABLE_INSERT HT_2, DBI32Type* lineitem__l_orderkey, DBCharType* lineitem__l_returnflag, size_t lineitem_size, DBI32Type* orders__o_custkey) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineitem_size) return;
auto reg_lineitem__l_returnflag = lineitem__l_returnflag[tid];
if (!(evaluatePredicate(reg_lineitem__l_returnflag, 'R', Predicate::eq))) return;
...
if (!(true)) return;
uint64_t KEY_2 = 0;
auto reg_orders__o_custkey = orders__o_custkey[BUF_0[slot_second0 * 1 + 0]];
...
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
__global__ void count_7(uint64_t* BUF_2, uint64_t* BUF_4, HASHTABLE_PROBE HT_2, HASHTABLE_PROBE HT_4, HASHTABLE_INSERT HT_6, DBI32Type* customer__c_custkey, DBI32Type* customer__c_nationkey, size_t customer_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void main_7(uint64_t* BUF_2, uint64_t* BUF_4, HASHTABLE_PROBE HT_2, HASHTABLE_PROBE HT_4, HASHTABLE_FIND HT_6, DBI32Type* KEY_6customer__c_custkey, DBDecimalType* aggr0__tmp_attr0, DBDecimalType* aggr__c_acctbal, DBI16Type* aggr__c_address_encoded, DBI16Type* aggr__c_comment_encoded, DBI16Type* aggr__c_name_encoded, DBI16Type* aggr__c_phone_encoded, DBI16Type* aggr__n_name_encoded, DBDecimalType* customer__c_acctbal, DBI16Type* customer__c_address_encoded, DBI16Type* customer__c_comment_encoded, DBI32Type* customer__c_custkey, DBI16Type* customer__c_name_encoded, DBI32Type* customer__c_nationkey, DBI16Type* customer__c_phone_encoded, size_t customer_size, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBI16Type* nation__n_name_encoded) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= customer_size) return;
uint64_t KEY_2 = 0;
auto reg_customer__c_custkey = customer__c_custkey[tid];
...
auto reg_customer__c_comment_encoded = customer__c_comment_encoded[tid];
aggregate_any(&aggr__c_comment_encoded[buf_idx_6], reg_customer__c_comment_encoded);
auto reg_customer__c_name_encoded = customer__c_name_encoded[tid];
aggregate_any(&aggr__c_name_encoded[buf_idx_6], reg_customer__c_name_encoded);
auto reg_customer__c_acctbal = customer__c_acctbal[tid];
aggregate_any(&aggr__c_acctbal[buf_idx_6], reg_customer__c_acctbal);
auto reg_customer__c_phone_encoded = customer__c_phone_encoded[tid];
aggregate_any(&aggr__c_phone_encoded[buf_idx_6], reg_customer__c_phone_encoded);
auto reg_nation__n_name_encoded = nation__n_name_encoded[BUF_4[slot_second4 * 1 + 0]];
aggregate_any(&aggr__n_name_encoded[buf_idx_6], reg_nation__n_name_encoded);
auto reg_customer__c_address_encoded = customer__c_address_encoded[tid];
aggregate_any(&aggr__c_address_encoded[buf_idx_6], reg_customer__c_address_encoded);
...
__global__ void main_9(size_t COUNT6, DBDecimalType* MAT8aggr0__tmp_attr0, DBDecimalType* MAT8aggr__c_acctbal, DBI16Type* MAT8aggr__c_address_encoded, DBI16Type* MAT8aggr__c_comment_encoded, DBI16Type* MAT8aggr__c_name_encoded, DBI16Type* MAT8aggr__c_phone_encoded, DBI16Type* MAT8aggr__n_name_encoded, DBI32Type* MAT8customer__c_custkey, uint64_t* MAT_IDX8, DBDecimalType* aggr0__tmp_attr0, DBDecimalType* aggr__c_acctbal, DBI16Type* aggr__c_address_encoded, DBI16Type* aggr__c_comment_encoded, DBI16Type* aggr__c_name_encoded, DBI16Type* aggr__c_phone_encoded, DBI16Type* aggr__n_name_encoded, DBI32Type* customer__c_custkey) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= COUNT6) return;
//Materialize buffers
...
auto reg_aggr__c_name_encoded = aggr__c_name_encoded[tid];
MAT8aggr__c_name_encoded[mat_idx8] = reg_aggr__c_name_encoded;
...
auto reg_aggr__c_address_encoded = aggr__c_address_encoded[tid];
MAT8aggr__c_address_encoded[mat_idx8] = reg_aggr__c_address_encoded;
auto reg_aggr__c_phone_encoded = aggr__c_phone_encoded[tid];
MAT8aggr__c_phone_encoded[mat_idx8] = reg_aggr__c_phone_encoded;
auto reg_aggr__c_comment_encoded = aggr__c_comment_encoded[tid];
MAT8aggr__c_comment_encoded[mat_idx8] = reg_aggr__c_comment_encoded;
}
...
count_3<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_0, d_COUNT2, d_HT_0.ref(cuco::for_each), d_lineitem__l_orderkey, d_lineitem__l_returnflag, lineitem_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::for_each), d_HT_2.ref(cuco::insert), d_lineitem__l_orderkey, d_lineitem__l_returnflag, lineitem_size, d_orders__o_custkey);
//Materialize count
uint64_t* d_COUNT4;
...
count_7<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_2, d_BUF_4, d_HT_2.ref(cuco::for_each), d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::insert), d_customer__c_custkey, d_customer__c_nationkey, customer_size);
...
DBI16Type* d_aggr__c_comment_encoded;
cudaMalloc(&d_aggr__c_comment_encoded, sizeof(DBI16Type) * COUNT6);
cudaMemset(d_aggr__c_comment_encoded, 0, sizeof(DBI16Type) * COUNT6);
auto aggr__c_comment_map = customer__c_comment_map;
DBI16Type* d_aggr__c_name_encoded;
cudaMalloc(&d_aggr__c_name_encoded, sizeof(DBI16Type) * COUNT6);
cudaMemset(d_aggr__c_name_encoded, 0, sizeof(DBI16Type) * COUNT6);
auto aggr__c_name_map = customer__c_name_map;
...
DBI16Type* d_aggr__c_phone_encoded;
cudaMalloc(&d_aggr__c_phone_encoded, sizeof(DBI16Type) * COUNT6);
cudaMemset(d_aggr__c_phone_encoded, 0, sizeof(DBI16Type) * COUNT6);
auto aggr__c_phone_map = customer__c_phone_map;
...
DBI16Type* d_aggr__c_address_encoded;
cudaMalloc(&d_aggr__c_address_encoded, sizeof(DBI16Type) * COUNT6);
cudaMemset(d_aggr__c_address_encoded, 0, sizeof(DBI16Type) * COUNT6);
auto aggr__c_address_map = customer__c_address_map;
...
main_7<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_2, d_BUF_4, d_HT_2.ref(cuco::for_each), d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::find), d_KEY_6customer__c_custkey, d_aggr0__tmp_attr0, d_aggr__c_acctbal, d_aggr__c_address_encoded, d_aggr__c_comment_encoded, d_aggr__c_name_encoded, d_aggr__c_phone_encoded, d_aggr__n_name_encoded, d_customer__c_acctbal, d_customer__c_address_encoded, d_customer__c_comment_encoded, d_customer__c_custkey, d_customer__c_name_encoded, d_customer__c_nationkey, d_customer__c_phone_encoded, customer_size, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_nation__n_name_encoded);
//Materialize count
uint64_t* d_COUNT8;
...
auto MAT8aggr__c_name_encoded = (DBI16Type*)malloc(sizeof(DBI16Type) * COUNT8);
DBI16Type* d_MAT8aggr__c_name_encoded;
cudaMalloc(&d_MAT8aggr__c_name_encoded, sizeof(DBI16Type) * COUNT8);
...
auto MAT8aggr__c_address_encoded = (DBI16Type*)malloc(sizeof(DBI16Type) * COUNT8);
DBI16Type* d_MAT8aggr__c_address_encoded;
cudaMalloc(&d_MAT8aggr__c_address_encoded, sizeof(DBI16Type) * COUNT8);
auto MAT8aggr__c_phone_encoded = (DBI16Type*)malloc(sizeof(DBI16Type) * COUNT8);
DBI16Type* d_MAT8aggr__c_phone_encoded;
cudaMalloc(&d_MAT8aggr__c_phone_encoded, sizeof(DBI16Type) * COUNT8);
auto MAT8aggr__c_comment_encoded = (DBI16Type*)malloc(sizeof(DBI16Type) * COUNT8);
DBI16Type* d_MAT8aggr__c_comment_encoded;
cudaMalloc(&d_MAT8aggr__c_comment_encoded, sizeof(DBI16Type) * COUNT8);
main_9<<<std::ceil((float)COUNT6/128.), 128>>>(COUNT6, d_MAT8aggr0__tmp_attr0, d_MAT8aggr__c_acctbal, d_MAT8aggr__c_address_encoded, d_MAT8aggr__c_comment_encoded, d_MAT8aggr__c_name_encoded, d_MAT8aggr__c_phone_encoded, d_MAT8aggr__n_name_encoded, d_MAT8customer__c_custkey, d_MAT_IDX8, d_aggr0__tmp_attr0, d_aggr__c_acctbal, d_aggr__c_address_encoded, d_aggr__c_comment_encoded, d_aggr__c_name_encoded, d_aggr__c_phone_encoded, d_aggr__n_name_encoded, d_KEY_6customer__c_custkey);
cudaMemcpy(MAT8customer__c_custkey, d_MAT8customer__c_custkey, sizeof(DBI32Type) * COUNT8, cudaMemcpyDeviceToHost);
cudaMemcpy(MAT8aggr__c_name_encoded, d_MAT8aggr__c_name_encoded, sizeof(DBI16Type) * COUNT8, cudaMemcpyDeviceToHost);
...
cudaMemcpy(MAT8aggr__c_address_encoded, d_MAT8aggr__c_address_encoded, sizeof(DBI16Type) * COUNT8, cudaMemcpyDeviceToHost);
cudaMemcpy(MAT8aggr__c_phone_encoded, d_MAT8aggr__c_phone_encoded, sizeof(DBI16Type) * COUNT8, cudaMemcpyDeviceToHost);
cudaMemcpy(MAT8aggr__c_comment_encoded, d_MAT8aggr__c_comment_encoded, sizeof(DBI16Type) * COUNT8, cudaMemcpyDeviceToHost);
for (auto i=0ull; i < COUNT8; i++) { std::cout << "" << MAT8customer__c_custkey[i];
std::cout << "|" << aggr__c_name_map[MAT8aggr__c_name_encoded[i]];
std::cout << "|" << MAT8aggr0__tmp_attr0[i];
std::cout << "|" << MAT8aggr__c_acctbal[i];
std::cout << "|" << aggr__n_name_map[MAT8aggr__n_name_encoded[i]];
std::cout << "|" << aggr__c_address_map[MAT8aggr__c_address_encoded[i]];
std::cout << "|" << aggr__c_phone_map[MAT8aggr__c_phone_encoded[i]];
std::cout << "|" << aggr__c_comment_map[MAT8aggr__c_comment_encoded[i]];
std::cout << std::endl; }
cudaFree(d_BUF_0);
...
cudaFree(d_aggr__c_address_encoded);
cudaFree(d_aggr__c_comment_encoded);
cudaFree(d_aggr__c_name_encoded);
cudaFree(d_aggr__c_phone_encoded);
cudaFree(d_aggr__n_name_encoded);
cudaFree(d_COUNT8);
cudaFree(d_MAT8aggr0__tmp_attr0);
cudaFree(d_MAT8aggr__c_acctbal);
cudaFree(d_MAT8aggr__c_address_encoded);
cudaFree(d_MAT8aggr__c_comment_encoded);
cudaFree(d_MAT8aggr__c_name_encoded);
cudaFree(d_MAT8aggr__c_phone_encoded);
cudaFree(d_MAT8aggr__n_name_encoded);
cudaFree(d_MAT8customer__c_custkey);
cudaFree(d_MAT_IDX8);
free(MAT8aggr0__tmp_attr0);
free(MAT8aggr__c_acctbal);
free(MAT8aggr__c_address_encoded);
free(MAT8aggr__c_comment_encoded);
free(MAT8aggr__c_name_encoded);
free(MAT8aggr__c_phone_encoded);
