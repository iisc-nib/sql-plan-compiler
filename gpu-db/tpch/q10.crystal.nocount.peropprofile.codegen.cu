...
BUF_7[(ITEM*TB + tid) * 1 + 0] = ITEM*TB + tid;
}
...
main_1<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_cycles_per_warp_main_1_join_build_4, d_cycles_per_warp_main_1_selection_0, d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
...
main_6<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_5, d_BUF_IDX_5, d_HT_5.ref(cuco::insert), d_customer__c_custkey, customer_size, d_cycles_per_warp_main_6_join_build_5);
...
main_8<<<std::ceil((float)nation_size/(float)TILE_SIZE), TB>>>(d_BUF_7, d_BUF_IDX_7, d_HT_7.ref(cuco::insert), d_cycles_per_warp_main_8_join_build_7, d_nation__n_nationkey, nation_size);
...
main_12<<<std::ceil((float)COUNT10/(float)TILE_SIZE), TB>>>(COUNT10, d_MAT11aggr0__tmp_attr0, d_MAT11aggr__c_acctbal, d_MAT11aggr__n_name_encoded, d_MAT11customer__c_custkey, d_MAT_IDX11, d_aggr0__tmp_attr0, d_aggr__c_acctbal, d_aggr__n_name_encoded, d_KEY_10customer__c_custkey, d_cycles_per_warp_main_12_materialize_11);
