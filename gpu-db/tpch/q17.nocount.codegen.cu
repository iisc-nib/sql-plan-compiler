...
template<typename HASHTABLE_PROBE_PK, typename HASHTABLE_FIND>
__global__ void main_7(uint64_t* BUF_4, HASHTABLE_PROBE_PK HT_4, HASHTABLE_FIND HT_6, int* SLOT_COUNT_6, DBDecimalType* aggr1__tmp_attr2, DBDecimalType* aggr_rw__rw0, DBI64Type* aggr_rw__rw1, DBDecimalType* lineitem__l_extendedprice, DBI32Type* lineitem__l_partkey, DBDecimalType* lineitem__l_quantity, size_t lineitem_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineitem_size) return;
uint64_t KEY_4 = 0;
...
main_3<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert_and_find), d_KEY_2lineitem_u_1__l_partkey, d_SLOT_COUNT_2, d_aggr_rw__rw0, d_aggr_rw__rw1, lineitem_size, d_lineitem__l_partkey, d_lineitem__l_quantity, d_moved_aggr__p_partkey, d_part__p_partkey);
COUNT2 = d_HT_2.size();
...
main_7<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert_and_find), d_SLOT_COUNT_6, d_aggr1__tmp_attr2, d_aggr_rw__rw0, d_aggr_rw__rw1, d_lineitem__l_extendedprice, d_lineitem__l_partkey, d_lineitem__l_quantity, lineitem_size);
COUNT6 = d_HT_6.size();
...
for (auto i=0ull; i < MATCOUNT_8; i++) { std::cout << "" << MAT8map1__tmp_attr3[i];
std::cout << std::endl; }
