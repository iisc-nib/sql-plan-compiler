...
count_1<<<std::ceil((float)region_size/32.), 32>>>(d_COUNT0, d_region__r_name, region_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)region_size/32.), 32>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_region__r_name, d_region__r_regionkey, region_size);
//Materialize count
uint64_t* d_COUNT2;
cudaMalloc(&d_COUNT2, sizeof(uint64_t));
cudaMemset(d_COUNT2, 0, sizeof(uint64_t));
count_3<<<std::ceil((float)nation_size/32.), 32>>>(d_BUF_0, d_COUNT2, d_HT_0.ref(cuco::for_each), d_nation__n_regionkey, nation_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)nation_size/32.), 32>>>(d_BUF_0, d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::for_each), d_HT_2.ref(cuco::insert), d_nation__n_nationkey, d_nation__n_regionkey, nation_size);
//Materialize count
uint64_t* d_COUNT4;
...
count_7<<<std::ceil((float)orders_size/32.), 32>>>(d_BUF_4, d_COUNT6, d_HT_4.ref(cuco::for_each), d_orders__o_custkey, d_orders__o_orderdate, orders_size);
uint64_t COUNT6;
...
main_7<<<std::ceil((float)orders_size/32.), 32>>>(d_BUF_4, d_BUF_6, d_BUF_IDX_6, d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::insert), d_orders__o_custkey, d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
//Materialize count
uint64_t* d_COUNT8;
cudaMalloc(&d_COUNT8, sizeof(uint64_t));
cudaMemset(d_COUNT8, 0, sizeof(uint64_t));
count_9<<<std::ceil((float)supplier_size/32.), 32>>>(d_COUNT8, supplier_size);
uint64_t COUNT8;
...
main_9<<<std::ceil((float)supplier_size/32.), 32>>>(d_BUF_8, d_BUF_IDX_8, d_HT_8.ref(cuco::insert), d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
//Create aggregation hash table
auto d_HT_10 = cuco::static_map{ (int)23003*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_11<<<std::ceil((float)lineitem_size/32.), 32>>>(d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::insert), d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::for_each), d_customer__c_nationkey, d_lineitem__l_orderkey, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded);
...
main_13<<<std::ceil((float)COUNT10/32.), 32>>>(COUNT10, d_MAT12aggr0__tmp_attr0, d_MAT12nation__n_name_encoded, d_MAT_IDX12, d_aggr0__tmp_attr0, d_KEY_10nation__n_name_encoded);
