...
template<typename HASHTABLE_PROBE_SJ, typename HASHTABLE_PROBE>
__global__ void count_7(uint64_t* BUF_4, uint64_t* COUNT6, HASHTABLE_PROBE_SJ HT_2, HASHTABLE_PROBE HT_4, DBI32Type* orders__o_custkey, DBI32Type* orders__o_orderkey, size_t orders_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
BUF_6[buf_idx_6 * 2 + 0] = BUF_4[slot_second4 * 1 + 0];
BUF_6[buf_idx_6 * 2 + 1] = tid;
});
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
__global__ void count_9(uint64_t* BUF_6, HASHTABLE_PROBE HT_6, HASHTABLE_INSERT HT_8, DBI32Type* lineitem__l_orderkey, size_t lineitem_size, DBI32Type* orders__o_orderkey) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineitem_size) return;
uint64_t KEY_6 = 0;
...
if (!(true)) return;
uint64_t KEY_8 = 0;
auto reg_orders__o_orderkey = orders__o_orderkey[BUF_6[slot_second6 * 2 + 1]];
...
__global__ void main_9(uint64_t* BUF_6, HASHTABLE_PROBE HT_6, HASHTABLE_FIND HT_8, DBI32Type* KEY_8orders__o_orderkey, DBDecimalType* aggr1__tmp_attr1, DBI32Type* aggr__c_custkey, DBI16Type* aggr__c_name_encoded, DBDateType* aggr__o_orderdate, DBDecimalType* aggr__o_totalprice, DBI32Type* customer__c_custkey, DBI16Type* customer__c_name_encoded, DBI32Type* lineitem__l_orderkey, DBDecimalType* lineitem__l_quantity, size_t lineitem_size, DBDateType* orders__o_orderdate, DBI32Type* orders__o_orderkey, DBDecimalType* orders__o_totalprice) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineitem_size) return;
uint64_t KEY_6 = 0;
auto reg_lineitem__l_orderkey = lineitem__l_orderkey[tid];
...
auto reg_customer__c_name_encoded = customer__c_name_encoded[BUF_6[slot_second6 * 2 + 0]];
aggregate_any(&aggr__c_name_encoded[buf_idx_8], reg_customer__c_name_encoded);
auto reg_customer__c_custkey = customer__c_custkey[BUF_6[slot_second6 * 2 + 0]];
aggregate_any(&aggr__c_custkey[buf_idx_8], reg_customer__c_custkey);
auto reg_orders__o_totalprice = orders__o_totalprice[BUF_6[slot_second6 * 2 + 1]];
aggregate_any(&aggr__o_totalprice[buf_idx_8], reg_orders__o_totalprice);
auto reg_orders__o_orderdate = orders__o_orderdate[BUF_6[slot_second6 * 2 + 1]];
...
__global__ void main_11(size_t COUNT8, DBDecimalType* MAT10aggr1__tmp_attr1, DBI32Type* MAT10aggr__c_custkey, DBI16Type* MAT10aggr__c_name_encoded, DBDateType* MAT10aggr__o_orderdate, DBDecimalType* MAT10aggr__o_totalprice, DBI32Type* MAT10orders__o_orderkey, uint64_t* MAT_IDX10, DBDecimalType* aggr1__tmp_attr1, DBI32Type* aggr__c_custkey, DBI16Type* aggr__c_name_encoded, DBDateType* aggr__o_orderdate, DBDecimalType* aggr__o_totalprice, DBI32Type* orders__o_orderkey) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= COUNT8) return;
//Materialize buffers
auto mat_idx10 = atomicAdd((int*)MAT_IDX10, 1);
auto reg_aggr__c_name_encoded = aggr__c_name_encoded[tid];
MAT10aggr__c_name_encoded[mat_idx10] = reg_aggr__c_name_encoded;
...
count_7<<<std::ceil((float)orders_size/128.), 128>>>(d_BUF_4, d_COUNT6, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::for_each), d_orders__o_custkey, d_orders__o_orderkey, orders_size);
uint64_t COUNT6;
...
main_7<<<std::ceil((float)orders_size/128.), 128>>>(d_BUF_4, d_BUF_6, d_BUF_IDX_6, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::for_each), d_HT_6.ref(cuco::insert), d_orders__o_custkey, d_orders__o_orderkey, orders_size);
//Create aggregation hash table
...
count_9<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_6, d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::insert), d_lineitem__l_orderkey, lineitem_size, d_orders__o_orderkey);
...
cudaMalloc(&d_aggr__c_name_encoded, sizeof(DBI16Type) * COUNT8);
cudaMemset(d_aggr__c_name_encoded, 0, sizeof(DBI16Type) * COUNT8);
...
main_9<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_6, d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::find), d_KEY_8orders__o_orderkey, d_aggr1__tmp_attr1, d_aggr__c_custkey, d_aggr__c_name_encoded, d_aggr__o_orderdate, d_aggr__o_totalprice, d_customer__c_custkey, d_customer__c_name_encoded, d_lineitem__l_orderkey, d_lineitem__l_quantity, lineitem_size, d_orders__o_orderdate, d_orders__o_orderkey, d_orders__o_totalprice);
//Materialize count
uint64_t* d_COUNT10;
...
auto MAT10aggr__c_name_encoded = (DBI16Type*)malloc(sizeof(DBI16Type) * COUNT10);
DBI16Type* d_MAT10aggr__c_name_encoded;
cudaMalloc(&d_MAT10aggr__c_name_encoded, sizeof(DBI16Type) * COUNT10);
...
main_11<<<std::ceil((float)COUNT8/128.), 128>>>(COUNT8, d_MAT10aggr1__tmp_attr1, d_MAT10aggr__c_custkey, d_MAT10aggr__c_name_encoded, d_MAT10aggr__o_orderdate, d_MAT10aggr__o_totalprice, d_MAT10orders__o_orderkey, d_MAT_IDX10, d_aggr1__tmp_attr1, d_aggr__c_custkey, d_aggr__c_name_encoded, d_aggr__o_orderdate, d_aggr__o_totalprice, d_KEY_8orders__o_orderkey);
cudaMemcpy(MAT10aggr__c_name_encoded, d_MAT10aggr__c_name_encoded, sizeof(DBI16Type) * COUNT10, cudaMemcpyDeviceToHost);
...
for (auto i=0ull; i < COUNT10; i++) { std::cout << "" << aggr__c_name_map[MAT10aggr__c_name_encoded[i]];
std::cout << "|" << MAT10aggr__c_custkey[i];
...
cudaFree(d_MAT10aggr__c_name_encoded);
...
free(MAT10aggr__c_name_encoded);
