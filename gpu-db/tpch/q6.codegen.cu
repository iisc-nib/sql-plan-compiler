...
count_1<<<std::ceil((float)lineitem_size/32.), 32>>>(d_HT_0.ref(cuco::insert), d_lineitem__l_discount, d_lineitem__l_quantity, d_lineitem__l_shipdate, lineitem_size);
...
main_1<<<std::ceil((float)lineitem_size/32.), 32>>>(d_HT_0.ref(cuco::find), d_aggr0__tmp_attr0, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_quantity, d_lineitem__l_shipdate, lineitem_size);
//Materialize count
uint64_t* d_COUNT2;
...
main_3<<<std::ceil((float)COUNT0/32.), 32>>>(COUNT0, d_MAT2aggr0__tmp_attr0, d_MAT_IDX2, d_aggr0__tmp_attr0);
