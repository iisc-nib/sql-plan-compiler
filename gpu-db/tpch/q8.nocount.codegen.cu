...
template<typename HASHTABLE_PROBE_PK, typename HASHTABLE_INSERT>
__global__ void main_9(uint64_t* BUF_2, uint64_t* BUF_8, uint64_t* BUF_IDX_8, HASHTABLE_PROBE_PK HT_2, HASHTABLE_INSERT HT_8, DBI32Type* n1___n_nationkey, DBI32Type* n1___n_regionkey, size_t nation_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
auto buf_idx_14 = get_aggregation_slot(KEY_14, HT_14, SLOT_COUNT_14);
...
main_15<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_10, d_BUF_12, d_HT_10.ref(cuco::for_each), d_HT_12.ref(cuco::find), d_HT_14.ref(cuco::insert_and_find), d_KEY_14map0__tmp_attr0, d_SLOT_COUNT_14, d_aggr0__tmp_attr2, d_aggr0__tmp_attr4, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_nation__n_name, d_orders__o_orderdate, d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
