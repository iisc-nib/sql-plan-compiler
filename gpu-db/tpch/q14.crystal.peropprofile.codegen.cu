...
__global__ void count_3(uint64_t* COUNT2, size_t part_size) {
...
__global__ void count_1(uint64_t* BUF_2, HASHTABLE_PROBE HT_2, HASHTABLE_INSERT HT_6, DBI32Type* lineitem__l_partkey, DBDateType* lineitem__l_shipdate, size_t lineitem_size, DBStringType* part__p_type) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
__global__ void count_8(size_t COUNT6, uint64_t* COUNT9) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
count_3<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT2, part_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_2, d_BUF_IDX_2, d_HT_2.ref(cuco::insert), d_cycles_per_warp_main_3_join_build_2, d_part__p_partkey, part_size);
...
count_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_2, d_HT_2.ref(cuco::find), d_HT_6.ref(cuco::insert), d_lineitem__l_partkey, d_lineitem__l_shipdate, lineitem_size, d_part__p_type);
...
main_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_2, d_HT_2.ref(cuco::find), d_HT_6.ref(cuco::find), d_aggr0__tmp_attr0, d_aggr0__tmp_attr2, d_cycles_per_warp_main_1_aggregation_6, d_cycles_per_warp_main_1_join_probe_2, d_cycles_per_warp_main_1_map_4, d_cycles_per_warp_main_1_map_5, d_cycles_per_warp_main_1_selection_0, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_partkey, d_lineitem__l_shipdate, lineitem_size, d_part__p_type);
...
count_8<<<std::ceil((float)COUNT6/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT6, d_COUNT9);
uint64_t COUNT9;
...
main_8<<<std::ceil((float)COUNT6/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT6, d_MAT9map1__tmp_attr4, d_MAT_IDX9, d_aggr0__tmp_attr0, d_aggr0__tmp_attr2, d_cycles_per_warp_main_8_map_7, d_cycles_per_warp_main_8_materialize_9);
