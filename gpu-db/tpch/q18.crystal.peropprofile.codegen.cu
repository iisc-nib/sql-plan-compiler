...
__global__ void main_1(HASHTABLE_FIND HT_0, DBI32Type* KEY_0lineitem_u_1__l_orderkey, DBDecimalType* aggr0__tmp_attr0, int64_t* cycles_per_warp_main_1_aggregation_0, size_t lineitem_size, DBI32Type* lineitem_u_1__l_orderkey, DBDecimalType* lineitem_u_1__l_quantity) {
...
__global__ void count_3(size_t COUNT0, uint64_t* COUNT4, DBDecimalType* aggr0__tmp_attr0) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
__global__ void count_5(uint64_t* BUF_6, uint64_t* COUNT8, HASHTABLE_PROBE_SJ HT_4, HASHTABLE_PROBE HT_6, DBI32Type* orders__o_custkey, DBI32Type* orders__o_orderkey, size_t orders_size) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
BUF_8[(buf_idx_8) * 2 + 0] = BUF_6[slot_second6[ITEM] * 1 + 0];
...
__global__ void count_9(uint64_t* BUF_8, HASHTABLE_INSERT HT_10, HASHTABLE_PROBE HT_8, DBI32Type* lineitem__l_orderkey, size_t lineitem_size, DBI32Type* orders__o_orderkey) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
count_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_HT_0.ref(cuco::insert), lineitem_size, d_lineitem__l_orderkey);
...
main_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_HT_0.ref(cuco::find), d_KEY_0lineitem_u_1__l_orderkey, d_aggr0__tmp_attr0, d_cycles_per_warp_main_1_aggregation_0, lineitem_size, d_lineitem__l_orderkey, d_lineitem__l_quantity);
...
count_3<<<std::ceil((float)COUNT0/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT0, d_COUNT4, d_aggr0__tmp_attr0);
uint64_t COUNT4;
...
main_3<<<std::ceil((float)COUNT0/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT0, d_HT_4.ref(cuco::insert), d_aggr0__tmp_attr0, d_cycles_per_warp_main_3_selection_2, d_cycles_per_warp_main_3_semi_join_build_4, d_KEY_0lineitem_u_1__l_orderkey);
//Materialize count
uint64_t* d_COUNT6;
...
main_7<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_6, d_BUF_IDX_6, d_HT_6.ref(cuco::insert), d_customer__c_custkey, customer_size, d_cycles_per_warp_main_7_join_build_6);
...
count_5<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_6, d_COUNT8, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_orders__o_custkey, d_orders__o_orderkey, orders_size);
uint64_t COUNT8;
...
main_5<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_6, d_BUF_8, d_BUF_IDX_8, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::insert), d_cycles_per_warp_main_5_join_build_8, d_cycles_per_warp_main_5_join_probe_6, d_cycles_per_warp_main_5_semi_join_probe_4, d_orders__o_custkey, d_orders__o_orderkey, orders_size);
...
count_9<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_8, d_HT_10.ref(cuco::insert), d_HT_8.ref(cuco::find), d_lineitem__l_orderkey, lineitem_size, d_orders__o_orderkey);
...
main_9<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_8, d_HT_10.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_10orders__o_orderkey, d_aggr1__tmp_attr1, d_aggr__c_custkey, d_aggr__o_orderdate, d_aggr__o_totalprice, d_customer__c_custkey, d_cycles_per_warp_main_9_aggregation_10, d_cycles_per_warp_main_9_join_probe_8, d_lineitem__l_orderkey, d_lineitem__l_quantity, lineitem_size, d_orders__o_orderdate, d_orders__o_orderkey, d_orders__o_totalprice);
