...
__global__ void count_1(HASHTABLE_INSERT HT_5, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_quantity, DBDateType* lineitem__l_shipdate, size_t lineitem_size) {
...
count_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_HT_5.ref(cuco::insert), d_lineitem__l_discount, d_lineitem__l_quantity, d_lineitem__l_shipdate, lineitem_size);
...
main_7<<<std::ceil((float)COUNT5/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT5, d_MAT6aggr0__tmp_attr0, d_MAT_IDX6, d_aggr0__tmp_attr0, d_cycles_per_warp_main_7_materialize_6);
