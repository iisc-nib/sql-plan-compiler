...
BUF_8[(buf_idx_8) * 2 + 0] = BUF_2[slot_second2[ITEM] * 1 + 0];
BUF_8[(buf_idx_8) * 2 + 1] = ITEM*TB + tid;
}
}
...
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
__global__ void count_15(uint64_t* BUF_0, uint64_t* BUF_10, uint64_t* BUF_12, uint64_t* BUF_4, uint64_t* BUF_6, uint64_t* BUF_8, HASHTABLE_PROBE HT_0, HASHTABLE_PROBE HT_10, HASHTABLE_PROBE HT_12, HASHTABLE_INSERT HT_14, HASHTABLE_PROBE HT_4, HASHTABLE_PROBE HT_6, HASHTABLE_PROBE HT_8, DBI32Type* customer__c_nationkey, DBI32Type* lineitem__l_orderkey, DBI32Type* lineitem__l_partkey, DBI32Type* lineitem__l_suppkey, size_t lineitem_size, DBI32Type* orders__o_custkey, DBDateType* orders__o_orderdate, DBI32Type* supplier__s_nationkey) {
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
for (int i=0; i<ITEMS_PER_THREAD; i++) selection_flags[i] = 1;
...
count_1<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT0, d_part__p_type, part_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)part_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_part__p_partkey, d_part__p_type, part_size);
//Materialize count
uint64_t* d_COUNT2;
cudaMalloc(&d_COUNT2, sizeof(uint64_t));
cudaMemset(d_COUNT2, 0, sizeof(uint64_t));
count_3<<<std::ceil((float)region_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT2, d_region__r_name, region_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)region_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_2, d_BUF_IDX_2, d_HT_2.ref(cuco::insert), d_region__r_name, d_region__r_regionkey, region_size);
//Materialize count
uint64_t* d_COUNT4;
cudaMalloc(&d_COUNT4, sizeof(uint64_t));
cudaMemset(d_COUNT4, 0, sizeof(uint64_t));
count_5<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT4, d_orders__o_orderdate, orders_size);
uint64_t COUNT4;
...
main_5<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
//Materialize count
uint64_t* d_COUNT6;
cudaMalloc(&d_COUNT6, sizeof(uint64_t));
cudaMemset(d_COUNT6, 0, sizeof(uint64_t));
count_7<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT6, customer_size);
uint64_t COUNT6;
...
main_7<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_6, d_BUF_IDX_6, d_HT_6.ref(cuco::insert), d_customer__c_custkey, customer_size);
//Materialize count
uint64_t* d_COUNT8;
cudaMalloc(&d_COUNT8, sizeof(uint64_t));
cudaMemset(d_COUNT8, 0, sizeof(uint64_t));
count_9<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_2, d_COUNT8, d_HT_2.ref(cuco::find), d_nation__n_regionkey, nation_size);
uint64_t COUNT8;
...
main_9<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_2, d_BUF_8, d_BUF_IDX_8, d_HT_2.ref(cuco::find), d_HT_8.ref(cuco::insert), d_nation__n_nationkey, d_nation__n_regionkey, nation_size);
//Materialize count
uint64_t* d_COUNT10;
cudaMalloc(&d_COUNT10, sizeof(uint64_t));
cudaMemset(d_COUNT10, 0, sizeof(uint64_t));
count_11<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT10, supplier_size);
uint64_t COUNT10;
...
main_11<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_10, d_BUF_IDX_10, d_HT_10.ref(cuco::insert), d_supplier__s_suppkey, supplier_size);
//Materialize count
uint64_t* d_COUNT12;
cudaMalloc(&d_COUNT12, sizeof(uint64_t));
cudaMemset(d_COUNT12, 0, sizeof(uint64_t));
count_13<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT12, nation_size);
uint64_t COUNT12;
...
main_13<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_12, d_BUF_IDX_12, d_HT_12.ref(cuco::insert), d_nation__n_nationkey, nation_size);
//Create aggregation hash table
...
count_17<<<std::ceil((float)COUNT14/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT14, d_COUNT16);
uint64_t COUNT16;
...
main_17<<<std::ceil((float)COUNT14/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT14, d_MAT16map0__tmp_attr0, d_MAT16map2__tmp_attr5, d_MAT_IDX16, d_aggr0__tmp_attr2, d_aggr0__tmp_attr4, d_KEY_14map0__tmp_attr0);
