...
if (!(Like(reg_part__p_name, "forest%"))) return;
//Materialize count
...
auto reg_partsupp__ps_suppkey = partsupp__ps_suppkey[BUF_2[slot_second2 * 1 + 0]];
aggregate_any(&moved_aggr__ps_suppkey[buf_idx_4], reg_partsupp__ps_suppkey);
auto reg_partsupp__ps_partkey = partsupp__ps_partkey[BUF_2[slot_second2 * 1 + 0]];
aggregate_any(&moved_aggr_u_1__ps_partkey[buf_idx_4], reg_partsupp__ps_partkey);
auto reg_partsupp__ps_availqty = partsupp__ps_availqty[BUF_2[slot_second2 * 1 + 0]];
...
count_5<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_2, d_HT_2.ref(cuco::for_each), d_HT_4.ref(cuco::insert), d_lineitem__l_partkey, d_lineitem__l_shipdate, d_lineitem__l_suppkey, lineitem_size);
...
main_5<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_2, d_HT_2.ref(cuco::for_each), d_HT_4.ref(cuco::find), d_KEY_4lineitem__l_partkey, d_KEY_4lineitem__l_suppkey, d_aggr0__tmp_attr0, d_lineitem__l_partkey, d_lineitem__l_quantity, d_lineitem__l_shipdate, d_lineitem__l_suppkey, lineitem_size, d_moved_aggr__ps_suppkey, d_moved_aggr_u_1__ps_partkey, d_moved_aggr_u_2__ps_availqty, d_partsupp__ps_availqty, d_partsupp__ps_partkey, d_partsupp__ps_suppkey);
//Materialize count
uint64_t* d_COUNT6;
...
count_11<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_6, d_COUNT10, d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::find), d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
uint64_t COUNT10;
...
main_11<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_6, d_HT_6.ref(cuco::for_each), d_HT_8.ref(cuco::find), d_MAT10supplier__s_address_encoded, d_MAT10supplier__s_name_encoded, d_MAT_IDX10, d_supplier__s_address_encoded, d_supplier__s_name_encoded, d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
