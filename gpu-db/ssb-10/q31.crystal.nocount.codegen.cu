...
main_1<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_supplier__s_region, d_supplier__s_suppkey, supplier_size);
...
main_3<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_2, d_BUF_IDX_2, d_HT_2.ref(cuco::insert), d_customer__c_custkey, d_customer__c_region, customer_size);
...
size_t COUNT6 = 1106643;
auto d_HT_6 = cuco::static_map{ (int)1106643*2, cuco::empty_key{(int64_t)-1},         cuco::empty_value{(int64_t)-1},         thrust::equal_to<int64_t>{},         cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
int* d_SLOT_COUNT_6;
...
main_9<<<std::ceil((float)COUNT6/(float)TILE_SIZE), TB>>>(COUNT6, d_MAT8aggr0__tmp_attr0, d_MAT8customer__c_nation_encoded, d_MAT8date__d_year, d_MAT8supplier__s_nation_encoded, d_MAT_IDX8, d_aggr0__tmp_attr0, d_KEY_6customer__c_nation_encoded, d_KEY_6date__d_year, d_KEY_6supplier__s_nation_encoded);
...
for (auto i=0ull; i < MATCOUNT_8; i++) { std::cout << "" << customer__c_nation_map[MAT8customer__c_nation_encoded[i]];
