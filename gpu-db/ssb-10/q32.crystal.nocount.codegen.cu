...
main_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_customer__c_custkey, d_customer__c_nation, customer_size);
...
main_3<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TB>>>(d_BUF_2, d_BUF_IDX_2, d_HT_2.ref(cuco::insert), d_supplier__s_nation, d_supplier__s_suppkey, supplier_size);
