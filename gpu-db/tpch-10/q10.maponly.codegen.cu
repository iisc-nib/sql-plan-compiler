...
__global__ void count_3(uint64_t* COUNT2, size_t customer_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename HASHTABLE_INSERT_PK>
__global__ void main_3(uint64_t* BUF_2, uint64_t* BUF_IDX_2, HASHTABLE_INSERT_PK HT_2, DBI32Type* customer__c_custkey, size_t customer_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= customer_size) return;
uint64_t KEY_2 = 0;
...
if (!(true)) return;
uint64_t KEY_4 = 0;
auto reg_customer__c_nationkey = customer__c_nationkey[BUF_2[SLOT_2->second * 1 + 0]];
...
if (!(true)) return;
uint64_t KEY_6 = 0;
auto reg_customer__c_custkey = customer__c_custkey[BUF_2[SLOT_2->second * 1 + 0]];
...
__global__ void main_7(uint64_t* BUF_0, uint64_t* BUF_2, uint64_t* BUF_4, HASHTABLE_PROBE_PK HT_0, HASHTABLE_PROBE_PK HT_2, HASHTABLE_PROBE_PK HT_4, HASHTABLE_FIND HT_6, DBI32Type* KEY_6customer__c_custkey, DBDecimalType* aggr0__tmp_attr0, DBDecimalType* aggr__c_acctbal, DBI16Type* aggr__n_name_encoded, DBDecimalType* customer__c_acctbal, DBI32Type* customer__c_custkey, DBI32Type* customer__c_nationkey, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBI32Type* lineitem__l_orderkey, DBCharType* lineitem__l_returnflag, size_t lineitem_size, DBI16Type* nation__n_name_encoded, DBI32Type* orders__o_custkey) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineitem_size) return;
auto reg_lineitem__l_returnflag = lineitem__l_returnflag[tid];
if (!(evaluatePredicate(reg_lineitem__l_returnflag, 'R', Predicate::eq))) return;
...
auto reg_customer__c_acctbal = customer__c_acctbal[BUF_2[SLOT_2->second * 1 + 0]];
...
count_3<<<std::ceil((float)customer_size/128.), 128>>>(d_COUNT2, customer_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_2, d_BUF_IDX_2, d_HT_2.ref(cuco::insert), d_customer__c_custkey, customer_size);
//Materialize count
uint64_t* d_COUNT4;
...
count_7<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_4, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_customer__c_custkey, d_customer__c_nationkey, d_lineitem__l_orderkey, d_lineitem__l_returnflag, lineitem_size, d_orders__o_custkey);
...
main_7<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_4, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_KEY_6customer__c_custkey, d_aggr0__tmp_attr0, d_aggr__c_acctbal, d_aggr__n_name_encoded, d_customer__c_acctbal, d_customer__c_custkey, d_customer__c_nationkey, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_returnflag, lineitem_size, d_nation__n_name_encoded, d_orders__o_custkey);
//Materialize count
uint64_t* d_COUNT8;
