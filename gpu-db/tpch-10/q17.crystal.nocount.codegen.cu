...
main_1<<<std::ceil((float)part_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_part__p_brand, d_part__p_container, d_part__p_partkey, part_size);
...
main_3<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert_and_find), d_KEY_2lineitem_u_1__l_partkey, d_SLOT_COUNT_2, d_aggr_rw__rw0, d_aggr_rw__rw1, lineitem_size, d_lineitem__l_partkey, d_lineitem__l_quantity, d_moved_aggr__p_partkey, d_part__p_partkey);
COUNT2 = d_HT_2.size();
...
main_5<<<std::ceil((float)COUNT2/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_IDX_4, COUNT2, d_HT_4.ref(cuco::insert), d_moved_aggr__p_partkey);
size_t COUNT6 = 1;
...
main_7<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert_and_find), d_SLOT_COUNT_6, d_aggr1__tmp_attr2, d_aggr_rw__rw0, d_aggr_rw__rw1, d_lineitem__l_extendedprice, d_lineitem__l_partkey, d_lineitem__l_quantity, lineitem_size);
COUNT6 = d_HT_6.size();
...
main_9<<<std::ceil((float)COUNT6/(float)TILE_SIZE), TB>>>(COUNT6, d_MAT8map1__tmp_attr3, d_MAT_IDX8, d_aggr1__tmp_attr2);
