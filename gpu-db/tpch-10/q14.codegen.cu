...
#include "dbruntime.h"
#include <chrono>
__global__ void count_1(uint64_t* COUNT0, DBDateType* lineitem__l_shipdate, size_t lineitem_size) {
...
__global__ void main_1(uint64_t* BUF_0, uint64_t* BUF_IDX_0, HASHTABLE_INSERT HT_0, DBI32Type* lineitem__l_partkey, DBDateType* lineitem__l_shipdate, size_t lineitem_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineitem_size) return;
auto reg_lineitem__l_shipdate = lineitem__l_shipdate[tid];
if (!(evaluatePredicate(reg_lineitem__l_shipdate, 9374, Predicate::gte) && evaluatePredicate(reg_lineitem__l_shipdate, 9404, Predicate::lt))) return;
...
__global__ void main_3(uint64_t* BUF_0, HASHTABLE_PROBE HT_0, HASHTABLE_FIND HT_2, DBDecimalType* aggr0__tmp_attr0, DBDecimalType* aggr0__tmp_attr2, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBI32Type* part__p_partkey, DBStringType* part__p_type, size_t part_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= part_size) return;
uint64_t KEY_0 = 0;
auto reg_part__p_partkey = part__p_partkey[tid];
...
auto reg_lineitem__l_discount = lineitem__l_discount[BUF_0[slot_second0 * 1 + 0]];
...
count_1<<<std::ceil((float)lineitem_size/128.), 128>>>(d_COUNT0, d_lineitem__l_shipdate, lineitem_size);
uint64_t COUNT0;
...
main_3<<<std::ceil((float)part_size/128.), 128>>>(d_BUF_0, d_HT_0.ref(cuco::for_each), d_HT_2.ref(cuco::find), d_aggr0__tmp_attr0, d_aggr0__tmp_attr2, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_part__p_partkey, d_part__p_type, part_size);
//Materialize count
uint64_t* d_COUNT4;
