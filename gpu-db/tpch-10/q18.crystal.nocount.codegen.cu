...
main_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_HT_0.ref(cuco::insert_and_find), d_KEY_0lineitem_u_1__l_orderkey, d_SLOT_COUNT_0, d_aggr0__tmp_attr0, lineitem_size, d_lineitem__l_orderkey, d_lineitem__l_quantity);
COUNT0 = d_HT_0.size();
...
main_3<<<std::ceil((float)COUNT0/(float)TILE_SIZE), TB>>>(COUNT0, d_HT_2.ref(cuco::insert), d_aggr0__tmp_attr0, d_KEY_0lineitem_u_1__l_orderkey);
...
main_5<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_customer__c_custkey, customer_size);
...
main_7<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_6, d_BUF_IDX_6, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_orders__o_custkey, d_orders__o_orderkey, orders_size);
size_t COUNT8 = 59972108;
...
main_11<<<std::ceil((float)COUNT8/(float)TILE_SIZE), TB>>>(COUNT8, d_MAT10aggr1__tmp_attr1, d_MAT10aggr__c_custkey, d_MAT10aggr__o_orderdate, d_MAT10aggr__o_totalprice, d_MAT10orders__o_orderkey, d_MAT_IDX10, d_aggr1__tmp_attr1, d_aggr__c_custkey, d_aggr__o_orderdate, d_aggr__o_totalprice, d_KEY_8orders__o_orderkey);
