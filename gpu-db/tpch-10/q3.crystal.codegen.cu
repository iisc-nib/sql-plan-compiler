...
if (!selection_flags[ITEM]) continue;
KEY_2[ITEM] = 0;
KEY_2[ITEM] |= reg_lineitem__l_orderkey[ITEM];
}
...
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_orders__o_shippriority[ITEM] = orders__o_shippriority[BUF_2[slot_second2[ITEM] * 2 + 1]];
}
DBDateType reg_orders__o_orderdate[ITEMS_PER_THREAD];
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
reg_orders__o_orderdate[ITEM] = orders__o_orderdate[BUF_2[slot_second2[ITEM] * 2 + 1]];
}
#pragma unroll
for (int ITEM = 0; ITEM < ITEMS_PER_THREAD && (ITEM*TB + tid < lineitem_size); ++ITEM) {
if (!selection_flags[ITEM]) continue;
auto buf_idx_4 = HT_4.find(KEY_4[ITEM])->second;
aggregate_sum(&aggr0__tmp_attr0[buf_idx_4], reg_map0__tmp_attr1[ITEM]);
aggregate_any(&aggr__o_shippriority[buf_idx_4], reg_orders__o_shippriority[ITEM]);
aggregate_any(&aggr__o_orderdate[buf_idx_4], reg_orders__o_orderdate[ITEM]);
KEY_4lineitem__l_orderkey[buf_idx_4] = reg_lineitem__l_orderkey[ITEM];
}
}
...
MAT6lineitem__l_orderkey[mat_idx6] = reg_lineitem__l_orderkey[ITEM];
MAT6aggr0__tmp_attr0[mat_idx6] = reg_aggr0__tmp_attr0[ITEM];
MAT6aggr__o_orderdate[mat_idx6] = reg_aggr__o_orderdate[ITEM];
MAT6aggr__o_shippriority[mat_idx6] = reg_aggr__o_shippriority[ITEM];
}
}
...
count_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT0, d_customer__c_mktsegment, customer_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_customer__c_custkey, d_customer__c_mktsegment, customer_size);
//Materialize count
uint64_t* d_COUNT2;
cudaMalloc(&d_COUNT2, sizeof(uint64_t));
cudaMemset(d_COUNT2, 0, sizeof(uint64_t));
count_3<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_COUNT2, d_HT_0.ref(cuco::find), d_orders__o_custkey, d_orders__o_orderdate, orders_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_orders__o_custkey, d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
//Create aggregation hash table
auto d_HT_4 = cuco::static_map{ (int)3133196*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_5<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_2, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::insert), d_lineitem__l_orderkey, d_lineitem__l_shipdate, lineitem_size);
...
main_5<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_2, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::find), d_KEY_4lineitem__l_orderkey, d_aggr0__tmp_attr0, d_aggr__o_orderdate, d_aggr__o_shippriority, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_shipdate, lineitem_size, d_orders__o_orderdate, d_orders__o_shippriority);
//Materialize count
uint64_t* d_COUNT6;
...
main_7<<<std::ceil((float)COUNT4/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT4, d_MAT6aggr0__tmp_attr0, d_MAT6aggr__o_orderdate, d_MAT6aggr__o_shippriority, d_MAT6lineitem__l_orderkey, d_MAT_IDX6, d_aggr0__tmp_attr0, d_aggr__o_orderdate, d_aggr__o_shippriority, d_KEY_4lineitem__l_orderkey);
