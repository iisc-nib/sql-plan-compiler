...
main_1<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
...
main_3<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_2, d_BUF_IDX_2, d_HT_2.ref(cuco::insert), d_customer__c_custkey, customer_size);
...
main_5<<<std::ceil((float)nation_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_nation__n_nationkey, nation_size);
size_t COUNT6 = 466296;
...
main_9<<<std::ceil((float)COUNT6/(float)TILE_SIZE), TB>>>(COUNT6, d_MAT8aggr0__tmp_attr0, d_MAT8aggr__c_acctbal, d_MAT8aggr__n_name_encoded, d_MAT8customer__c_custkey, d_MAT_IDX8, d_aggr0__tmp_attr0, d_aggr__c_acctbal, d_aggr__n_name_encoded, d_KEY_6customer__c_custkey);
