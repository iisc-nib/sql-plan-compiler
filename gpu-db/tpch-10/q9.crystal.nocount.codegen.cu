...
BUF_2[(ITEM*TB + tid) * 2 + 0] = ITEM*TB + tid;
BUF_2[(ITEM*TB + tid) * 2 + 1] = BUF_0[slot_second0[ITEM] * 1 + 0];
}
}
template<typename HASHTABLE_INSERT>
...
BUF_8[(ITEM*TB + tid) * 3 + 0] = BUF_2[slot_second2[ITEM] * 2 + 0];
BUF_8[(ITEM*TB + tid) * 3 + 1] = ITEM*TB + tid;
BUF_8[(ITEM*TB + tid) * 3 + 2] = BUF_2[slot_second2[ITEM] * 2 + 1];
}
}
...
main_1<<<std::ceil((float)nation_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_nation__n_nationkey, nation_size);
...
main_3<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
...
main_5<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_IDX_4, d_HT_4.ref(cuco::insert), d_orders__o_orderkey, orders_size);
...
main_7<<<std::ceil((float)part_size/(float)TILE_SIZE), TB>>>(d_BUF_6, d_BUF_IDX_6, d_HT_6.ref(cuco::insert), d_part__p_partkey, part_size);
...
main_9<<<std::ceil((float)partsupp_size/(float)TILE_SIZE), TB>>>(d_BUF_2, d_BUF_8, d_BUF_IDX_8, d_HT_2.ref(cuco::find), d_HT_8.ref(cuco::insert), d_partsupp__ps_partkey, d_partsupp__ps_suppkey, partsupp_size, d_supplier__s_suppkey);
...
main_11<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::insert_and_find), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_10map0__tmp_attr0, d_KEY_10nation__n_name_encoded, d_SLOT_COUNT_10, d_aggr0__tmp_attr2, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_partkey, d_lineitem__l_quantity, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded, d_orders__o_orderdate, d_partsupp__ps_supplycost);
...
main_13<<<std::ceil((float)COUNT10/(float)TILE_SIZE), TB>>>(COUNT10, d_MAT12aggr0__tmp_attr2, d_MAT12map0__tmp_attr0, d_MAT12nation__n_name_encoded, d_MAT_IDX12, d_aggr0__tmp_attr2, d_KEY_10map0__tmp_attr0, d_KEY_10nation__n_name_encoded);
