...
BUF_2[(ITEM*TB + tid) * 2 + 0] = BUF_0[slot_second0[ITEM] * 1 + 0];
BUF_2[(ITEM*TB + tid) * 2 + 1] = ITEM*TB + tid;
}
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_FIND>
__global__ void main_5(uint64_t* BUF_2, HASHTABLE_PROBE HT_2, HASHTABLE_FIND HT_4, DBI32Type* KEY_4lineitem__l_orderkey, int* SLOT_COUNT_4, DBDecimalType* aggr0__tmp_attr0, DBDateType* aggr__o_orderdate, DBI32Type* aggr__o_shippriority, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBI32Type* lineitem__l_orderkey, DBDateType* lineitem__l_shipdate, size_t lineitem_size, DBDateType* orders__o_orderdate, DBI32Type* orders__o_shippriority) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
main_1<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_customer__c_custkey, d_customer__c_mktsegment, customer_size);
size_t COUNT2 = orders_size;
// Insert hash table control;
...
main_3<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_orders__o_custkey, d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
size_t COUNT4 = 3133196;
...
main_5<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_2, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::insert_and_find), d_KEY_4lineitem__l_orderkey, d_SLOT_COUNT_4, d_aggr0__tmp_attr0, d_aggr__o_orderdate, d_aggr__o_shippriority, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_shipdate, lineitem_size, d_orders__o_orderdate, d_orders__o_shippriority);
COUNT4 = d_HT_4.size();
...
main_7<<<std::ceil((float)COUNT4/(float)TILE_SIZE), TB>>>(COUNT4, d_MAT6aggr0__tmp_attr0, d_MAT6aggr__o_orderdate, d_MAT6aggr__o_shippriority, d_MAT6lineitem__l_orderkey, d_MAT_IDX6, d_aggr0__tmp_attr0, d_aggr__o_orderdate, d_aggr__o_shippriority, d_KEY_4lineitem__l_orderkey);
...
for (auto i=0ull; i < MATCOUNT_6; i++) { std::cout << "" << MAT6lineitem__l_orderkey[i];
