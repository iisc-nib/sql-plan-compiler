...
__global__ void main_3(HASHTABLE_PROBE_SJ HT_0, HASHTABLE_FIND HT_2, DBI16Type* KEY_2orders__o_orderpriority_encoded, int* SLOT_COUNT_2, DBI64Type* aggr0__tmp_attr0, DBDateType* orders__o_orderdate, DBI32Type* orders__o_orderkey, DBI16Type* orders__o_orderpriority_encoded, size_t orders_size) {
int64_t start, stop, cycles_per_warp;
size_t tile_offset = blockIdx.x * TILE_SIZE;
size_t tid = tile_offset + threadIdx.x;
int selection_flags[ITEMS_PER_THREAD];
...
main_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_HT_0.ref(cuco::insert), d_lineitem__l_commitdate, d_lineitem__l_orderkey, d_lineitem__l_receiptdate, lineitem_size);
size_t COUNT2 = 498047;
auto d_HT_2 = cuco::static_map{ (int)498047*2, cuco::empty_key{(int64_t)-1},         cuco::empty_value{(int64_t)-1},         thrust::equal_to<int64_t>{},         cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
int* d_SLOT_COUNT_2;
...
main_3<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert_and_find), d_KEY_2orders__o_orderpriority_encoded, d_SLOT_COUNT_2, d_aggr0__tmp_attr0, d_orders__o_orderdate, d_orders__o_orderkey, d_orders__o_orderpriority_encoded, orders_size);
COUNT2 = d_HT_2.size();
...
main_5<<<std::ceil((float)COUNT2/(float)TILE_SIZE), TB>>>(COUNT2, d_MAT4aggr0__tmp_attr0, d_MAT4orders__o_orderpriority_encoded, d_MAT_IDX4, d_aggr0__tmp_attr0, d_KEY_2orders__o_orderpriority_encoded);
...
for (auto i=0ull; i < MATCOUNT_4; i++) { std::cout << "" << orders__o_orderpriority_map[MAT4orders__o_orderpriority_encoded[i]];
