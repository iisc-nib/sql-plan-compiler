...
BUF_4[(buf_idx_4) * 2 + 0] = BUF_2[slot_second2[ITEM] * 1 + 0];
BUF_4[(buf_idx_4) * 2 + 1] = ITEM*TB + tid;
}
}
template<typename HASHTABLE_PROBE>
...
BUF_6[(buf_idx_6) * 3 + 0] = ITEM*TB + tid;
BUF_6[(buf_idx_6) * 3 + 1] = BUF_4[slot_second4[ITEM] * 2 + 0];
BUF_6[(buf_idx_6) * 3 + 2] = BUF_4[slot_second4[ITEM] * 2 + 1];
}
}
template<typename HASHTABLE_PROBE>
...
BUF_8[(buf_idx_8) * 2 + 0] = ITEM*TB + tid;
BUF_8[(buf_idx_8) * 2 + 1] = BUF_0[slot_second0[ITEM] * 1 + 0];
}
}
...
count_1<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT0, d_nation__n_name, nation_size);
uint64_t COUNT0;
...
count_3<<<std::ceil((float)nation_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT2, d_nation__n_name, nation_size);
uint64_t COUNT2;
...
count_7<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_4, d_COUNT6, d_HT_4.ref(cuco::find), d_orders__o_custkey, orders_size);
uint64_t COUNT6;
...
count_9<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_0, d_COUNT8, d_HT_0.ref(cuco::find), d_supplier__s_nationkey, supplier_size);
uint64_t COUNT8;
...
count_11<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::insert), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_lineitem__l_orderkey, d_lineitem__l_shipdate, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name, d_nation__n_name_encoded, d_nation__n_name, d_nation__n_name_encoded);
...
main_11<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_10map0__tmp_attr0, d_KEY_10n1___n_name_encoded, d_KEY_10n2___n_name_encoded, d_aggr0__tmp_attr2, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_shipdate, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name, d_nation__n_name_encoded, d_nation__n_name, d_nation__n_name_encoded);
//Materialize count
uint64_t* d_COUNT12;
