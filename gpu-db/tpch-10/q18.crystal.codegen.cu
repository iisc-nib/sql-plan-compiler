...
//Create aggregation hash table
auto d_HT_0 = cuco::static_map{ (int)59986052*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
...
main_1<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_HT_0.ref(cuco::find), d_KEY_0lineitem_u_1__l_orderkey, d_aggr0__tmp_attr0, lineitem_size, d_lineitem__l_orderkey, d_lineitem__l_quantity);
//Materialize count
uint64_t* d_COUNT2;
cudaMalloc(&d_COUNT2, sizeof(uint64_t));
cudaMemset(d_COUNT2, 0, sizeof(uint64_t));
count_3<<<std::ceil((float)COUNT0/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(COUNT0, d_COUNT2, d_aggr0__tmp_attr0);
uint64_t COUNT2;
...
count_5<<<std::ceil((float)customer_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_COUNT4, customer_size);
uint64_t COUNT4;
...
count_7<<<std::ceil((float)orders_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_4, d_COUNT6, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::find), d_orders__o_custkey, d_orders__o_orderkey, orders_size);
uint64_t COUNT6;
...
//Create aggregation hash table
auto d_HT_8 = cuco::static_map{ (int)59972108*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_9<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_6, d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::insert), d_lineitem__l_orderkey, lineitem_size, d_orders__o_orderkey);
...
main_9<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TILE_SIZE/ITEMS_PER_THREAD>>>(d_BUF_6, d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_8orders__o_orderkey, d_aggr1__tmp_attr1, d_aggr__c_custkey, d_aggr__o_orderdate, d_aggr__o_totalprice, d_customer__c_custkey, d_lineitem__l_orderkey, d_lineitem__l_quantity, lineitem_size, d_orders__o_orderdate, d_orders__o_orderkey, d_orders__o_totalprice);
//Materialize count
uint64_t* d_COUNT10;
