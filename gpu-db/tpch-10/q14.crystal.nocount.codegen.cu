...
main_1<<<std::ceil((float)part_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_part__p_partkey, part_size);
size_t COUNT2 = 1;
...
main_3<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert_and_find), d_SLOT_COUNT_2, d_aggr0__tmp_attr0, d_aggr0__tmp_attr2, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_partkey, d_lineitem__l_shipdate, lineitem_size, d_part__p_type);
COUNT2 = d_HT_2.size();
...
main_5<<<std::ceil((float)COUNT2/(float)TILE_SIZE), TB>>>(COUNT2, d_MAT4map1__tmp_attr4, d_MAT_IDX4, d_aggr0__tmp_attr0, d_aggr0__tmp_attr2);
