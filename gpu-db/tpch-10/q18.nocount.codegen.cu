...
template<typename HASHTABLE_PROBE_SJ, typename HASHTABLE_PROBE_PK, typename HASHTABLE_INSERT>
__global__ void main_7(uint64_t* BUF_4, uint64_t* BUF_6, uint64_t* BUF_IDX_6, HASHTABLE_PROBE_SJ HT_2, HASHTABLE_PROBE_PK HT_4, HASHTABLE_INSERT HT_6, DBI32Type* orders__o_custkey, DBI32Type* orders__o_orderkey, size_t orders_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
main_1<<<std::ceil((float)lineitem_size/128.), 128>>>(d_HT_0.ref(cuco::insert_and_find), d_KEY_0lineitem_u_1__l_orderkey, d_SLOT_COUNT_0, d_aggr0__tmp_attr0, lineitem_size, d_lineitem__l_orderkey, d_lineitem__l_quantity);
COUNT0 = d_HT_0.size();
