...
main_1<<<std::ceil((float)part_size/(float)TILE_SIZE), TB>>>(d_HT_0.ref(cuco::insert), d_part__p_name, d_part__p_partkey, part_size);
size_t COUNT2 = partsupp_size;
// Insert hash table control;
...
main_3<<<std::ceil((float)partsupp_size/(float)TILE_SIZE), TB>>>(d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_partsupp__ps_partkey, d_partsupp__ps_suppkey, partsupp_size);
size_t COUNT4 = 9138501;
...
main_5<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_2, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::insert_and_find), d_KEY_4lineitem__l_partkey, d_KEY_4lineitem__l_suppkey, d_SLOT_COUNT_4, d_aggr0__tmp_attr0, d_lineitem__l_partkey, d_lineitem__l_quantity, d_lineitem__l_shipdate, d_lineitem__l_suppkey, lineitem_size, d_moved_aggr__ps_suppkey, d_moved_aggr_u_1__ps_availqty, d_moved_aggr_u_2__ps_partkey, d_partsupp__ps_availqty, d_partsupp__ps_partkey, d_partsupp__ps_suppkey);
...
main_7<<<std::ceil((float)nation_size/(float)TILE_SIZE), TB>>>(d_BUF_6, d_BUF_IDX_6, d_HT_6.ref(cuco::insert), d_nation__n_name, d_nation__n_nationkey, nation_size);
COUNT4 = d_HT_4.size();
size_t COUNT8 = COUNT4;
// Insert hash table control;
...
main_9<<<std::ceil((float)COUNT4/(float)TILE_SIZE), TB>>>(COUNT4, d_HT_8.ref(cuco::insert), d_aggr0__tmp_attr0, d_moved_aggr__ps_suppkey, d_moved_aggr_u_1__ps_availqty, d_moved_aggr_u_2__ps_partkey);
...
for (auto i=0ull; i < MATCOUNT_10; i++) { std::cout << "" << supplier__s_name_map[MAT10supplier__s_name_encoded[i]];
