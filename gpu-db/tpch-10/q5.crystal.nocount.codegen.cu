...
BUF_4[(ITEM*TB + tid) * 3 + 0] = ITEM*TB + tid;
BUF_4[(ITEM*TB + tid) * 3 + 1] = BUF_2[slot_second2[ITEM] * 2 + 0];
BUF_4[(ITEM*TB + tid) * 3 + 2] = BUF_2[slot_second2[ITEM] * 2 + 1];
}
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
...
BUF_6[(ITEM*TB + tid) * 4 + 0] = BUF_4[slot_second4[ITEM] * 3 + 0];
BUF_6[(ITEM*TB + tid) * 4 + 1] = ITEM*TB + tid;
BUF_6[(ITEM*TB + tid) * 4 + 2] = BUF_4[slot_second4[ITEM] * 3 + 1];
BUF_6[(ITEM*TB + tid) * 4 + 3] = BUF_4[slot_second4[ITEM] * 3 + 2];
}
}
template<typename HASHTABLE_INSERT>
...
main_1<<<std::ceil((float)region_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_region__r_name, d_region__r_regionkey, region_size);
...
main_3<<<std::ceil((float)nation_size/(float)TILE_SIZE), TB>>>(d_BUF_0, d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_nation__n_nationkey, d_nation__n_regionkey, nation_size);
...
main_5<<<std::ceil((float)customer_size/(float)TILE_SIZE), TB>>>(d_BUF_2, d_BUF_4, d_BUF_IDX_4, d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::insert), d_customer__c_custkey, d_customer__c_nationkey, customer_size);
...
main_7<<<std::ceil((float)orders_size/(float)TILE_SIZE), TB>>>(d_BUF_4, d_BUF_6, d_BUF_IDX_6, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_orders__o_custkey, d_orders__o_orderdate, d_orders__o_orderkey, orders_size);
...
main_9<<<std::ceil((float)supplier_size/(float)TILE_SIZE), TB>>>(d_BUF_8, d_BUF_IDX_8, d_HT_8.ref(cuco::insert), d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
size_t COUNT10 = 221140;
auto d_HT_10 = cuco::static_map{ (int)221140*2, cuco::empty_key{(int64_t)-1},         cuco::empty_value{(int64_t)-1},         thrust::equal_to<int64_t>{},         cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
int* d_SLOT_COUNT_10;
...
main_11<<<std::ceil((float)lineitem_size/(float)TILE_SIZE), TB>>>(d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::insert_and_find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_10nation__n_name_encoded, d_SLOT_COUNT_10, d_aggr0__tmp_attr0, d_customer__c_nationkey, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded);
...
main_13<<<std::ceil((float)COUNT10/(float)TILE_SIZE), TB>>>(COUNT10, d_MAT12aggr0__tmp_attr0, d_MAT12nation__n_name_encoded, d_MAT_IDX12, d_aggr0__tmp_attr0, d_KEY_10nation__n_name_encoded);
...
for (auto i=0ull; i < MATCOUNT_12; i++) { std::cout << "" << nation__n_name_map[MAT12nation__n_name_encoded[i]];
