...
auto reg_supplier__s_nation_encoded = supplier__s_nation_encoded[BUF_6[SLOT_6->second * 4 + 2]];
KEY_8 <<= 16;
...
count_9<<<std::ceil((float)part_size/128.), 128>>>(d_BUF_6, d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::insert), d_date__d_year, d_part__p_category_encoded, d_part__p_mfgr, d_part__p_partkey, part_size, d_supplier__s_nation_encoded);
