...
#include "db_types.h"
#include "dbruntime.h"
__global__ void count_1(uint64_t* COUNT0, DBStringType* supplier__s_city, size_t supplier_size) {
...
__global__ void main_1(uint64_t* BUF_0, uint64_t* BUF_IDX_0, HASHTABLE_INSERT HT_0, DBStringType* supplier__s_city, DBI32Type* supplier__s_suppkey, size_t supplier_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= supplier_size) return;
auto reg_supplier__s_city = supplier__s_city[tid];
if (!((evaluatePredicate(reg_supplier__s_city, "UNITED KI1", Predicate::eq)) || (evaluatePredicate(reg_supplier__s_city, "UNITED KI5", Predicate::eq)))) return;
...
__global__ void count_3(uint64_t* COUNT2, DBStringType* customer__c_city, size_t customer_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
BUF_4[buf_idx_4 * 3 + 0] = BUF_2[SLOT_2->second * 1 + 0];
BUF_4[buf_idx_4 * 3 + 1] = tid;
BUF_4[buf_idx_4 * 3 + 2] = BUF_0[SLOT_0->second * 1 + 0];
}
template<typename HASHTABLE_PROBE, typename HASHTABLE_INSERT>
...
if (!(true)) return;
uint64_t KEY_6 = 0;
auto reg_customer__c_city_encoded = customer__c_city_encoded[BUF_4[SLOT_4->second * 3 + 0]];

KEY_6 |= reg_customer__c_city_encoded;
auto reg_supplier__s_city_encoded = supplier__s_city_encoded[BUF_4[SLOT_4->second * 3 + 2]];
KEY_6 <<= 16;
...
auto reg_lineorder__lo_revenue = lineorder__lo_revenue[BUF_4[SLOT_4->second * 3 + 1]];
...
count_1<<<std::ceil((float)supplier_size/128.), 128>>>(d_COUNT0, d_supplier__s_city, supplier_size);
uint64_t COUNT0;
...
main_1<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_0, d_BUF_IDX_0, d_HT_0.ref(cuco::insert), d_supplier__s_city, d_supplier__s_suppkey, supplier_size);
//Materialize count
uint64_t* d_COUNT2;
cudaMalloc(&d_COUNT2, sizeof(uint64_t));
cudaMemset(d_COUNT2, 0, sizeof(uint64_t));
count_3<<<std::ceil((float)customer_size/128.), 128>>>(d_COUNT2, d_customer__c_city, customer_size);
uint64_t COUNT2;
...
count_5<<<std::ceil((float)lineorder_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_COUNT4, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_lineorder__lo_custkey, d_lineorder__lo_suppkey, lineorder_size);
uint64_t COUNT4;
...
main_5<<<std::ceil((float)lineorder_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_4, d_BUF_IDX_4, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::find), d_HT_4.ref(cuco::insert), d_lineorder__lo_custkey, d_lineorder__lo_orderdate, d_lineorder__lo_suppkey, lineorder_size);
//Create aggregation hash table
auto d_HT_6 = cuco::static_map{ (int)208*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_7<<<std::ceil((float)date_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_customer__c_city_encoded, d_date__d_datekey, d_date__d_year, date_size, d_supplier__s_city_encoded);
...
main_7<<<std::ceil((float)date_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_KEY_6customer__c_city_encoded, d_KEY_6date__d_year, d_KEY_6supplier__s_city_encoded, d_aggr0__tmp_attr0, d_customer__c_city_encoded, d_date__d_datekey, d_date__d_year, date_size, d_lineorder__lo_revenue, d_supplier__s_city_encoded);
//Materialize count
uint64_t* d_COUNT8;
