...
BUF_6[buf_idx_6 * 4 + 1] = BUF_2[SLOT_2->second * 1 + 0];
BUF_6[buf_idx_6 * 4 + 2] = BUF_0[SLOT_0->second * 1 + 0];
...
if (!(true)) return;
uint64_t KEY_8 = 0;
auto reg_date__d_year = date__d_year[BUF_6[SLOT_6->second * 4 + 0]];

KEY_8 |= reg_date__d_year;
auto reg_supplier__s_city_encoded = supplier__s_city_encoded[BUF_6[SLOT_6->second * 4 + 1]];
KEY_8 <<= 16;
KEY_8 |= reg_supplier__s_city_encoded;
auto reg_part__p_brand1_encoded = part__p_brand1_encoded[BUF_6[SLOT_6->second * 4 + 2]];
KEY_8 <<= 16;
...
auto reg_lineorder__lo_supplycost = lineorder__lo_supplycost[BUF_6[SLOT_6->second * 4 + 3]];
auto reg_lineorder__lo_revenue = lineorder__lo_revenue[BUF_6[SLOT_6->second * 4 + 3]];
...
//Create aggregation hash table
auto d_HT_8 = cuco::static_map{ (int)1744*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_9<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_6, d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::insert), d_customer__c_custkey, customer_size, d_date__d_year, d_part__p_brand1_encoded, d_supplier__s_city_encoded);
...
main_9<<<std::ceil((float)customer_size/128.), 128>>>(d_BUF_6, d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_8date__d_year, d_KEY_8part__p_brand1_encoded, d_KEY_8supplier__s_city_encoded, d_aggr0__tmp_attr0, d_customer__c_custkey, customer_size, d_date__d_year, d_lineorder__lo_revenue, d_lineorder__lo_supplycost, d_part__p_brand1_encoded, d_supplier__s_city_encoded);
//Materialize count
uint64_t* d_COUNT10;
