...
auto reg_part__p_brand1_encoded = part__p_brand1_encoded[BUF_4[SLOT_4->second * 3 + 1]];
KEY_6 <<= 16;
...
auto reg_lineorder__lo_revenue = lineorder__lo_revenue[BUF_4[SLOT_4->second * 3 + 0]];
...
//Create aggregation hash table
auto d_HT_6 = cuco::static_map{ (int)2210*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_7<<<std::ceil((float)date_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_date__d_datekey, d_date__d_year, date_size, d_part__p_brand1_encoded);
...
main_7<<<std::ceil((float)date_size/128.), 128>>>(d_BUF_4, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_KEY_6date__d_year, d_KEY_6part__p_brand1_encoded, d_aggr0__tmp_attr0, d_date__d_datekey, d_date__d_year, date_size, d_lineorder__lo_revenue, d_part__p_brand1_encoded);
//Materialize count
uint64_t* d_COUNT8;
