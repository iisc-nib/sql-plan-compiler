...
#include "dbruntime.h"
#include <chrono>
__global__ void count_1(uint64_t* COUNT0, DBStringType* n1___n_name, size_t nation_size) {
...
__global__ void main_1(uint64_t* BUF_0, uint64_t* BUF_IDX_0, HASHTABLE_INSERT_PK HT_0, DBStringType* n1___n_name, DBI32Type* n1___n_nationkey, size_t nation_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= nation_size) return;
auto reg_n1___n_name = n1___n_name[tid];
if (!((evaluatePredicate(reg_n1___n_name, "FRANCE", Predicate::eq)) || (evaluatePredicate(reg_n1___n_name, "GERMANY", Predicate::eq)))) return;
...
__global__ void count_3(uint64_t* COUNT2, DBStringType* n2___n_name, size_t nation_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= nation_size) return;
...
template<typename HASHTABLE_INSERT_PK>
__global__ void main_3(uint64_t* BUF_2, uint64_t* BUF_IDX_2, HASHTABLE_INSERT_PK HT_2, DBStringType* n2___n_name, DBI32Type* n2___n_nationkey, size_t nation_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= nation_size) return;
auto reg_n2___n_name = n2___n_name[tid];
if (!((evaluatePredicate(reg_n2___n_name, "GERMANY", Predicate::eq)) || (evaluatePredicate(reg_n2___n_name, "FRANCE", Predicate::eq)))) return;
...
BUF_4[buf_idx_4 * 2 + 0] = BUF_2[SLOT_2->second * 1 + 0];
BUF_4[buf_idx_4 * 2 + 1] = tid;
}
template<typename HASHTABLE_PROBE_PK>
__global__ void count_7(uint64_t* BUF_4, uint64_t* COUNT6, HASHTABLE_PROBE_PK HT_4, DBI32Type* orders__o_custkey, size_t orders_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename HASHTABLE_PROBE_PK, typename HASHTABLE_INSERT_PK>
__global__ void main_7(uint64_t* BUF_4, uint64_t* BUF_6, uint64_t* BUF_IDX_6, HASHTABLE_PROBE_PK HT_4, HASHTABLE_INSERT_PK HT_6, DBI32Type* orders__o_custkey, DBI32Type* orders__o_orderkey, size_t orders_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= orders_size) return;
uint64_t KEY_4 = 0;
auto reg_orders__o_custkey = orders__o_custkey[tid];
...
BUF_6[buf_idx_6 * 3 + 1] = BUF_4[SLOT_4->second * 2 + 0];
BUF_6[buf_idx_6 * 3 + 2] = BUF_4[SLOT_4->second * 2 + 1];
}
template<typename HASHTABLE_PROBE_PK>
__global__ void count_9(uint64_t* BUF_0, uint64_t* COUNT8, HASHTABLE_PROBE_PK HT_0, DBI32Type* supplier__s_nationkey, size_t supplier_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename HASHTABLE_PROBE_PK, typename HASHTABLE_INSERT_PK>
__global__ void main_9(uint64_t* BUF_0, uint64_t* BUF_8, uint64_t* BUF_IDX_8, HASHTABLE_PROBE_PK HT_0, HASHTABLE_INSERT_PK HT_8, DBI32Type* supplier__s_nationkey, DBI32Type* supplier__s_suppkey, size_t supplier_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= supplier_size) return;
uint64_t KEY_0 = 0;
auto reg_supplier__s_nationkey = supplier__s_nationkey[tid];
...
auto reg_n1___n_name = n1___n_name[BUF_8[SLOT_8->second * 2 + 1]];
auto reg_n2___n_name = n2___n_name[BUF_6[SLOT_6->second * 3 + 1]];
...
uint64_t KEY_10 = 0;
auto reg_n1___n_name_encoded = n1___n_name_encoded[BUF_8[SLOT_8->second * 2 + 1]];

KEY_10 |= reg_n1___n_name_encoded;
auto reg_n2___n_name_encoded = n2___n_name_encoded[BUF_6[SLOT_6->second * 3 + 1]];
KEY_10 <<= 16;
...
__global__ void main_11(uint64_t* BUF_6, uint64_t* BUF_8, HASHTABLE_FIND HT_10, HASHTABLE_PROBE_PK HT_6, HASHTABLE_PROBE_PK HT_8, DBI64Type* KEY_10map0__tmp_attr0, DBI16Type* KEY_10n1___n_name_encoded, DBI16Type* KEY_10n2___n_name_encoded, DBDecimalType* aggr0__tmp_attr2, DBDecimalType* lineitem__l_discount, DBDecimalType* lineitem__l_extendedprice, DBI32Type* lineitem__l_orderkey, DBDateType* lineitem__l_shipdate, DBI32Type* lineitem__l_suppkey, size_t lineitem_size, DBStringType* n1___n_name, DBI16Type* n1___n_name_encoded, DBStringType* n2___n_name, DBI16Type* n2___n_name_encoded) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= lineitem_size) return;
auto reg_lineitem__l_shipdate = lineitem__l_shipdate[tid];
if (!(evaluatePredicate(reg_lineitem__l_shipdate, 9131, Predicate::gte) && evaluatePredicate(reg_lineitem__l_shipdate, 9861, Predicate::lte))) return;
...
count_1<<<std::ceil((float)nation_size/128.), 128>>>(d_COUNT0, d_nation__n_name, nation_size);
uint64_t COUNT0;
...
count_3<<<std::ceil((float)nation_size/128.), 128>>>(d_COUNT2, d_nation__n_name, nation_size);
uint64_t COUNT2;
...
count_7<<<std::ceil((float)orders_size/128.), 128>>>(d_BUF_4, d_COUNT6, d_HT_4.ref(cuco::find), d_orders__o_custkey, orders_size);
uint64_t COUNT6;
...
main_7<<<std::ceil((float)orders_size/128.), 128>>>(d_BUF_4, d_BUF_6, d_BUF_IDX_6, d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::insert), d_orders__o_custkey, d_orders__o_orderkey, orders_size);
//Materialize count
uint64_t* d_COUNT8;
cudaMalloc(&d_COUNT8, sizeof(uint64_t));
cudaMemset(d_COUNT8, 0, sizeof(uint64_t));
count_9<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_0, d_COUNT8, d_HT_0.ref(cuco::find), d_supplier__s_nationkey, supplier_size);
uint64_t COUNT8;
...
main_9<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_0, d_BUF_8, d_BUF_IDX_8, d_HT_0.ref(cuco::find), d_HT_8.ref(cuco::insert), d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
//Create aggregation hash table
auto d_HT_10 = cuco::static_map{ (int)118381*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_11<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::insert), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_lineitem__l_orderkey, d_lineitem__l_shipdate, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name, d_nation__n_name_encoded, d_nation__n_name, d_nation__n_name_encoded);
...
main_11<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_10map0__tmp_attr0, d_KEY_10n1___n_name_encoded, d_KEY_10n2___n_name_encoded, d_aggr0__tmp_attr2, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_shipdate, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name, d_nation__n_name_encoded, d_nation__n_name, d_nation__n_name_encoded);
//Materialize count
uint64_t* d_COUNT12;
