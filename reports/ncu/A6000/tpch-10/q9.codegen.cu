...
__global__ void main_1(uint64_t* BUF_0, uint64_t* BUF_IDX_0, HASHTABLE_INSERT_PK HT_0, DBI32Type* nation__n_nationkey, size_t nation_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= nation_size) return;
uint64_t KEY_0 = 0;
...
__global__ void count_3(uint64_t* BUF_0, uint64_t* COUNT2, HASHTABLE_PROBE_PK HT_0, DBI32Type* supplier__s_nationkey, size_t supplier_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void main_3(uint64_t* BUF_0, uint64_t* BUF_2, uint64_t* BUF_IDX_2, HASHTABLE_PROBE_PK HT_0, HASHTABLE_INSERT_PK HT_2, DBI32Type* supplier__s_nationkey, DBI32Type* supplier__s_suppkey, size_t supplier_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= supplier_size) return;
uint64_t KEY_0 = 0;
auto reg_supplier__s_nationkey = supplier__s_nationkey[tid];
...
BUF_2[buf_idx_2 * 2 + 1] = BUF_0[SLOT_0->second * 1 + 0];
}
...
template<typename HASHTABLE_INSERT_PK>
__global__ void main_5(uint64_t* BUF_4, uint64_t* BUF_IDX_4, HASHTABLE_INSERT_PK HT_4, DBI32Type* orders__o_orderkey, size_t orders_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= orders_size) return;
uint64_t KEY_4 = 0;
...
template<typename HASHTABLE_INSERT_PK>
__global__ void main_7(uint64_t* BUF_6, uint64_t* BUF_IDX_6, HASHTABLE_INSERT_PK HT_6, DBI32Type* part__p_partkey, size_t part_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= part_size) return;
uint64_t KEY_6 = 0;
...
template<typename HASHTABLE_PROBE_PK>
__global__ void count_9(uint64_t* BUF_2, uint64_t* COUNT8, HASHTABLE_PROBE_PK HT_2, DBI32Type* partsupp__ps_suppkey, size_t partsupp_size) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template<typename HASHTABLE_PROBE_PK, typename HASHTABLE_INSERT_PK>
__global__ void main_9(uint64_t* BUF_2, uint64_t* BUF_8, uint64_t* BUF_IDX_8, HASHTABLE_PROBE_PK HT_2, HASHTABLE_INSERT_PK HT_8, DBI32Type* partsupp__ps_partkey, DBI32Type* partsupp__ps_suppkey, size_t partsupp_size, DBI32Type* supplier__s_suppkey) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
if (tid >= partsupp_size) return;
uint64_t KEY_2 = 0;
auto reg_partsupp__ps_suppkey = partsupp__ps_suppkey[tid];
...
if (!(true)) return;
uint64_t KEY_8 = 0;
auto reg_supplier__s_suppkey = supplier__s_suppkey[BUF_2[SLOT_2->second * 2 + 0]];
...
BUF_8[buf_idx_8 * 3 + 0] = BUF_2[SLOT_2->second * 2 + 0];
BUF_8[buf_idx_8 * 3 + 1] = tid;
BUF_8[buf_idx_8 * 3 + 2] = BUF_2[SLOT_2->second * 2 + 1];
}
template<typename HASHTABLE_INSERT, typename HASHTABLE_PROBE_PK>
__global__ void count_11(uint64_t* BUF_4, uint64_t* BUF_6, uint64_t* BUF_8, HASHTABLE_INSERT HT_10, HASHTABLE_PROBE_PK HT_4, HASHTABLE_PROBE_PK HT_6, HASHTABLE_PROBE_PK HT_8, DBI32Type* lineitem__l_orderkey, DBI32Type* lineitem__l_partkey, DBI32Type* lineitem__l_suppkey, size_t lineitem_size, DBI16Type* nation__n_name_encoded, DBDateType* orders__o_orderdate) {
size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
uint64_t KEY_10 = 0;
auto reg_nation__n_name_encoded = nation__n_name_encoded[BUF_8[SLOT_8->second * 3 + 2]];
...
auto reg_partsupp__ps_supplycost = partsupp__ps_supplycost[BUF_8[SLOT_8->second * 3 + 1]];
...
count_3<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_0, d_COUNT2, d_HT_0.ref(cuco::find), d_supplier__s_nationkey, supplier_size);
uint64_t COUNT2;
...
main_3<<<std::ceil((float)supplier_size/128.), 128>>>(d_BUF_0, d_BUF_2, d_BUF_IDX_2, d_HT_0.ref(cuco::find), d_HT_2.ref(cuco::insert), d_supplier__s_nationkey, d_supplier__s_suppkey, supplier_size);
//Materialize count
uint64_t* d_COUNT4;
...
count_9<<<std::ceil((float)partsupp_size/128.), 128>>>(d_BUF_2, d_COUNT8, d_HT_2.ref(cuco::find), d_partsupp__ps_suppkey, partsupp_size);
uint64_t COUNT8;
...
main_9<<<std::ceil((float)partsupp_size/128.), 128>>>(d_BUF_2, d_BUF_8, d_BUF_IDX_8, d_HT_2.ref(cuco::find), d_HT_8.ref(cuco::insert), d_partsupp__ps_partkey, d_partsupp__ps_suppkey, partsupp_size, d_supplier__s_suppkey);
//Create aggregation hash table
auto d_HT_10 = cuco::static_map{ (int)479888417*2, cuco::empty_key{(int64_t)-1},cuco::empty_value{(int64_t)-1},thrust::equal_to<int64_t>{},cuco::linear_probing<1, cuco::default_hash_function<int64_t>>() };
count_11<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_4, d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::insert), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_lineitem__l_orderkey, d_lineitem__l_partkey, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded, d_orders__o_orderdate);
...
main_11<<<std::ceil((float)lineitem_size/128.), 128>>>(d_BUF_4, d_BUF_6, d_BUF_8, d_HT_10.ref(cuco::find), d_HT_4.ref(cuco::find), d_HT_6.ref(cuco::find), d_HT_8.ref(cuco::find), d_KEY_10map0__tmp_attr0, d_KEY_10nation__n_name_encoded, d_aggr0__tmp_attr2, d_lineitem__l_discount, d_lineitem__l_extendedprice, d_lineitem__l_orderkey, d_lineitem__l_partkey, d_lineitem__l_quantity, d_lineitem__l_suppkey, lineitem_size, d_nation__n_name_encoded, d_orders__o_orderdate, d_partsupp__ps_supplycost);
//Materialize count
uint64_t* d_COUNT12;
